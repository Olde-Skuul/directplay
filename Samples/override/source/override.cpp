...
static HRESULT UpdateAddressInfo(HWND hWnd, LPDIRECTPLAYLOBBY3A /* pDPLobby */)
{
	...
	HRESULT hr = GetServiceProviderGuid(hWnd, &guidServiceProvider);
	if (FAILED(hr)) {
		return hr;
	}
	...
	DWORD /* dwDataSize */, const VOID* pData, VOID* pContext)
{
	...
	if (IsEqualGUID(guidDataType, DPAID_Modem)) {
		...
		while (strlen(strStr)) {
			...
			SendDlgItemMessageA(
				hWnd, IDC_MODEMCOMBO, CB_ADDSTRING, 0, (LPARAM)strStr);
...
// Returns:
//  HRESULT                 any error
static HRESULT FillModemComboBox(HWND hWnd, LPDIRECTPLAYLOBBY3A pDPLobby)
{
	...
	HRESULT hr = DirectPlayCreate(&guidServiceProvider, &pDP1, NULL);
	if (FAILED(hr)) {
		return hr;
	}
	...
	SendDlgItemMessageA(hWnd, IDC_MODEMCOMBO, CB_SETCURSEL, 0, 0);
...
static BOOL FAR PASCAL EnumConnectionsCallback(const GUID* pSPGUID,
	VOID* /* pConnection */, DWORD /* dwConnectionSize */, const DPNAME* pName,
...
static HRESULT InitializeOverrideWindow(
	HWND hWnd, LPDIRECTPLAYLOBBY3A* ppDPLobby)
{
	...
	SendDlgItemMessageA(hWnd, IDC_PORTEDIT, EM_LIMITTEXT, 5, 0);
...
// Desc:
//-----------------------------------------------------------------------------
static BOOL CALLBACK SessionsWndProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
		...
		EnableDlgButton(
			hWnd, IDC_JOINSESSIONBUTTON, (iIndex > 0) ? TRUE : FALSE);

		switch (hr) {
		case DP_OK:
			SetDlgItemTextA(
				hWnd, IDC_SESSIONSTATUSEDIT, "Searching for sessions...");
			break;

		case DPERR_CONNECTING:
			SetDlgItemTextA(
				hWnd, IDC_SESSIONSTATUSEDIT, "Making connection...");
			break;

		case DPERR_NOCONNECTION:
			SetDlgItemTextA(hWnd, IDC_SESSIONSTATUSEDIT, "Connection failed");
			KillTimer(hWnd, idTimer);
			idTimer = 0;
			break;
		...
		default:
			SetDlgItemTextA(
				hWnd, IDC_SESSIONSTATUSEDIT, "Error making connection");
			KillTimer(hWnd, idTimer);
			idTimer = 0;
			break;
		}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT CreateServiceProviderAddress(HWND hWnd,
	LPDIRECTPLAYLOBBY3A pDPLobby, VOID** ppAddress, DWORD* pdwAddressSize)
{
	...
	if (IsEqualGUID(guidServiceProvider, DPSPGUID_MODEM)) {
		// Modem needs a service provider, a phone number string and a modem
		// string
		...
		strcpy(strModemString, "");
		if (GetDlgItemTextA(hWnd, IDC_MODEMCOMBO, strModemString, NAMEMAX)) {
		...
		strcpy(strPhoneNumberString, "");
		GetDlgItemTextA(hWnd, IDC_PHONEEDIT, strPhoneNumberString, NAMEMAX);
		...
		// IP address string
		strcpy(strIPAddressString, "");
		GetDlgItemTextA(hWnd, IDC_TCPEDIT, strIPAddressString, NAMEMAX);
		...
		WORD wPort =
			static_cast<WORD>(GetDlgItemInt(hWnd, IDC_PORTEDIT, NULL, FALSE));
		if (wPort > 0) {
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoHostOrJoin(HINSTANCE hInstance, HWND hWnd,
	LPDIRECTPLAYLOBBY3A pDPLobby, BOOL bHost, LPDIRECTPLAY4A* ppDP)
{
	LPDIRECTPLAY4A pDP = NULL;
	VOID* pAddress = NULL;
	DWORD dwAddressSize = 0;
	...
	HRESULT hr =
		CreateServiceProviderAddress(hWnd, pDPLobby, &pAddress, &dwAddressSize);
	if (FAILED(hr)) {
		return hr;
	}
...
// Desc:
//-----------------------------------------------------------------------------
static VOID DeleteServiceProviderCombo(HWND hWnd)
{
...
// Desc:
//-----------------------------------------------------------------------------
static VOID DestroyOverrideWindow(
	HWND hWnd, LPDIRECTPLAY4A pDP, LPDIRECTPLAYLOBBY3A pDPLobby)
{
...
static BOOL CALLBACK OverrideWndProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		if (FAILED(hr)) {
			SetDlgItemTextA(
				hWnd, IDC_STATUSEDIT, "Could not initialize DirectPlay");
		}
		return TRUE;
	...
	case WM_COMMAND:
		...
		case IDC_HOSTBUTTON:
			SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Hosting...");
			hr = DoHostOrJoin(hInstance, hWnd, pDPLobby, TRUE, &pDP);
			if (FAILED(hr)) {
				SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Failed to host");
			} else {
				SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Host successfull");
			}
			return TRUE;

		case IDC_JOINBUTTON:
			SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Joining...");
			hr = DoHostOrJoin(hInstance, hWnd, pDPLobby, FALSE, &pDP);
			if (FAILED(hr)) {
				SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Failed to join");
			} else {
				SetDlgItemTextA(hWnd, IDC_STATUSEDIT, "Join successfull");
			}
			return TRUE;
