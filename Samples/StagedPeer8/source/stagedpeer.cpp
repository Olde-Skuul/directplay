...
GUID g_guidApp = {0x2d6d17ec, 0x2e22, 0x499a,
	{0xb4, 0xf0, 0x7d, 0x40, 0x88, 0x64, 0xe5, 0xc8}};
...
HRESULT WINAPI DirectPlayLobbyMessageHandler(
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer);
INT_PTR CALLBACK GreetingDlgProc(
		...
		for (;;) {
			...
			case 1:
				if (!g_bWasLobbyLaunched) {
					...
					hr =
						g_pNetStage->DoStage(g_strLocalPlayerName, bHostPlayer);
				} else {
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	if (g_pNetConnectWizard) {
		g_pNetConnectWizard->MessageHandler(
	...
	if (g_pNetStage) {
		hr =
			g_pNetStage->MessageHandler(pvUserContext, dwMessageId, pMsgBuffer);
