...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetStage::StaticStageDlgProc(
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetStage::StageDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_APP_DISPLAY_STAGE: {
		// This function is called from the DirectPlay message handler so it
		// could be called simultaneously from multiple threads, so enter a
		// critical section to assure that it we don't get race conditions.
	...
	case WM_APP_CHAT: {
		...
		wsprintf(strChatBuffer, TEXT("<%s> %s"), pPlayerInfo->strPlayerName,
		...
		int nIndex = (int)SendMessage(
			hWndChatBox, LB_ADDSTRING, 0, (LPARAM)strChatBuffer);
		SendMessage(
			hWndChatBox, LB_SETTOPINDEX, static_cast<WPARAM>(nIndex), 0);
	...
	case WM_COMMAND:
		...
		case IDC_READY_P10:
			...
			if (FAILED(m_hrDialog = ChangePlayerReadyStatus(
						   hDlg, !m_bLocalPlayerReady))) {
		...
		case IDC_SLOT_BTN_P10:
			...
			if (m_bHostPlayer) {
				DWORD dwSlot =
					static_cast<DWORD>(LOWORD(wParam) - IDC_SLOT_BTN_P2 + 1);
...
//-----------------------------------------------------------------------------
HRESULT WINAPI CNetStage::MessageHandler(
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_RECEIVE: {
		...
		STAGEMSG_GENERIC* pStageMsg =
			(STAGEMSG_GENERIC*)pReceiveMsg->pReceiveData;
	...
	case DPN_MSGID_CREATE_PLAYER: {
		...
		APP_PLAYER_INFO* pPlayerInfo =
			(APP_PLAYER_INFO*)pCreatePlayerMsg->pvPlayerContext;
		...
		if (m_bHostPlayer) {
			...
			DXUtil_Trace(
				TEXT(
					"NetStage: adding player (name=%s, dpid=%0.8x, context=%0.8x)\n"),
				pPlayerInfo->strPlayerName, pCreatePlayerMsg->dpnidPlayer,
				pPlayerInfo);
	...
	case DPN_MSGID_DESTROY_PLAYER: {
		...
		if (m_bHostPlayer) {
			DXUtil_Trace(
				TEXT(
					"NetStage: removing player (name=%s, dpid=%0.8x, context=%0.8x)\n"),
				pPlayerInfo->strPlayerName, pPlayerInfo->dpnidPlayer,
				pPlayerInfo);
	...
	case DPN_MSGID_TERMINATE_SESSION: {
		...
		if (pTerminateSessionMsg->pvTerminateData != NULL &&
			...
			*((DWORD*)(pTerminateSessionMsg->pvTerminateData)) ==
				NS_S_REJECTED) {
...
//-----------------------------------------------------------------------------
HRESULT CNetStage::HandleAppMessages(
	HWND hDlg, CNetStage::STAGEMSG_GENERIC* pMsg, PDPNMSG_RECEIVE pReceiveMsg)
{
	HRESULT hr;
	...
	case STAGE_MSGID_CHAT: {
		...
		PostMessage(m_hDlg, WM_APP_CHAT, pPlayerInfo->dpnidPlayer,
			(LPARAM)pChatMessage);
...
HRESULT CNetStage::SendChatMessage(HWND hDlg)
{
	...
	DWORD dwEditboxBufferSize =
		(DWORD)SendDlgItemMessage(hDlg, IDC_CHAT_EDIT, WM_GETTEXTLENGTH, 0, 0);
	...
	GetDlgItemText(
		hDlg, IDC_CHAT_EDIT, msgChat.strChatString, MAX_CHAT_STRING_LENGTH);
...
VOID CNetStage::AddChatStringToListBox(HWND hDlg, LPSTR strMsgText)
{
	...
	// Add it, and make sure it is visible
	int nIndex =
		(int)SendMessage(hWndChatBox, LB_ADDSTRING, 0, (LPARAM)strMsgText);
	SendMessage(hWndChatBox, LB_SETTOPINDEX, static_cast<WPARAM>(nIndex), 0);
}
...
HRESULT CNetStage::GetFreePlayerSlot(DWORD* pdwPlayerSlotID)
{
	...
	for (iSlot = 0; iSlot < MAX_SLOTS; iSlot++) {
		if (SLOT_BUTTON_OPEN ==
			(m_Slots[iSlot].dwSlotStatus & SLOT_BUTTON_MASK)) {
...
HRESULT CNetStage::SendSlotDataToPlayers(HWND /* hDlg */)
{
	...
	// section to assure that it we don't get race conditions.
	EnterCriticalSection(&m_csSlots);
	...
	DXUtil_Trace(
		TEXT("NetStage: sending STAGE_MSGID_SETSTAGEDATA to all players\n"));
	...
	if (bEveryOneReady) {
		if (m_bHostPlayer) {
			if (m_dwNumberOfFullSlots == 1) {
				...
				_tcscpy(strStatus,
					TEXT("All are ready! Waiting for you to start the game."));
			...
			_tcscpy(strStatus,
				TEXT("All are ready! Waiting for host to start the game."));
		}
	} else {
		if (m_bLocalPlayerReady) {
			_tcscpy(strStatus,
				TEXT("Waiting until all players check the Ready flag."));
		} else {
			_tcscpy(strStatus,
				TEXT("Ready?  Flag the checkbox next to your name."));
		}
	}
...
HRESULT CNetStage::AddPlayerToStage(HWND hDlg, APP_PLAYER_INFO* pPlayerInfo)
{
	...
	if (hr != S_OK) {
		...
		m_pDP->DestroyPeer(
			pPlayerInfo->dpnidPlayer, &dwDestroyData, sizeof(DWORD), 0);
	...
	msgSlotID.lSlotNumber = static_cast<LONG>(dwPlayerSlot);
	...
	// if there are any severe errors, so ignore any errors
	DPNHANDLE hAsync;
	m_pDP->SendTo(pPlayerInfo->dpnidPlayer, &bufferDesc, 1, 0, NULL, &hAsync,
		DPNSEND_GUARANTEED);

	DXUtil_Trace(
		TEXT(
			"NetStage: sending new player (dpid=%0.8x) a STAGE_MSGID_SETSLOTID message\n"),
...
HRESULT CNetStage::RemovePlayerToStage(HWND hDlg, APP_PLAYER_INFO* pPlayerInfo)
{
	...
	if (SLOT_BUTTON_FULL ==
		(m_Slots[pPlayerInfo->dwSlotIndex].dwSlotStatus & SLOT_BUTTON_MASK)) {
...
HRESULT CNetStage::ChangePlayerReadyStatus(HWND hDlg, BOOL bPlayerReady)
{
	...
	// handler threads simultaneously change m_Slots, so enter a critical
	// section to assure that it we don't get race conditions.
	EnterCriticalSection(&m_csSlots);
	...
	if (m_bHostPlayer) {
		...
		// if there are any severe errors, so ignore any errors
		DPNHANDLE hAsync;
		m_pDP->SendTo(m_dpnidHostPlayer, &bufferDesc, 1, 0, NULL, &hAsync,
			DPNSEND_GUARANTEED);
	}
...
HRESULT CNetStage::ChangePlayerSlotStatus(HWND hDlg, DWORD dwSlot)
{
	...
	case SLOT_BUTTON_FULL: {
		...
		wsprintf(strQuestion, TEXT("Do you want to reject %s?"),
			m_Slots[dwSlot].strSlotName);
		nResult = MessageBox(
			hDlg, strQuestion, TEXT("DirectPlay Multiplayer Stage"), MB_YESNO);
		if (nResult == IDNO) {
			hr = S_OK;
			goto LCleanup;
		}
		...
		DWORD dwDestroyData = NS_S_REJECTED;
		m_pDP->DestroyPeer(
			m_Slots[dwSlot].dpnidPlayer, &dwDestroyData, sizeof(DWORD), 0);
...
VOID CNetStage::UpdateStageWhenInitComplete()
{
	...
	// dialog has competed
	if (m_bStageInitialized && m_pStageDataMsg != NULL &&
		m_bDialogInitialized) {
		assert(m_lLocalSlotNumber >= 0);
		assert(m_hDlg != NULL);
		...
		DXUtil_Trace(
			TEXT(
				"NetStage: updating stage (m_hDlg=%0.8x m_lLocalSlotNumber=%d m_pStageDataMsg=%0.8x\n)"),
		...
		// Disable all of the ready checkboxes.
		int nIDDlgItem;
		for (nIDDlgItem = IDC_READY_P1; nIDDlgItem <= IDC_READY_P10;
			 nIDDlgItem++) {
			EnableWindow(GetDlgItem(m_hDlg, nIDDlgItem), FALSE);
		}
		EnableWindow(
			GetDlgItem(m_hDlg, IDC_READY_P1 + m_lLocalSlotNumber), TRUE);
