...
GUID g_guidApp = {0xba214178, 0xaae6, 0x4ea6,
	{0x84, 0xe0, 0x65, 0xce, 0x36, 0xf8, 0x44, 0x79}};
...
APP_PLAYER_INFO* g_pConnInfoTargetPlayer =
	NULL; // The current target for Connection Info
...
//-----------------------------------------------------------------------------
static HRESULT LinkPlayer(DPNID dpnid, APP_PLAYER_INFO** ppPlayerInfo)
{
	...
	while (hr == DPNERR_CONNECTING) {
		hr = g_pDP->GetPeerInfo(dpnid, NULL, &dwSize, 0);
	}
	...
	// Enter the player list CS, because we are about to modify shared memory,
	// and the data may be changed concurrently from the DirectPlay message
	// handler threads
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_CREATE_PLAYER: {
		...
		if (FAILED(
				hr = LinkPlayer(pCreatePlayerMsg->dpnidPlayer, &pPlayerInfo))) {
	...
	case DPN_MSGID_RECEIVE: {
		...
		if (pMsg->dwType == GAME_MSGID_GAMEPACKET) {
			...
			// Enter the data list CS, because we are about to modify shared
			// memory, and the data may be changed concurrently from other
			// DirectPlay message handler threads, as well as the the app worker
			// thread
	...
	case DPN_MSGID_SEND_COMPLETE: {
		PDPNMSG_SEND_COMPLETE pSendCompleteMsg =
			(PDPNMSG_SEND_COMPLETE)pMsgBuffer;

		GAMEMSG_GENERIC* pGameMsg =
			(GAMEMSG_GENERIC*)pSendCompleteMsg->pvUserContext;
	...
	if (DPNSUCCESS_PENDING != hReturn && SUCCEEDED(hReturn) &&
		g_pNetConnectWizard) {
		hReturn = g_pNetConnectWizard->MessageHandler(
...
//-----------------------------------------------------------------------------
static VOID ReadCombos(HWND hDlg)
{
	TCHAR strText[128];

	// Get target player
	HWND hTargetCombo = GetDlgItem(hDlg, IDC_SEND_TARGET_COMBO);
	int nCurSelect = (int)SendMessage(hTargetCombo, CB_GETCURSEL, 0, 0);
	if (nCurSelect != CB_ERR) {
		g_pTargetPlayer = (APP_PLAYER_INFO*)SendMessage(
			hTargetCombo, CB_GETITEMDATA, static_cast<WPARAM>(nCurSelect), 0);
	}
	...
	if (nCurSelect != CB_ERR) {
		SendMessage(hRateCombo, CB_GETLBTEXT, static_cast<WPARAM>(nCurSelect),
			(LPARAM)strText);
		g_dwTimeBetweenSends = static_cast<DWORD>(_ttoi(strText));
	...
	if (nCurSelect != CB_ERR) {
		SendMessage(hSizeCombo, CB_GETLBTEXT, static_cast<WPARAM>(nCurSelect),
			(LPARAM)strText);
		g_dwSendSize = static_cast<DWORD>(_ttoi(strText));
	}
	...
	if (nCurSelect != CB_ERR) {
		SendMessage(hTimeoutCombo, CB_GETLBTEXT,
			static_cast<WPARAM>(nCurSelect), (LPARAM)strText);
		g_dwSendTimeout = static_cast<DWORD>(_ttoi(strText));
	}
	...
	if (nCurSelect != CB_ERR) {
		g_pConnInfoTargetPlayer =
			(APP_PLAYER_INFO*)SendMessage(hConnTargetCombo, CB_GETITEMDATA,
				static_cast<WPARAM>(nCurSelect), 0);
	}
}
	...
	while (pNode != &g_DataHead) {
		...
		case DATA_TYPE_NETPACKET_RECIEVE:
			wsprintf(strNewLogLine, TEXT("Got %s's #%d (%d bytes)\r\n"),
				pNode->pPlayerFrom->strPlayerName, pNode->dwPacketId,
		...
		case DATA_TYPE_NETPACKET_SENT:
			wsprintf(strNewLogLine, TEXT("Sending #%d to %s (%d bytes)\r\n"),
				pNode->dwPacketId, g_pTargetPlayer->strPlayerName,
				pNode->dwReceiveDataSize);
			break;
...
//-----------------------------------------------------------------------------
static UINT WINAPI ProcessNetDataProc(LPVOID /* lpParameter */)
{
	...
	while (!bDone) {
		DWORD dwResult = WaitForMultipleObjects(2, ahHandles, FALSE, INFINITE);
...
//-----------------------------------------------------------------------------
static VOID FillOneTimeCombos(HWND hDlg)
{
	if (hDlg == NULL) {
		return;
	}
...
static HRESULT OnInitDialog(HWND hDlg)
{
	...
	g_hProcessNetDataThread = (HANDLE)_beginthreadex(
		NULL, 0, ProcessNetDataProc, hDlg, 0, &g_dwProcessNetDataThreadID);
...
//-----------------------------------------------------------------------------
static VOID FillTargetCombo(HWND hDlg)
{
	...
	int nCurSelect = (int)SendMessage(hTargetCombo, CB_GETCURSEL, 0, 0);
	LONG_PTR lCurItemData = (LONG_PTR)SendMessage(
		hTargetCombo, CB_GETITEMDATA, static_cast<WPARAM>(nCurSelect), 0);
	...
	// Add "everyone"
	int nIndex = (int)SendMessage(
		hTargetCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("Everyone"));
	SendMessage(hTargetCombo, CB_SETITEMDATA, static_cast<WPARAM>(nIndex),
		(LPARAM)&g_PlayerHead);
	...
	while (pNode != &g_PlayerHead) {
		if ((pNode->dwFlags & DPNPLAYER_LOCAL) == 0) {
			nIndex = (int)SendMessage(
				hTargetCombo, CB_ADDSTRING, 0, (LPARAM)pNode->strPlayerName);
			SendMessage(hTargetCombo, CB_SETITEMDATA,
				static_cast<WPARAM>(nIndex), (LPARAM)pNode);
	...
	SendMessage(hTargetCombo, CB_SETCURSEL, static_cast<WPARAM>(nNewSelect), 0);
	...
	HWND hInfoTargetCombo = GetDlgItem(hDlg, IDC_CONNINFO_COMBO);

	nCurSelect = (int)SendMessage(hInfoTargetCombo, CB_GETCURSEL, 0, 0);
	lCurItemData = (LONG_PTR)SendMessage(
		hInfoTargetCombo, CB_GETITEMDATA, static_cast<WPARAM>(nCurSelect), 0);
	...
	// Add "none"
	nIndex = (int)SendMessage(
		hInfoTargetCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("None"));
	SendMessage(hInfoTargetCombo, CB_SETITEMDATA, static_cast<WPARAM>(nIndex),
		(LPARAM)&g_PlayerHead);
	...
	while (pNode != &g_PlayerHead) {
		if ((pNode->dwFlags & DPNPLAYER_LOCAL) == 0) {
			nIndex = (int)SendMessage(hInfoTargetCombo, CB_ADDSTRING, 0,
				(LPARAM)pNode->strPlayerName);
			SendMessage(hInfoTargetCombo, CB_SETITEMDATA,
				static_cast<WPARAM>(nIndex), (LPARAM)pNode);
			if (lCurItemData == (LONG_PTR)pNode) {
				nNewSelect = nIndex;
			}
	...
	SendMessage(
		hInfoTargetCombo, CB_SETCURSEL, static_cast<WPARAM>(nNewSelect), 0);
}
...
//-----------------------------------------------------------------------------
static HRESULT SendNetworkData()
{
	HRESULT hr;
...
// Desc: Using the info buffer passed in,  display some interesting tidbits
// about
//       the connection.
//-----------------------------------------------------------------------------
static VOID UpdateConnectionInfo(HWND hDlg,
	PDPN_CONNECTION_INFO pConnectionInfo, DWORD dwHighMessages,
	DWORD dwHighBytes, DWORD dwNormalMessages, DWORD dwNormalBytes,
	DWORD dwLowMessages, DWORD dwLowBytes)
{
	...
	if (pConnectionInfo) {
		int nDrops = static_cast<int>(pConnectionInfo->dwPacketsDropped +
			pConnectionInfo->dwPacketsRetried);
		nDrops *= 10000;

		int nSends = static_cast<int>(pConnectionInfo->dwPacketsSentGuaranteed +
		...
		_stprintf(strText,
			...
			dwHighMessages, dwHighBytes, dwNormalMessages, dwNormalBytes,
			...
			pConnectionInfo->dwBytesRetried, pConnectionInfo->dwPacketsRetried,
			pConnectionInfo->dwBytesDropped, pConnectionInfo->dwPacketsDropped,
...
// Desc:
//-----------------------------------------------------------------------------
static VOID UpdateStats(HWND hDlg)
{
	HRESULT hr;
	...
	//  If a player has been selected for Connection Info Display we will handle
	//  it next
	if (g_pConnInfoTargetPlayer->dpnidPlayer != 0) {
		...
		hr = g_pDP->GetConnectionInfo(
			g_pConnInfoTargetPlayer->dpnidPlayer, &dpnConnectionInfo, 0);
		...
		if (SUCCEEDED(hr)) {
			...
			hr = g_pDP->GetSendQueueInfo(g_pConnInfoTargetPlayer->dpnidPlayer,
				&dwLowMessages, &dwLowBytes, DPNGETSENDQUEUEINFO_PRIORITY_LOW);
			if (FAILED(hr)) {
				DXTRACE_ERR(TEXT("GetSendQueueInfo"), hr);
			}

			UpdateConnectionInfo(hDlg, &dpnConnectionInfo, dwHighMessages,
				dwHighBytes, dwNormalMessages, dwNormalBytes, dwLowMessages,
				dwLowBytes);
		} else {
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK SampleDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	...
	case WM_APP_UPDATE_TARGETS: {
		...
		CheckDlgButton(hDlg, IDC_SEND_READY,
			static_cast<UINT>((g_bSendingData) ? BST_CHECKED : BST_UNCHECKED));
	...
	case WM_APP_APPEND_TEXT: {
		...
		AppendTextToEditControl(g_hDlg, strNewLogLine, IDC_LOG_EDIT, 18);
	...
	case WM_COMMAND: {
		...
		case IDC_SEND_READY:
			g_bSendingData =
				(IsDlgButtonChecked(hDlg, IDC_SEND_READY) == BST_CHECKED);
			if (g_bSendingData) {
				SetTimer(g_hDlg, TIMERID_NETWORK, g_dwTimeBetweenSends, NULL);
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	if (g_bWasLobbyLaunched &&
		...
		if (FAILED(hr = g_pNetConnectWizard->ConnectUsingLobbySettings())) {
			...
			_tcsncpy(g_strLocalPlayerName, g_pNetConnectWizard->GetPlayerName(),
				MAX_PLAYER_NAME);
	...
	// Cleanup circular linked list, g_PlayerHead
	{
		...
		APP_PLAYER_INFO* pNode = g_PlayerHead.pNext;
		while (pNode != &g_PlayerHead) {
			APP_PLAYER_INFO* pDeleteNode = pNode;
			pNode = pNode->pNext;
			SAFE_DELETE(pDeleteNode);
		}
		LeaveCriticalSection(&g_csPlayerList);
	...
	// Cleanup circular linked list, g_DataHead
	{
		...
		GAMEMSG_DATA_NODE* pNode = g_DataHead.pNext;
		while (pNode != &g_DataHead) {
			GAMEMSG_DATA_NODE* pDeleteNode = pNode;
			pNode = pNode->pNext;
			SAFE_DELETE(pDeleteNode);
		}
		LeaveCriticalSection(&g_csDataList);
