...
GUID g_guidApp = {0x9f2da206, 0xd197, 0x4871,
	{0xad, 0x1d, 0xd1, 0x2f, 0x59, 0x5, 0x8b, 0x5b}};
...
//-----------------------------------------------------------------------------
static HRESULT SendLocalStateToAll(HWND /* hDlg */)
{
	if (g_pPlayerLocal) {
	...
	case DPN_MSGID_CREATE_PLAYER: {
		...
		while (hr == DPNERR_CONNECTING) {
			hr = g_pDP->GetPeerInfo(
				pCreatePlayerMsg->dpnidPlayer, NULL, &dwSize, 0);
		}
	...
	case DPN_MSGID_RECEIVE: {
		...
		if (pMsg->dwType == GAME_MSGID_PLAYERSTATECHANGE) {
			GAMEMSG_PLAYERSTATECHANGE* pPlayerStateChangedMsg =
				(GAMEMSG_PLAYERSTATECHANGE*)pMsg;
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT SetGroupAndTarget()
{
	HRESULT hr;
	...
	if (g_LocalGroupDPNID != 0) {
		...
		DPNHANDLE hAsync;
		hr = g_pDP->RemovePlayerFromGroup(
			g_LocalGroupDPNID, g_LocalPlayerDPNID, NULL, &hAsync, 0);
		if (FAILED(hr)) {
	...
	do {
		...
		hr =
			g_pDP->EnumPlayersAndGroups(aGroupsDPNID, &dwCount, DPNENUM_GROUPS);
	...
	for (DWORD i = 0; i < dwCount; i++) {
		...
		if (dwGroupNumber == g_pPlayerLocal->ps.dwGroup) {
			...
			DPNHANDLE hAsync;
			hr = g_pDP->AddPlayerToGroup(
				aGroupsDPNID[i], g_LocalPlayerDPNID, NULL, &hAsync, 0);
			if (FAILED(hr)) {
...
static HRESULT OnInitDialog(HWND hDlg)
{
	...
	dwVertScrollBar = static_cast<DWORD>(GetSystemMetrics(SM_CXVSCROLL));
	...
	column.cx = static_cast<int>(dwListViewWidth * 8 / 21);
	...
	column.cx = static_cast<int>(dwListViewWidth * 4 / 21);
	column.pszText = strHeader;
	column.cchTextMax = static_cast<int>(_tcslen(strHeader));
	...
	column.cx = static_cast<int>(dwListViewWidth * 5 / 21);
	column.pszText = strHeader;
	column.cchTextMax = static_cast<int>(_tcslen(strHeader));
	...
	if (g_bHostPlayer) {
		...
		for (dwIndex = 1; dwIndex < MAX_GROUPS + 1; dwIndex++) {
			...
			*pdwData = dwIndex;
			...
			DPNHANDLE hAsync;
			hr = g_pDP->CreateGroup(&dpGroupInfo, NULL, NULL, &hAsync, 0);
			if (FAILED(hr)) {
...
static VOID VoiceConfigDlgOnOK(HWND hDlg)
{
	...
	if (g_bHostPlayer) {
		...
		lCurSelection = (LONG)SendDlgItemMessage(
			hDlg, IDC_COMPRESSION_COMBO, CB_GETCURSEL, 0, 0);
	...
	for (DWORD i = 0; i < dwNumPlayers; i++) {
		...
		do {
			...
			hr = g_pDP->GetPlayerContext(
				g_pPlayers[i], (LPVOID* const)&pPlayerInfo, 0);
		...
		lvItem.cchTextMax = static_cast<int>(_tcslen(strText));
	...
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	...
	case WM_INITDIALOG: {
		...
		if (g_pNetVoice->IsHalfDuplex()) {
			MessageBox(hDlg,
				TEXT(
					"You are running in half duplex mode. In half duplex mode no recording takes place."),
				TEXT("DirectPlay Sample"), MB_OK);
		}
	...
	case WM_COMMAND:
		...
		case IDC_SETUP: {
			...
			DWORD dwResult =
				(DWORD)DialogBox(g_hInst, MAKEINTRESOURCE(IDD_VOICE_SETUP),
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	if (bConnectSuccess) {
		...
		// Ask the user for DirectPlayVoice setup params
		DWORD dwResult =
			(DWORD)DialogBox(hInst, MAKEINTRESOURCE(IDD_VOICE_SETUP), NULL,
				(DLGPROC)VoiceConfigDlgProc);
		if (dwResult != IDCANCEL) {
			g_pNetVoice->ChangeVoiceClientSettings(&g_dvClientConfig);
		}
	}
	...
	if (bConnectSuccess) {
		...
		g_hrDialog = S_OK;
		DialogBox(
			hInst, MAKEINTRESOURCE(IDD_MAIN_GAME), NULL, (DLGPROC)VoiceDlgProc);
