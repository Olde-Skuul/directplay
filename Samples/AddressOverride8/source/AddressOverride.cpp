...
//       simple DirectPlay message to all of the other players. When this
//       message is receieved by the other players, they simply display a dialog
//       box.
//
...
GUID g_guidApp = {0x2ae835d, 0x9179, 0x485f,
	{0x83, 0x43, 0x90, 0x1d, 0x32, 0x7c, 0xe7, 0x94}};
...
HANDLE g_hConnectCompleteEvent =
	NULL;                           // Event signaled when connection complete
...
DPN_APPLICATION_DESC* g_pEnumedSessionAppDesc =
	NULL; // App desc of first session enumed
IDirectPlay8Address* g_pEnumedSessionHostAddr =
	NULL; // Host addr of first session enumed
IDirectPlay8Address* g_pEnumedSessionDeviceAddr =
	NULL; // Address of device to use
	...
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
		...
		if (NULL == g_pEnumedSessionHostAddr) {
			...
			// g_pEnumedSessionHostAddr.
			if (pEnumHostsResponseMsg->pAddressSender) {
				pEnumHostsResponseMsg->pAddressSender->Duplicate(
					&g_pEnumedSessionHostAddr);
			}

			// Duplicate pEnumHostsResponseMsg->pAddressDevice in
			// g_pEnumedSessionDeviceAddr.
			if (pEnumHostsResponseMsg->pAddressDevice) {
				pEnumHostsResponseMsg->pAddressDevice->Duplicate(
					&g_pEnumedSessionDeviceAddr);
			}

			// Copy pEnumHostsResponseMsg->pApplicationDescription to
			// g_pEnumedSessionAppDesc
			g_pEnumedSessionAppDesc = new DPN_APPLICATION_DESC;
			ZeroMemory(g_pEnumedSessionAppDesc, sizeof(DPN_APPLICATION_DESC));
			memcpy(g_pEnumedSessionAppDesc,
				pEnumHostsResponseMsg->pApplicationDescription,
				sizeof(DPN_APPLICATION_DESC));
			if (pEnumHostsResponseMsg->pApplicationDescription
					->pwszSessionName) {
				g_pEnumedSessionAppDesc->pwszSessionName =
					new WCHAR[wcslen(
								  pEnumHostsResponseMsg->pApplicationDescription
									  ->pwszSessionName) +
						1];
	...
	case DPN_MSGID_ASYNC_OP_COMPLETE: {
		...
		if (pAsyncOpCompleteMsg->hAsyncOp == g_hEnumAsyncOp) {
			if (WAIT_TIMEOUT == WaitForSingleObject(g_hEnumHostEvent, 0)) {
				MessageBox(
					g_hDlg, TEXT("No session found."), g_strAppName, MB_OK);
	...
	case DPN_MSGID_RECEIVE: {
		...
		APP_PLAYER_INFO* pPlayerInfo =
			static_cast<APP_PLAYER_INFO*>(pReceiveMsg->pvPlayerContext);
		if (NULL == pPlayerInfo) {
			break;
		}
		...
		if (pMsg->dwType == GAME_MSGID_WAVE) {
			...
			PostMessage(
				g_hDlg, WM_APP_DISPLAY_WAVE, pPlayerInfo->dpnidPlayer, 0);
		}
		break;
	}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT OnInitOverrideDialog(HWND hDlg)
{
	HRESULT hr;
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT LaunchMultiplayerGame(HWND hDlg)
{
	HRESULT hr;
		...
		if (g_bHostPlayer) {
			if (_tcslen(strPort) > 0) {
				...
				if (FAILED(hr = pDeviceAddress->AddComponent(DPNA_KEY_PORT,
							   &dwPort, sizeof(dwPort), DPNA_DATATYPE_DWORD))) {
	...
	if (g_bHostPlayer) {
		...
		hr = g_pDP->Host(&dpnAppDesc, // the application desc
			&pDeviceAddress, // array of addresses of the local devices used to
							 // connect to the host
			1,               // number in array
			NULL, NULL,      // DPN_SECURITY_DESC, DPN_SECURITY_CREDENTIALS
			NULL,            // player context
			static_cast<DWORD>(bOkToQuery)); // flags
		...
		if (FAILED(hr)) {
			...
			if (hr == DPNERR_INVALIDDEVICEADDRESS) { // This will be returned if
		...
		if (FAILED(hr)) {
			...
			if (hr == DPNERR_ADDRESSING) { // This will be returned if the ip
										   // address is invalid
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK OverrideDlgProc(
		...
		case IDOK:
			if (FAILED(g_hrDialog = LaunchMultiplayerGame(hDlg))) {
				if (g_hrDialog == DPNERR_ADDRESSING) {
					...
					MessageBox(hDlg,
						TEXT("User cancelled the DirectPlay dialog."),
						g_strAppName, MB_OK);
				} else {
	...
	case WM_TIMER:
		if (wParam == TIMER_WAIT_HOSTS_RESPONSE) {
			...
			if (dwResult == WAIT_OBJECT_0) {
				// Connect to host that was found. There should only be on
				// device address in the connection settings structure when
				// connecting to a session, so just pass in the first one. The
				// enumeration is automatically cancelled after Connect is
				// called
				DPNHANDLE hAsync;
				hr = g_pDP->Connect(
					g_pEnumedSessionAppDesc,    // the application desc
					g_pEnumedSessionHostAddr,   // address of the host of the
												// session
					g_pEnumedSessionDeviceAddr, // address of the local device
				...
				if (FAILED(g_hrConnectComplete)) {
					DXTRACE_ERR(TEXT("DPN_MSGID_CONNECT_COMPLETE"),
						g_hrConnectComplete);
					MessageBox(hDlg, TEXT("Unable to join game."), g_strAppName,
						MB_OK | MB_ICONERROR);
...
//-----------------------------------------------------------------------------
static HRESULT WaveToAllPlayers()
{
...
// Desc: Handles dialog messages
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK GreetingDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	switch (msg) {
	case WM_INITDIALOG: {
	...
	case WM_APP_DISPLAY_WAVE: {
		...
		AppendTextToEditControl(hDlg, szWaveMessage, IDC_LOG_EDIT, 9);
		break;
	}
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	if (FAILED(g_hrDialog)) {
		...
		DXUtil_WriteStringRegKey(hDPlaySampleRegKey, TEXT("Preferred Provider"),
