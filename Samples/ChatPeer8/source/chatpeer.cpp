...
GUID g_guidApp = {0x876a3036, 0xffd7, 0x46bc,
	{0x92, 0x9, 0xb4, 0x2f, 0x61, 0x7b, 0x9b, 0xe7}};
...
TCHAR g_strAppName[] = TEXT("ChatPeer");
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_RECEIVE: {
		...
		if (pMsg->nType == GAME_MSGID_CHAT) {
			// This message is sent when a player has send a chat message to us,
			// so post a message to the dialog thread to update the UI. This
			// keeps the DirectPlay threads from blocking, and also serializes
			// the recieves since DirectPlayMessageHandler can be called
			// simultaneously from a pool of DirectPlay threads.
			...
			PostMessage(g_hDlg, WM_APP_CHAT, pPlayerInfo->dpnidPlayer,
				(LPARAM)pQueuedData);
	...
	if (hReturn != DPNSUCCESS_PENDING && SUCCEEDED(hReturn) &&
		g_pNetConnectWizard) {
	...
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_APP_CHAT: {
		...
		wsprintf(strChatBuffer, TEXT("<%s> %s"), pPlayerInfo->strPlayerName,
			pQueuedData->pChatMsg->strChatString);
	...
	if (FAILED(hr = InitDirectPlay())) {
		...
		if (g_bWasLobbyLaunched &&
			...
			if (FAILED(hr = g_pNetConnectWizard->ConnectUsingLobbySettings())) {
				...
				_tcscpy(
					g_strLocalPlayerName, g_pNetConnectWizard->GetPlayerName());
			...
			if (FAILED(hr)) {
				...
				_tcscpy(
					g_strLocalPlayerName, g_pNetConnectWizard->GetPlayerName());
				_tcscpy(
					g_strSessionName, g_pNetConnectWizard->GetSessionName());
