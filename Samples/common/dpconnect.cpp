...
CHAR g_strPreferredProvider[MAX_SESSION_NAME];

// Name of the session
CHAR g_strSessionName[MAX_SESSION_NAME];

// Name of the local player
CHAR g_strLocalPlayerName[MAX_PLAYER_NAME];
...
INT_PTR DPConnect_StartDirectPlayConnect(
	HINSTANCE hInst, BOOL bBackTrack = FALSE)
{
	int nStep;
	...
	INT_PTR nExitCode = EXITCODE_ERROR;
	...
	for (;;) {
		...
		case 0:
			...
			nExitCode =
				DialogBoxA(hInst, MAKEINTRESOURCEA(IDD_MUTLIPLAYER_CONNECT),
		...
		case 1:
			...
			nExitCode =
				DialogBoxA(hInst, MAKEINTRESOURCEA(IDD_MUTLIPLAYER_GAMES), NULL,
					DPConnect_SessionsDlgProc);
			break;
		default:
			break;
		}
...
INT_PTR CALLBACK DPConnect_ConnectionsDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	...
	switch (msg) {
	case WM_INITDIALOG: {
		SetDlgItemTextA(hDlg, IDC_PLAYER_NAME_EDIT, g_strLocalPlayerName);

		// Load and set the icon
		HINSTANCE hInst = (HINSTANCE)GetWindowLongPtrA(hDlg, GWLP_HINSTANCE);
		HICON hIcon = LoadIconA(hInst, MAKEINTRESOURCEA(IDI_MAIN));
...
HRESULT DPConnect_ConnectionsDlgFillListBox(HWND hDlg)
{
	...
	if (FAILED(hr = CoCreateInstance(CLSID_DirectPlay, NULL, CLSCTX_ALL,
		...
		if (hr == E_NOINTERFACE) {
			MessageBoxA(NULL,
				"This application requires DirectPlay 6 or later. "
	...
	SendMessageA(
		hWndListBox, LB_ADDSTRING, 0, (LPARAM) "Wait for Lobby Connection");

	SetFocus(hWndListBox);
	LRESULT nIndex = SendMessageA(hWndListBox, LB_FINDSTRINGEXACT, (WPARAM)-1,
		(LPARAM)g_strPreferredProvider);
	if (nIndex != LB_ERR) {
		SendMessageA(hWndListBox, LB_SETCURSEL, (WPARAM)nIndex, 0);
	} else {
		SendMessageA(hWndListBox, LB_SETCURSEL, 0, 0);
	}
	return S_OK;
}
...
BOOL FAR PASCAL DPConnect_EnumConnectionsCallback(LPCGUID /* pguidSP */,
	VOID* pConnection, DWORD dwConnectionSize, LPCDPNAME pName,
	DWORD /* dwFlags */, VOID* pvContext)
{
	HRESULT hr;
	LPDIRECTPLAY4 pDP = NULL;
	BYTE* pConnectionBuffer = NULL;
	...
	if (FAILED(hr = pDP->InitializeConnection(pConnection, 0))) {
		SAFE_RELEASE(pDP);
		// Unavailable connection, keep enumerating
		return TRUE;
	}
	...
	iIndex = SendMessageA(
		hWndListBox, LB_ADDSTRING, 0, (LPARAM)pName->lpszShortNameA);
	if (iIndex == CB_ERR) {
		// Error, stop enumerating
		return FALSE;
	}
	...
	SendMessageA(
		hWndListBox, LB_SETITEMDATA, (WPARAM)iIndex, (LPARAM)pConnectionBuffer);
...
HRESULT DPConnect_ConnectionsDlgOnOK(HWND hDlg)
{
	...
	GetDlgItemTextA(
		hDlg, IDC_PLAYER_NAME_EDIT, g_strLocalPlayerName, MAX_PLAYER_NAME);

	if (strlen(g_strLocalPlayerName) == 0) {
		MessageBoxA(hDlg, "You must enter a valid player name.",
			"DirectPlay Sample", MB_OK);
		return S_OK;
	}

	HWND hWndListBox = GetDlgItem(hDlg, IDC_CONNECTION_LIST);

	iIndex = SendMessageA(hWndListBox, LB_GETCURSEL, 0, 0);
	SendMessageA(hWndListBox, LB_GETTEXT, (WPARAM)iIndex,
		(LPARAM)g_strPreferredProvider);

	pConnection =
		(VOID*)SendMessageA(hWndListBox, LB_GETITEMDATA, (WPARAM)iIndex, 0);
	if (NULL == pConnection) {
...
VOID DPConnect_ConnectionsDlgCleanup(HWND hDlg)
{
	...
	LRESULT dwCount = SendMessageA(hWndListBox, LB_GETCOUNT, 0, 0);
	for (iIndex = 0; iIndex < dwCount; iIndex++) {
		BYTE* pConnectionBuffer =
			(BYTE*)SendMessageA(hWndListBox, LB_GETITEMDATA, (WPARAM)iIndex, 0);
...
INT_PTR CALLBACK DPConnect_SessionsDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	HRESULT hr;
	...
	case WM_INITDIALOG: {
		...
		SetDlgItemTextA(hDlg, IDC_SEARCH_CHECK, "Start Search");
	...
	case WM_TIMER:
		if (FAILED(hr = DPConnect_SessionsDlgShowGames(hDlg))) {
			KillTimer(hDlg, 1);
			MessageBoxA(hDlg, "Error enumerating DirectPlay games.",
				"DirectPlay Sample", MB_OK | MB_ICONERROR);
		}
		break;
	...
	case WM_COMMAND:
		...
		case IDC_SEARCH_CHECK:
			...
			if (g_bSearchingForSessions) {
				...
				SetDlgItemTextA(hDlg, IDC_SEARCH_CHECK, "Searching...");
		...
		case IDC_JOIN:
			if (FAILED(hr = DPConnect_SessionsDlgJoinGame(hDlg))) {
				MessageBoxA(hDlg, "Unable to join game.", "DirectPlay Sample",
					MB_OK | MB_ICONERROR);
			}
			break;
		...
		case IDC_CREATE:
			if (FAILED(hr = DPConnect_SessionsDlgCreateGame(hDlg))) {
				MessageBoxA(hDlg, "Unable to create game.", "DirectPlay Sample",
					MB_OK | MB_ICONERROR);
			}
			break;
...
VOID DPConnect_SessionsDlgInitListbox(HWND hDlg)
{
	...
	if (g_bSearchingForSessions) {
		SendMessageA(
			hWndListBox, LB_ADDSTRING, 0, (LPARAM) "Looking for games...");
	} else {
		SendMessageA( hWndListBox, LB_ADDSTRING, 0, 
                     (LPARAM)"Click Start Search to see a list of games.  "
                             "Click Create to start a new game." );
	}

	SendMessageA(hWndListBox, LB_SETITEMDATA, 0, NULL);
	SendMessageA(hWndListBox, LB_SETCURSEL, 0, 0);
...
HRESULT DPConnect_SessionsDlgShowGames(HWND hDlg)
{
	...
	LRESULT nItemSelected = SendMessageA(hWndListBox, LB_GETCURSEL, 0, 0);
	if (nItemSelected != LB_ERR) {
		pDPSISelected = (DPSessionInfo*)SendMessageA(
			hWndListBox, LB_GETITEMDATA, (WPARAM)nItemSelected, 0);
	...
	if (FAILED(hr)) {
		if (hr == DPERR_USERCANCEL) {
			...
			if (g_bSearchingForSessions) {
				...
				SendMessageA(hDlg, WM_COMMAND, IDC_SEARCH_CHECK, 0);
			}
	...
	SendMessageA(hWndListBox, WM_SETREDRAW, FALSE, 0);
	...
	if (g_DPSIHead.pDPSINext != &g_DPSIHead) {
		...
		while (pDPSI != &g_DPSIHead) {
			...
			LRESULT nIndex = SendMessageA(
				hWndListBox, LB_ADDSTRING, 0, (LPARAM)pDPSI->szSession);
			SendMessageA(
				hWndListBox, LB_SETITEMDATA, (WPARAM)nIndex, (LPARAM)pDPSI);
	...
	SendMessageA(hWndListBox, WM_SETREDRAW, TRUE, 0);
	...
	DWORD* /* pdwTimeout */, DWORD dwFlags, VOID* /* pvContext */)
{
	...
	if (dwFlags & DPESC_TIMEDOUT) {
		// The enumeration has timed out, so stop the enumeration.
		return FALSE;
	}
...
HRESULT DPConnect_SessionsDlgJoinGame(HWND hDlg)
{
	...
	if (NULL == pDPSISelected) {
		MessageBoxA(
			hDlg, "There are no games to join.", "DirectPlay Sample", MB_OK);
		return S_OK;
	}
	...
	// Also all DirectPlay messages for this player will signal
	// g_hDPMessageEvent
...
HRESULT DPConnect_SessionsDlgCreateGame(HWND hDlg)
{
	...
	hInst = (HINSTANCE)GetWindowLongPtrA(hDlg, GWLP_HINSTANCE);

	// Display a modal multiplayer connect dialog box.
	EnableWindow(hDlg, FALSE);
	LRESULT nResult =
		DialogBoxA(hInst, MAKEINTRESOURCEA(IDD_MULTIPLAYER_CREATE), hDlg,
			DPConnect_CreateSessionDlgProc);
	...
	// The DPSESSION_KEEPALIVE flag keeps the session alive if players
	// abnormally exit
	ZeroMemory(&dpsd, sizeof(dpsd));
	dpsd.dwSize = sizeof(dpsd);
	dpsd.guidApplication = g_AppGUID;
	...
	if (FAILED(hr = g_pDP->CreatePlayer(&g_LocalPlayerDPID, &dpname,
				   g_hDPMessageEvent, NULL, 0, DPPLAYER_SERVERPLAYER))) {
		return hr;
	}
...
INT_PTR CALLBACK DPConnect_CreateSessionDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	DWORD dwNameLength;
	...
	switch (msg) {
	case WM_INITDIALOG:
		SetDlgItemTextA(hDlg, IDC_EDIT_SESSION_NAME, g_strSessionName);
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case IDOK:
			dwNameLength = GetDlgItemTextA(hDlg, IDC_EDIT_SESSION_NAME,
				g_strSessionName, MAX_SESSION_NAME);
			if (dwNameLength == 0) {
				// Don't accept blank session names
				return TRUE;
			}
...
HRESULT DPConnect_WaitForLobbyLaunch(HWND hParentDlg)
{
	...
	DialogBoxParamA(NULL, MAKEINTRESOURCEA(IDD_LOBBY_WAIT_STATUS), hParentDlg,
	...
	if (FAILED(hr = DPConnect_CheckForLobbyLaunch(&bLaunchedByLobby))) {
		// This may be DPERR_USERCANCEL if the user canceled the
		// dialog
		return hr;
	}
...
HRESULT DPConnect_DoLobbyLaunch()
{
	...
	g_bHostPlayer =
		((g_pDPLConnection->dwFlags & DPLCONNECTION_CREATESESSION) != 0);
	...
	if (FAILED(
			hr = g_pDPLobby->SetConnectionSettings(0, 0, g_pDPLConnection))) {
		return hr;
	}
	...
	if (g_bHostPlayer) {
		...
		if (FAILED(hr = g_pDPLobby->ConnectEx(
					   0, IID_IDirectPlay4A, (VOID**)&g_pDP, NULL))) {
			return hr;
		}
	} else {
		...
		DialogBoxParamA(NULL, MAKEINTRESOURCEA(IDD_LOBBY_WAIT_STATUS), NULL,
			DPConnect_LobbyWaitDlgProc, (LPARAM)&hr);
		if (FAILED(hr)) {
			// This may be DPERR_USERCANCEL if the user canceled the
			// dialog
	...
	strcpy(
		g_strLocalPlayerName, g_pDPLConnection->lpPlayerName->lpszShortNameA);
...
INT_PTR CALLBACK DPConnect_LobbyWaitDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		if (g_bWaitForConnectionSettings) {
			SetDlgItemTextA(hDlg, IDC_WAIT_TEXT, "Waiting for connection...");
		} else {
			SetDlgItemTextA(hDlg, IDC_WAIT_TEXT, "Finding Game...");
		}
	...
	case WM_TIMER:
		...
		if (g_bWaitForConnectionSettings) {
			...
			*phr = g_pDPLobby->ConnectEx(DPCONNECT_RETURNSTATUS,
				IID_IDirectPlay4A, (VOID**)&g_pDP, NULL);
			if (*phr == DPERR_CONNECTING) {
				// Try again on the next WM_TIMER message
				break;
			}
			EndDialog(hDlg, 0);
		}
...
{
	HRESULT hr = NOERROR;
	BYTE* pvMsgBuffer = NULL;
	DWORD dwMsgBufferSize = 0;
	DWORD dwMsgFlags;

	for (;;) {
		hr = g_pDPLobby->ReceiveLobbyMessage(
			0, 0, &dwMsgFlags, pvMsgBuffer, &dwMsgBufferSize);
		...
		if (dwMsgFlags & DPLMSG_SYSTEM) {
			...
			if (((DPLMSG_GENERIC*)pvMsgBuffer)->dwType ==
