...
CNetConnectWizard::CNetConnectWizard(
	HINSTANCE hInst, HWND hWndParent, const TCHAR* strAppName, GUID* pGuidApp)
{
	g_pNCW = this;
	m_hInst = hInst;
...
// Desc:
//-----------------------------------------------------------------------------
VOID CNetConnectWizard::Init(
	IDirectPlay8Peer* pDP, IDirectPlay8LobbiedApplication* pLobbiedApp)
{
	m_pDP = pDP;
...
HRESULT CNetConnectWizard::DoConnectWizard(BOOL bBackTrack)
{
	...
	for (;;) {
		...
		if (FAILED(m_hrDialog) || m_hrDialog == NCW_S_QUIT ||
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::StaticConnectionsDlgProc(
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::ConnectionsDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	...
	case WM_INITDIALOG: {
		...
		SendMessage(
			hDlg, WM_SETICON, ICON_SMALL, (LPARAM)hIcon); // Set small icon

		// Set the window title
		TCHAR strWindowTitle[256];
		wsprintf(
			strWindowTitle, TEXT("%s - Multiplayer Connect"), m_strAppName);
...
HRESULT CNetConnectWizard::ConnectionsDlgFillListBox(HWND hDlg)
{
	...
	HRESULT hr = m_pDP->EnumServiceProviders(
	...
	if (hr == DPNERR_BUFFERTOOSMALL) {
		...
		if (FAILED(hr = m_pDP->EnumServiceProviders(
					   NULL, NULL, pdnSPInfo, &dwSize, &dwItems, 0))) {
			return DXTRACE_ERR(TEXT("EnumServiceProviders"), hr);
		}
	}
	...
	for (DWORD i = 0; i < dwItems; i++) {
		...
		iLBIndex =
			(int)SendMessage(hWndListBox, LB_ADDSTRING, 0, (LPARAM)strName);
		if (iLBIndex == CB_ERR) {
		...
		SendMessage(hWndListBox, LB_SETITEMDATA, static_cast<WPARAM>(iLBIndex),
			(LPARAM)pGuid);
	...
	if (iLBIndex != LB_ERR) {
		SendMessage(
			hWndListBox, LB_SETCURSEL, static_cast<WPARAM>(iLBIndex), 0);
	} else {
...
HRESULT CNetConnectWizard::ConnectionsDlgOnOK(HWND hDlg)
{
	...
	SendMessage(hWndListBox, LB_GETTEXT, static_cast<WPARAM>(iIndex),
		(LPARAM)m_strPreferredProvider);

	GUID* pGuid = (GUID*)SendMessage(
		hWndListBox, LB_GETITEMDATA, static_cast<WPARAM>(iIndex), 0);
	if (NULL == pGuid) {
		// 'Wait for lobby launch' SP has been selected, so wait for a
		// connection
		...
		// Display the multiplayer connect dialog box.
		DialogBox(m_hInst, MAKEINTRESOURCE(IDD_LOBBY_WAIT_STATUS), hDlg,
			(DLGPROC)StaticLobbyWaitDlgProc);
		...
		// 'Wait for lobby launch' was canceled, so don't wait for a connection
		// anymore
...
VOID CNetConnectWizard::ConnectionsDlgCleanup(HWND hDlg)
{
	...
	DWORD dwCount = (DWORD)SendMessage(hWndListBox, LB_GETCOUNT, 0, 0);
	for (iIndex = 0; iIndex < dwCount; iIndex++) {
		GUID* pGuid =
			(GUID*)SendMessage(hWndListBox, LB_GETITEMDATA, iIndex, 0);
		SAFE_DELETE(pGuid);
	}
}
...
// Desc: Static msg handler which passes messages
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::StaticSessionsDlgProc(
	HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (g_pNCW) {
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::SessionsDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	HRESULT hr;
	...
	case WM_TIMER:
		...
		if (wParam == TIMERID_DISPLAY_HOSTS) {
			...
			// Check to see if the MessageHandler has set an event to tell us
			// the DPN_MSGID_CONNECT_COMPLETE has been processed.  Now
			// m_hrConnectComplete is valid.
			if (WAIT_OBJECT_0 ==
				WaitForSingleObject(m_hConnectCompleteEvent, 0)) {
				m_bConnecting = FALSE;
				...
				if (FAILED(m_hrConnectComplete)) {
					DXTRACE_ERR(TEXT("DPN_MSGID_CONNECT_COMPLETE"),
						m_hrConnectComplete);
	...
	case WM_COMMAND:
		...
		case IDC_SEARCH_CHECK:
			...
			if (m_bSearchingForSessions) {
				...
				SetTimer(
					hDlg, TIMERID_DISPLAY_HOSTS, DISPLAY_REFRESH_RATE, NULL);
				...
				if (FAILED(hr = SessionsDlgEnumHosts(hDlg))) {
					...
					KillTimer(hDlg, TIMERID_DISPLAY_HOSTS);
					MessageBox(hDlg,
						TEXT("Error enumerating DirectPlay games."),
						TEXT("DirectPlay Sample"), MB_OK | MB_ICONERROR);
				}
			} else {
				SetDlgItemText(hDlg, IDC_SEARCH_CHECK, TEXT("Start Search"));
...
VOID CNetConnectWizard::SessionsDlgInitListbox(HWND hDlg)
{
	...
	if (m_bSearchingForSessions) {
		SendMessage(
			hWndListBox, LB_ADDSTRING, 0, (LPARAM)TEXT("Looking for games..."));
	} else {
		SendMessage(hWndListBox, LB_ADDSTRING, 0,
...
HRESULT CNetConnectWizard::SessionsDlgEnumHosts(HWND /* hDlg */)
{
	...
	HRESULT hr = m_pDP->EnumHosts(&dnAppDesc, // application description
...
//-----------------------------------------------------------------------------
HRESULT CNetConnectWizard::SessionsDlgNoteEnumResponse(
	PDPNMSG_ENUM_HOSTS_RESPONSE pEnumHostsResponseMsg)
{
	HRESULT hr = S_OK;

	// This function is called from the DirectPlay message handler so it could
	// be called simultaneously from multiple threads, so enter a critical
	// section to assure that it we don't get race conditions.  Locking the
	// entire function is crude, and could be more optimal but is effective for
	// this simple sample
	...
	while (pDPHostEnum != &m_DPHostEnumHead) {
		if (pResponseMsgAppDesc->guidInstance ==
			pDPHostEnum->pAppDesc->guidInstance) {
			bFound = TRUE;
			break;
		}
	...
	if (pResponseMsgAppDesc->pwszSessionName) {
		DXUtil_ConvertWideStringToGeneric(
			strName, pResponseMsgAppDesc->pwszSessionName);
	}
	...
	//
	// Duplicate pEnumHostsResponseMsg->pAddressSender in
	// pDPHostEnum->pHostAddr. Duplicate pEnumHostsResponseMsg->pAddressDevice
	// in pDPHostEnum->pDeviceAddr.
	//
	if (FAILED(hr = pEnumHostsResponseMsg->pAddressSender->Duplicate(
				   &pDPHostEnum->pHostAddr))) {
		DXTRACE_ERR(TEXT("Duplicate"), hr);
	} else {

		if (FAILED(hr = pEnumHostsResponseMsg->pAddressDevice->Duplicate(
					   &pDPHostEnum->pDeviceAddr))) {
			DXTRACE_ERR(TEXT("Duplicate"), hr);
		} else {
			...
			memcpy(pDPHostEnum->pAppDesc, pResponseMsgAppDesc,
				sizeof(DPN_APPLICATION_DESC));
			if (pResponseMsgAppDesc->pwszSessionName) {
				pDPHostEnum->pAppDesc->pwszSessionName =
					new WCHAR[wcslen(pResponseMsgAppDesc->pwszSessionName) + 1];
			...
			// if this node was previously invalidated, or the session name is
			// now different the session list in the dialog needs to be updated
...
VOID CNetConnectWizard::SessionsDlgExpireOldHostEnums()
{
	...
	while (pDPHostEnum != &m_DPHostEnumHead) {
		...
		// changed because that causes the list in the dialog to constantly
		// redraw.
		if ((pDPHostEnum->bValid != FALSE) &&
			(pDPHostEnum->dwLastPollTime <
				dwCurrentTime - m_dwEnumHostExpireInterval)) {
...
HRESULT CNetConnectWizard::SessionsDlgDisplayEnumList(HWND hDlg)
{
	...
	BOOL bFoundSelectedGUID = FALSE;

	// Try to keep the same session selected unless it goes away or
	// there is no real session currently selected
	int nItemSelected = (int)SendMessage(hWndListBox, LB_GETCURSEL, 0, 0);
	if (nItemSelected != LB_ERR) {
		DPHostEnumInfo* pDPHostEnumSelected = (DPHostEnumInfo*)SendMessage(
			hWndListBox, LB_GETITEMDATA, static_cast<WPARAM>(nItemSelected), 0);
		...
		while (pDPHostEnum != &m_DPHostEnumHead) {
			...
			if (pDPHostEnum->bValid) {
				...
				SendMessage(hWndListBox, LB_SETITEMDATA,
					static_cast<WPARAM>(nIndex), (LPARAM)pDPHostEnum);

				if (bFindSelectedGUID) {
					// Look for the session the was selected before
					if (pDPHostEnum->pAppDesc->guidInstance ==
						guidSelectedInstance) {
						SendMessage(hWndListBox, LB_SETCURSEL,
							static_cast<WPARAM>(nIndex), 0);
...
HRESULT CNetConnectWizard::SessionsDlgJoinGame(HWND hDlg)
{
	...
	if (FAILED(
			hr = m_pDP->SetPeerInfo(&dpPlayerInfo, NULL, NULL, DPNOP_SYNC))) {
	...
	// We also pass in copies of the app desc and host addr, since
	// pDPHostEnumSelected might be deleted from another thread that calls
	// SessionsDlgExpireOldHostEnums(). This process could also be done using
	...
	hr = m_pDP->Connect(pDPHostEnumSelected->pAppDesc, // the application desc
		...
		pDPHostEnumSelected->pDeviceAddr, // address of the local device the
										  // enum responses were received on
...
HRESULT CNetConnectWizard::SessionsDlgCreateGame(HWND hDlg)
{
	...
	int nResult =
		(int)DialogBox(m_hInst, MAKEINTRESOURCE(IDD_MULTIPLAYER_CREATE), hDlg,
			(DLGPROC)StaticCreateSessionDlgProc);
	...
	if (FAILED(hr = m_pDP->Host(&dnAppDesc, // the application desc
				   &m_pDeviceAddress, // array of addresses of the local devices
...
// Desc: Static msg handler which passes messages
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::StaticCreateSessionDlgProc(
	HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (g_pNCW) {
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::CreateSessionDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	switch (msg) {
	case WM_INITDIALOG:
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
		case IDOK: {
			DWORD dwNameLength = GetDlgItemText(
				hDlg, IDC_EDIT_SESSION_NAME, m_strSessionName, MAX_PATH);
			if (dwNameLength == 0) {
				return TRUE; // Don't accept blank session names
			}
			m_bMigrateHost =
				(IsDlgButtonChecked(hDlg, IDC_MIGRATE_HOST) == BST_CHECKED);
...
VOID CNetConnectWizard::SessionsDlgEnumListCleanup()
{
	...
	while (pDPHostEnum != &m_DPHostEnumHead) {
		DPHostEnumInfo* pDPHostEnumDelete = pDPHostEnum;
		pDPHostEnum = pDPHostEnum->pNext;
...
//-----------------------------------------------------------------------------
HRESULT WINAPI CNetConnectWizard::MessageHandler(
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_ASYNC_OP_COMPLETE: {
		PDPNMSG_ASYNC_OP_COMPLETE pAsyncOpCompleteMsg =
			(PDPNMSG_ASYNC_OP_COMPLETE)pMsgBuffer;
...
HRESULT CNetConnectWizard::ConnectUsingLobbySettings()
{
	...
	HRESULT hr = m_pLobbiedApp->GetConnectionSettings(
		m_hLobbyClient, NULL, &dwSettingsSize, 0);
	if (hr != DPNERR_BUFFERTOOSMALL) {
	...
	if (FAILED(hr = m_pLobbiedApp->GetConnectionSettings(
				   m_hLobbyClient, pSettings, &dwSettingsSize, 0)))
		return DXTRACE_ERR(TEXT("GetConnectionSettings"), hr);

	// Check if the lobby told us to host the game
	m_bHostPlayer =
		static_cast<BOOL>(pSettings->dwFlags & DPLCONNECTSETTINGS_HOST);
	...
	if (m_bHostPlayer) {
		...
		if (FAILED(
				hr = m_pDP->Host(&pSettings->dpnAppDesc, // the application desc
					pSettings->ppdp8DeviceAddresses, // array of addresses of
													 // the local devices used
		...
		// Connect to an existing session. There should only be on device
		// address in the connection settings structure when connecting to a
		// session, so just pass in the first one. The enumeration is
		// automatically cancelled after Connect is called
		hr = m_pDP->Connect(&pSettings->dpnAppDesc, // the application desc
			pSettings->pdp8HostAddress, // address of the host of the session
			pSettings->ppdp8DeviceAddresses[0], // address of the local device
												// used to connect to the host
			NULL, NULL,    // DPN_SECURITY_DESC, DPN_SECURITY_CREDENTIALS
			NULL, 0,       // user data, user data size
			NULL,          // player context,
		...
		if (FAILED(m_hrConnectComplete)) {
			DXTRACE_ERR(
				TEXT("DPN_MSGID_CONNECT_COMPLETE"), m_hrConnectComplete);
			MessageBox(m_hDlg, TEXT("Unable to join game."),
				TEXT("DirectPlay Sample"), MB_OK | MB_ICONERROR);
	...
	for (DWORD dwIndex = 0; dwIndex < pSettings->cNumDeviceAddresses;
		 dwIndex++) {
...
//-----------------------------------------------------------------------------
HRESULT WINAPI CNetConnectWizard::LobbyMessageHandler(
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	HRESULT hr = S_OK;
	...
	case DPL_MSGID_CONNECT: {
		...
		// the thread who called Initialize().  The same is true for
		// WaitForConnection()
		...
		if (FAILED(hr = m_pDP->RegisterLobby(
					   m_hLobbyClient, m_pLobbiedApp, DPNLOBBY_REGISTER))) {
		...
		if (pSettings == NULL) {
			...
			if (pSettings->pwszPlayerName != NULL) {
				...
				DXUtil_ConvertWideStringToGeneric(
					strPlayerName, pSettings->pwszPlayerName);
...
// Desc: Static msg handler which passes messages
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::StaticLobbyWaitDlgProc(
	HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (g_pNCW) {
...
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CNetConnectWizard::LobbyWaitDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	switch (msg) {
	case WM_INITDIALOG:
		...
		SetDlgItemText(
			hDlg, IDC_WAIT_TEXT, TEXT("Waiting for lobby connection..."));
		return TRUE;
