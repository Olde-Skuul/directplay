...
public:
	...
	HRESULT WINAPI LobbyMessageHandler(
		PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer);
	...
	{
		_tcscpy(m_strSessionName, strSessionName);
	}
	void SetPreferredProvider(TCHAR* strPreferredProvider)
	{
		_tcscpy(m_strPreferredProvider, strPreferredProvider);
	}
...
protected:
	...
	static INT_PTR CALLBACK StaticConnectionsDlgProc(
		HWND, UINT, WPARAM, LPARAM);
	static INT_PTR CALLBACK StaticSessionsDlgProc(HWND, UINT, WPARAM, LPARAM);
	static INT_PTR CALLBACK StaticCreateSessionDlgProc(
		HWND, UINT, WPARAM, LPARAM);
	...
	HRESULT SessionsDlgNoteEnumResponse(
		PDPNMSG_ENUM_HOSTS_RESPONSE pEnumHostsResponse);
