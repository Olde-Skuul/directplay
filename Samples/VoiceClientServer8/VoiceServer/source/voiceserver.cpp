...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT SendDestroyPlayerMsgToAll(APP_PLAYER_INFO* pPlayerInfo)
{
	...
	g_pDPServer->SendTo(DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1, 0, NULL,
		&hAsync, DPNSEND_NOLOOPBACK | DPNSEND_GUARANTEED);
...
// Desc:
//-----------------------------------------------------------------------------
static VOID DisplayPlayers(HWND hDlg)
{
	HRESULT hr;
	...
	for (;;) {
		hr = g_pDPServer->EnumPlayersAndGroups(
			aPlayers, &dwNumPlayers, DPNENUM_PLAYERS);
		if (SUCCEEDED(hr)) {
			break;
		}
	...
	for (DWORD i = 0; i < dwNumPlayers; i++) {
		...
		do {
			...
			hr = g_pDPServer->GetPlayerContext(
				aPlayers[i], (LPVOID*)&pPlayerInfo, 0);
		...
		wsprintf(strTemp, TEXT("DPNID: 0x%0.8x (%s)"), pPlayerInfo->dpnidPlayer,
			pPlayerInfo->strPlayerName);
		SendMessage(GetDlgItem(hDlg, IDC_PLAYER_LIST), LB_ADDSTRING, 0,
			(LPARAM)strTemp);
	}
...
//-----------------------------------------------------------------------------
static HRESULT EnumVoiceCompressionCodecs(HWND hDlg)
{
...
//-----------------------------------------------------------------------------
static HRESULT SendCreatePlayerMsg(
	APP_PLAYER_INFO* pPlayerAbout, DPNID dpnidTarget)
{
	...
	// if there are any severe errors, so ignore any errors
	DPNHANDLE hAsync;
	g_pDPServer->SendTo(dpnidTarget, &bufferDesc, 1, 0, NULL, &hAsync,
...
//-----------------------------------------------------------------------------
static HRESULT SendWorldStateToNewPlayer(DPNID dpnidNewPlayer)
{
	HRESULT hr;
	DWORD dwNumPlayers = 0;
	DPNID* aPlayers = NULL;
	...
	// if there are any severe errors, so ignore any errors
	DPNHANDLE hAsync;
	g_pDPServer->SendTo(dpnidNewPlayer, &bufferDesc, 1, 0, NULL, &hAsync,
		DPNSEND_NOLOOPBACK | DPNSEND_GUARANTEED);
	...
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_CREATE_PLAYER: {
		...
		DWORD dwSize = 0;
		HRESULT hr = g_pDPServer->GetClientInfo(
	...
	case DPN_MSGID_DESTROY_PLAYER: {
		...
		// Send all connected players a message telling about this destroyed
		// player
	...
	case DPN_MSGID_RECEIVE: {
		...
		// APP_PLAYER_INFO* pPlayerInfo = (APP_PLAYER_INFO*)
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT StartServer(HWND hDlg)
{
	DWORD dwSessionType;
	HRESULT hr;
	...
	if (FAILED(
			hr = g_pDPServer->Initialize(NULL, DirectPlayMessageHandler, 0))) {
	...
	if (FAILED(hr)) {
		...
		if (FAILED(hr)) {
			...
			hr = g_pDPServer->Host(
				&dpnAppDesc, &pDP8AddrLocal, 1, NULL, NULL, NULL, 0);
			if (FAILED(hr)) {
				...
				LRESULT lCurSelection = SendDlgItemMessage(
					hDlg, IDC_COMPRESSION_COMBO, CB_GETCURSEL, 0, 0);
				if (lCurSelection != CB_ERR) {
					GUID* pGuidCT = (LPGUID)SendDlgItemMessage(hDlg,
						IDC_COMPRESSION_COMBO, CB_GETITEMDATA,
						static_cast<WPARAM>(lCurSelection), 0);
					if (pGuidCT != NULL) {
				...
				g_pNetVoice =
					new CNetVoice(NULL, DirectPlayVoiceServerMessageHandler);
				...
				if (FAILED(g_hrDialog =
							   g_pNetVoice->Init(hDlg, TRUE, FALSE, g_pDPServer,
								   dwSessionType, &g_guidDVSessionCT, NULL))) {
					if (FAILED(g_hrDialog)) {
...
// Desc:
//-----------------------------------------------------------------------------
static VOID StopServer(HWND hDlg)
{
	if (hDlg) {
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK ServerDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM /* lParam */)
{
	...
	case WM_COMMAND: {
		...
		case IDC_START:
			...
			EnableWindow(
				GetDlgItem(hDlg, IDC_COMPRESSION_COMBO), !g_bServerStarted);
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	if (FAILED(g_hrDialog)) {
		if (g_hrDialog == DPNERR_CONNECTIONLOST) {
			...
			MessageBox(NULL,
				TEXT("An error occured. ") TEXT("The server will now quit."),
				g_strAppName, MB_OK | MB_ICONERROR);
		}
	}
	...
	DXUtil_WriteIntRegKey(
		hDPlaySampleRegKey, TEXT("VoiceServer Port"), g_dwPort);
