...
#define PLAYER_RELEASE(pPlayerInfo) \
	if (pPlayerInfo) { \
		...
		if (pPlayerInfo->lRefCount <= 0) { \
			DestoryPlayerStruct(pPlayerInfo); \
		} \
	} \
	pPlayerInfo = NULL;
...
BOOL g_bMixingSessionType =
	FALSE; // TRUE if the server is mixing, otherwise its forwarding.

//-----------------------------------------------------------------------------
// Name: AddPlayerStruct()
// Desc: Add pPlayerInfo to the circular linked list, g_playerHead
//-----------------------------------------------------------------------------
static void AddPlayerStruct(APP_PLAYER_INFO* pPlayerInfo)
{
...
//-----------------------------------------------------------------------------
static void DestoryPlayerStruct(APP_PLAYER_INFO* pPlayerInfo)
{
...
static void OnInitDialog(HWND hDlg)
{
	...
	if (FAILED(g_hrDialog = g_pNetVoice->Init(hDlg, FALSE, TRUE, g_pDPClient,
				   DVSESSIONTYPE_FORWARDING, NULL, &g_dvClientConfig))) {
		if (g_hrDialog == DVERR_USERBACK) {
			MessageBox(hDlg,
				TEXT("The user backed out of the wizard.  ")
		...
		if (g_hrDialog == DVERR_USERCANCEL) {
			MessageBox(hDlg,
				TEXT("The user canceled the wizard. ")
					TEXT("This simple sample does not handle this case, so ")
						TEXT("the sample will quit."),
				TEXT("DirectPlay Sample"), MB_OK);
			g_hrDialog = S_OK;
	...
	// Get the session description to figure out if we are connected to a mixing
	// server
...
static HRESULT DisplayPlayersInChat(HWND hDlg)
{
	...
	while (pCurPlayer != &g_playerHead) {
		...
		lvItem.cchTextMax =
			static_cast<int>(_tcslen(pCurPlayer->strPlayerName));
...
//-----------------------------------------------------------------------------
static HRESULT GetPlayerStruct(
	DPNID dpnidPlayer, APP_PLAYER_INFO** ppPlayerInfo)
{
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	// threads, so be careful of thread synchronization problems with shared
	// memory
	...
	case DPN_MSGID_RECEIVE: {
		...
		case GAME_MSGID_SET_ID: {
			...
			GAMEMSG_SET_ID* pSetIDMsg =
				(GAMEMSG_SET_ID*)pReceiveMsg->pReceiveData;
		...
		case GAME_MSGID_CREATE_PLAYER: {
			...
			GAMEMSG_CREATE_PLAYER* pCreatePlayerMsg =
				(GAMEMSG_CREATE_PLAYER*)pReceiveMsg->pReceiveData;
			...
			_tcscpy(
				pPlayerInfo->strPlayerName, pCreatePlayerMsg->strPlayerName);
		...
		case GAME_MSGID_DESTROY_PLAYER: {
			...
			GAMEMSG_DESTROY_PLAYER* pDestroyPlayerMsg =
				(GAMEMSG_DESTROY_PLAYER*)pReceiveMsg->pReceiveData;
	...
	if (g_pNetClientWizard) {
		g_pNetClientWizard->MessageHandler(
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	// DPL_MSGID_CONNECT
	if (g_pNetClientWizard) {
		return g_pNetClientWizard->LobbyMessageHandler(
			pvUserContext, dwMessageId, pMsgBuffer);
	}
	return S_OK;
}
	...
	LPVOID lpvUserContext, DWORD dwMessageType, LPVOID lpMessage)
{
	...
	// With a mixing sever, the client won't recieve DVMSGID_RECORDSTART/STOP
	// msgs. It will still receieve DVMSGID_PLAYERVOICESTART/STOP messages
	// whenever audio occurs, however the messages do not identify the source.
	// So in this sample, if the server is in mixing mode then it just reports
	// 'n/a' for the players.  This could be done differently in more complex
	// apps.
	if (g_bMixingSessionType) {
		return S_OK;
	}
...
static HRESULT InitDirectPlay()
{
	...
	if (FAILED(
			hr = g_pDPClient->Initialize(NULL, DirectPlayMessageHandler, 0))) {
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	// need to prompt the user to detrimine how to connect.
	if (g_bWasLobbyLaunched &&
		g_pNetClientWizard->HaveConnectionSettingsFromLobby()) {
