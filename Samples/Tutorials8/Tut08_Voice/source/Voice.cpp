...
// File: Voice.cpp
//
// Desc: This simple program builds upon the last tutorial and adds voice
// support
//
...
// Watcom needs the DirectSound GUIDs
...
GUID g_guidApp = {0x91f570f2, 0xce88, 0x428a,
	{0x80, 0x25, 0x25, 0xa1, 0xe4, 0x91, 0x8b, 0x44}};
	...
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
		...
		if (FAILED(pEnumHostsResponseMsg->pAddressSender->Duplicate(
...
//-----------------------------------------------------------------------------
static HRESULT WINAPI DirectVoiceServerMessageHandler(
	PVOID /* pvUserContext */, DWORD /* dwMessageId */, PVOID /* pMsgBuffer */)
{
...
//-----------------------------------------------------------------------------
static HRESULT WINAPI DirectVoiceClientMessageHandler(
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID /* pMsgBuffer */)
{
	HRESULT hr = S_OK;
	...
	if (hr != DPNERR_BUFFERTOOSMALL) {
		...
		if (pSettings == NULL) {
			...
			if (FAILED(hr = g_pLobbyApp->GetConnectionSettings(
				...
				if ((g_bHost = static_cast<BOOL>(
						 pSettings->dwFlags & DPLCONNECTSETTINGS_HOST)) != 0) {
...
//-----------------------------------------------------------------------------
static HRESULT TestDirectVoice()
{
	HRESULT hr = S_OK;
	...
	if (FAILED(hr = CoCreateInstance(CLSID_DirectPlayVoiceTest, NULL,
				   CLSCTX_INPROC_SERVER, IID_IDirectPlayVoiceTest,
				   (LPVOID*)&pVoiceTest))) {
		...
		hr = pVoiceTest->CheckAudioSetup(
			&guidPlayback, &guidCapture, NULL, DVFLAGS_QUERYONLY);

		if (hr == DVERR_RUNSETUP) {
			hwnd = GetConsoleHwnd();

			if (FAILED(hr = pVoiceTest->CheckAudioSetup(&guidPlayback,
						   &guidCapture, hwnd, DVFLAGS_ALLOWBACK))) {
...
//-----------------------------------------------------------------------------
static HRESULT InitDirectPlayVoice()
{
	HRESULT hr = S_OK;
	...
	if (g_bHost) {
		...
		if (FAILED(hr = CoCreateInstance(CLSID_DirectPlayVoiceServer, NULL,
			...
			if (FAILED(hr = g_pVoiceServer->Initialize(g_pDP,
						   DirectVoiceServerMessageHandler, NULL, 0, 0))) {
				printf(
					"Failed Initializing the IDirectPlayVoiceServer Object:  0x%X\n",
					hr);
			} else {
				...
				dvSessionDesc.dwBufferAggressiveness =
	...
	if (SUCCEEDED(hr)) {
		...
		if (FAILED(hr = TestDirectVoice())) {
			...
			if (FAILED(hr = CoCreateInstance(CLSID_DirectPlayVoiceClient, NULL,
				...
				printf(
					"Failed Creating the IDirectPlayVoiceClient Object:  0x%X\n",
					hr);
			} else {

				// Init the Voice client
				if (FAILED(hr = g_pVoiceClient->Initialize(g_pDP,
							   DirectVoiceClientMessageHandler, NULL, 0, 0))) {
					printf(
						"Failed Initializing the IDirectPlayVoiceClient Object:  0x%X\n",
						hr);
				} else {

					ZeroMemory(
						&dvSoundDeviceConfig, sizeof(DVSOUNDDEVICECONFIG));
					dvSoundDeviceConfig.dwSize = sizeof(DVSOUNDDEVICECONFIG);
					dvSoundDeviceConfig.dwFlags = DVSOUNDCONFIG_AUTOSELECT;
					dvSoundDeviceConfig.guidPlaybackDevice =
						DSDEVID_DefaultVoicePlayback;
					dvSoundDeviceConfig.lpdsPlaybackDevice = NULL;
					dvSoundDeviceConfig.guidCaptureDevice =
					...
					dvClientConfig.dwBufferAggressiveness =
						DVBUFFERAGGRESSIVENESS_DEFAULT;
					dvClientConfig.dwNotifyPeriod = 0;

					if (FAILED(
							hr = g_pVoiceClient->Connect(&dvSoundDeviceConfig,
								&dvClientConfig, DVFLAGS_SYNC))) {
						printf(
							"Failed DirectVoice Client Connect:  0x%X\n", hr);
					} else {

						dvid = DVID_ALLPLAYERS;
						if (FAILED(hr = g_pVoiceClient->SetTransmitTargets(
									   &dvid, 1, 0))) {
