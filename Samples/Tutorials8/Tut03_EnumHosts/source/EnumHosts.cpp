...
GUID g_guidApp = {0x358e1c8d, 0xdb4a, 0x4867,
	{0xb2, 0xc3, 0xde, 0xf3, 0xf5, 0x4, 0x6b, 0x17}};
...
static HRESULT EnumDirectPlayHosts()
{
	...
	if (FAILED(hr = CreateHostAddress(wszHost))) {
					   ...
					   DPNENUMHOSTS_SYNC)))          // dwFlags
		{
			...
			for (pHostNode = g_pHostList; pHostNode;
				 pHostNode = pHostNode->pNext, dwNumChars = 0) {
				...
				if (hr == DPNERR_BUFFERTOOSMALL) {
					...
					if (pwszURL &&
						SUCCEEDED(hr = pHostNode->pHostAddress->GetURLW(
									  pwszURL, &dwNumChars))) {
						printf("Found a Session Called %ls at Address:  %ls\n",
...
int main(int /* argc */, char** /* argv */)
{
	...
	if (FAILED(hr = InitDirectPlay())) {
		...
		// Get the necessary user input on whether they are hosting or
		// connecting
		do {
