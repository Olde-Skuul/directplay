...
// Desc:
//-----------------------------------------------------------------------------
static LONG FAR PASCAL EditCtrlSubProc(
	HWND hWnd, WORD wMsg, WORD wParam, LONG lParam)
{
	switch (wMsg) {
	case WM_GETDLGCODE:
		return (DLGC_WANTALLKEYS |
			CallWindowProcA(gwpOrigEditProc, hWnd, wMsg, wParam, lParam));
...
// Desc:
//-----------------------------------------------------------------------------
static BOOL FAR PASCAL TV_EnumPlayersCallback(DPID dpId, DWORD dwPlayerType,
	...
	if (DPPLAYERTYPE_GROUP == dwPlayerType) {
		if (DPENUMGROUPS_SHORTCUT & dwFlags) {
			...
			if (NULL == p->dpidParent) {
				...
				p->lpTree->CreateGroupInGroup(
					p->dpidParent, dpId, pName->lpszShortNameA, dwFlags);
			}
			if (ets.bRecursive) {
				hr = p->lpTree->m_lpDP4A->EnumGroupsInGroup(dpId, NULL,
					TV_EnumPlayersCallback, &ets, DPENUMPLAYERS_ALL);
...
// Desc:
//-----------------------------------------------------------------------------
static int CALLBACK CompareFunc(
...
CGroupTree::CGroupTree()
{
	...
	// Prepare popup menus
	m_hMenu = LoadMenuA(m_hInst, MAKEINTRESOURCEA(IDM_MENU));
	...
	m_hImageList = ImageList_LoadImage(m_hInst, MAKEINTRESOURCE(IDB_BITMAP1),
		32, 4, RGB(0, 128, 128), IMAGE_BITMAP, 0);
...
// Desc:
//-----------------------------------------------------------------------------
HTREEITEM CGroupTree::FindItem(
	HTREEITEM htiSearchRoot, DPID dpidTarget, BRANCH_TYPE bt, DWORD dwSearch)
{
	TV_ITEM tvi;
	HTREEITEM hItem;
	...
	while (hItem) {
		...
		if ((bRet) && (lpbs)) {
			...
			OutputDebugStringA(
				"CGroupTree::FindItem: Tree view item has no data.\r\n");
		}
...
HTREEITEM CGroupTree::Insert(HTREEITEM htiParent, DPID dpID,
	LPSTR lpszShortNameA, BRANCH_TYPE bt, DWORD dwFlags)
{
	BRANCHSTRUCT* lpbs = NULL;
	...
	if ((lpbs = (BRANCHSTRUCT*)LocalAlloc(LPTR, sizeof(BRANCHSTRUCT))) != 0) {
		TV_INSERTSTRUCT tvi;
		TV_SORTCB tvscb;
		...
		tvi.item.cchTextMax = static_cast<int>(strlen(lpszShortNameA) + 1);

		switch (bt) {
		case BT_PLAYER:
		case BT_PLAYER_IN_GROUP:
			tvi.item.iImage =
				(DPPLAYER_SPECTATOR & dwFlags) ? IMG_SPECTATOR : IMG_PLAYER;
		...
		case BT_GROUP:
			GetGroupIcons(
				dpID, dwFlags, &tvi.item.iImage, &tvi.item.iSelectedImage);
			break;
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::CreatePlayer(
	DPID /* dpidPlayer */, LPSTR /* strShortNameA */, DWORD /* dwFlags */)
{
	/*
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::CreateGroup(
	DPID dpidGroup, LPSTR strShortNameA, DWORD dwFlags)
{
	DPNAME* pName = NULL;
...
HRESULT CGroupTree::CreateGroupInGroup(DPID dpidParentGroup,
	DPID dpidChildGroup, LPSTR strShortNameA, DWORD dwFlags)
{
	HTREEITEM hParentItem = NULL;

	if ((hParentItem = FindItem(
			 TVI_ROOT, dpidParentGroup, BT_GROUP, ST_SEARCH_SUBGROUPS)) != 0) {
		DPNAME* pName = NULL;
		...
		if (strShortNameA) {
			Insert(
				hParentItem, dpidChildGroup, strShortNameA, BT_GROUP, dwFlags);
		}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::AddPlayerToGroup(
	DPID dpidGroup, DPID dpidPlayer, DWORD dwFlags)
{
	HRESULT hr = DPERR_GENERIC;
	...
	if (SUCCEEDED(hr)) {
		...
		if ((hParentItem = FindItem(
				 TVI_ROOT, dpidGroup, BT_GROUP, ST_SEARCH_SUBGROUPS)) != 0) {
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::AddGroupToGroup(
	DPID dpidParentGroup, DPID dpidShortcut, DWORD dwFlags)
{
	HRESULT hr = DPERR_GENERIC;
...
HRESULT CGroupTree::DestroyPlayer(DPID /* dpidPlayer */)
{
	...
	HTREEITEM htiPlayer =
		FindItem(TVI_ROOT, dpidPlayer, BT_PLAYER, ST_NO_SUBGROUPS);
...
{
	HTREEITEM htiGroup =
		FindItem(TVI_ROOT, dpidGroup, BT_GROUP, ST_SEARCH_SUBGROUPS);
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::DeleteGroupFromGroup(
	DPID dpidParentGroup, DPID dpidShortcut)
{
	HRESULT hr = DPERR_GENERIC;
	HTREEITEM htiParent = NULL;
	...
	if ((htiParent = FindItem(
		...
		if ((htiShortcut = FindItem(htiParent, dpidShortcut,
				 BT_SHORTCUT_IN_GROUP, ST_NO_SUBGROUPS)) != 0) {
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CGroupTree::RecursiveRename(
	HTREEITEM htiSearchRoot, DPID dpidTarget, LPSTR strName)
{
	TV_ITEM tvi;
	HTREEITEM hItem;
	BRANCHSTRUCT* lpbs = NULL;
	...
	while (hItem) {
		...
		if (lpbs->dpid == dpidTarget) {
			...
			tvi.mask = TVIF_TEXT;
			tvi.pszText = strName;
			tvi.cchTextMax = static_cast<int>(strlen(strName) + 1);
...
HRESULT CGroupTree::Refresh(BOOL bRecursive)
{
	...
	if (m_lpDP4A) {
		...
		// hr = m_lpDP4A->EnumPlayers(
		//	NULL, TV_EnumPlayersCallback, &ets, DPENUMPLAYERS_ALL);

		// if (SUCCEEDED(hr)) {
		hr = m_lpDP4A->EnumGroups(
			NULL, TV_EnumPlayersCallback, &ets, DPENUMPLAYERS_ALL);
		//}
	}
...
BOOL CGroupTree::Update(VOID* lpvMsg)
{
	...
	case DPSYS_CREATEPLAYERORGROUP: {
		...
		if (DPPLAYERTYPE_PLAYER == lp->dwPlayerType) {
			...
			if (NULL == lp->dpIdParent) {
				...
				CreateGroupInGroup(
					lp->dpIdParent, lp->dpId, lpszShortName, lp->dwFlags);
			}
		}
	} break;
	...
	case DPSYS_DELETEPLAYERFROMGROUP: {
		LPDPMSG_DELETEPLAYERFROMGROUP lp =
			(LPDPMSG_DELETEPLAYERFROMGROUP)lpvMsg;
...
VOID CGroupTree::OnBeginDrag(NM_TREEVIEW* lpnmtv)
{
	...
	// BRANCHSTRUCT* lp = (BRANCHSTRUCT*)lpnmtv->itemNew.lParam;
	...
	// for dragging.
	// Handle of image list
	HIMAGELIST himl =
		TreeView_CreateDragImage(m_hwndTreeView, lpnmtv->itemNew.hItem);
	...
	UINT xIndent = TreeView_GetIndent(m_hwndTreeView) * level;

	ImageList_BeginDrag(himl, 0,
		static_cast<int>(lpnmtv->ptDrag.x - rcItem.left - xIndent),
...
VOID CGroupTree::OnMouseMove(LONG xCur, LONG yCur)
{
	...
	if (m_fDragging) {
		...
		if ((htiTarget = TreeView_HitTest(m_hwndTreeView, &tvht)) != NULL) {
			...
			memcpy(&m_bsDropTarget, (BRANCHSTRUCT*)tvi.lParam,
				sizeof(BRANCHSTRUCT));
...
VOID CGroupTree::OnLButtonUp()
{
	if (m_fDragging) {
		...
		if (m_bsDropTarget.dpid) {
			...
			case BT_PLAYER:
				m_lpDP4A->AddPlayerToGroup(
					m_bsDropTarget.dpid, m_bsDragging.dpid);
				break;

			case BT_GROUP:
				m_lpDP4A->AddGroupToGroup(
					m_bsDropTarget.dpid, m_bsDragging.dpid);
				break;
			...
			case BT_PLAYER_IN_GROUP:
				if (m_bsDropTarget.dpid != m_bsParentOfDragging.dpid) {
					m_lpDP4A->AddPlayerToGroup(
						m_bsDropTarget.dpid, m_bsDragging.dpid);
					m_lpDP4A->DeletePlayerFromGroup(
						m_bsParentOfDragging.dpid, m_bsDragging.dpid);
				}
				break;
...
void CGroupTree::OnRButtonDown(LONG xCur, LONG yCur)
{
	...
	if ((htiTarget = TreeView_HitTest(m_hwndTreeView, &tvht)) != NULL) {
		...
		if ((lpbs = (BRANCHSTRUCT*)tvi.lParam) != 0) {
			...
			case BT_GROUP:
				...
				EnableMenuItem(hMenu, ID_GROUP_STARTSESSION,
					static_cast<UINT>((DPGROUP_STAGINGAREA & lpbs->dwFlags) ?
							MF_ENABLED :
							MF_GRAYED));
...
HRESULT CGroupTree::BeginLabelEdit()
{
	...
	gwpOrigEditProc = (SWNDPROC)SetWindowLongA(
		m_hwndEditCtrl, GWL_WNDPROC, (LONG)EditCtrlSubProc);
...
void CGroupTree::EndLabelEdit(TV_DISPINFO FAR* lpTVDisp)
{
	...
	// Subclass the edit control.
	SetWindowLongA(m_hwndEditCtrl, GWL_WNDPROC, (LONG)gwpOrigEditProc);
	...
	if (lpTVDisp->item.pszText) {
		...
		if (SUCCEEDED(hr)) {
			...
			MessageBoxA(m_hwndParent,
				"Cannot change the name of remote players or groups", "Error",
				MB_OK);
			return;
		}
	}
...
// Desc:
//-----------------------------------------------------------------------------
DPID CGroupTree::GetBranchStructOfParent(
	HTREEITEM htChildItem, BRANCHSTRUCT* lpbt)
{
...
// Desc:
//-----------------------------------------------------------------------------
void CGroupTree::GetGroupIcons(
	DPID dpID, DWORD dwFlags, int* lpnNormal, int* lpnSelected)
{
	if (DPGROUP_STAGINGAREA & dwFlags) {
		...
		if (DPGROUP_HIDDEN & dwFlags) {
			*lpnNormal = (bSessionInProgress) ?
				IMG_HIDDEN_STAGING_AREA_IN_PROGRESS :
				IMG_HIDDEN_STAGING_AREA;
			*lpnSelected = *lpnNormal;
		} else {
			*lpnNormal = (bSessionInProgress) ? IMG_STAGING_AREA_IN_PROGRESS :
												IMG_STAGING_AREA;
			*lpnSelected = *lpnNormal;
		}
	} else {
