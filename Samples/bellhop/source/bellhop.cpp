...
// globals
static HANDLE g_hReceiveThread = NULL;    // Handle of receive thread
static DWORD g_idReceiveThread = 0;       // ID of receive thread
static HANDLE g_hKillReceiveEvent = NULL; // Event used to kill receive thread
static HWND g_hMainWnd = NULL;            // Main window
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT GetPlayerName(
	LPDIRECTPLAY4A pDP, DPID dpidPlayer, DPNAME** ppName)
{
	DPNAME* pName;
	DWORD dwNameSize;
	HRESULT hr;
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT GetGroupName(LPDIRECTPLAY4A pDP, DPID dpidGroup, DPNAME** ppName)
{
	DPNAME* pName = NULL;
	DWORD dwNameSize;

	// Get size of player name data
	HRESULT hr = pDP->GetGroupName(dpidGroup, NULL, &dwNameSize);
	if (hr != DPERR_BUFFERTOOSMALL) {
		return hr;
	}
...
// Desc:
//-----------------------------------------------------------------------------
static VOID LogString(LPSTR strDisplayFormat, LPSTR strData)
{
	// Allocate space for string
	LPSTR strStr = (LPSTR)GlobalAllocPtr(
		GHND, strlen(strDisplayFormat) + strlen(strData) + 1);
	if (strStr == NULL) {
		return;
	}

	// Build string
	sprintf(strStr, strDisplayFormat, strData);

	// Post string to chat window
	// Make sure window is still valid
	if (g_hMainWnd) {
		PostMessageA(g_hMainWnd, WM_USER_ADDSTRING, 0, (LPARAM)strStr);
	} else {
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT WaitForRunMsg(
	DWORD dwAppId, HANDLE hReceiveEvent, DWORD* pdwStatus, DPLAYINFO* pDPInfo)
{
	VOID* pMsg = NULL;
	DWORD dwMsgFlags;
	DWORD dwMsgSize = 0;
	...
	// For simplicity sake, the app/lobby client messaging loop is contained
	// below.
	...
	*pdwStatus = 0;
	...
	while (bNotFinished) {
		...
		do {
			hr = pDPInfo->pDPLobby->ReceiveLobbyMessage(
				0, dwAppId, &dwMsgFlags, pMsg, &dwMsgSize);
...
// Desc:
//-----------------------------------------------------------------------------
static BOOL FAR PASCAL GetLocalAppNameCallback(
	const DPLAPPINFO* pAppInfo, VOID* pContext, DWORD /* dwFlags */)
{
	APPNAMECONTEXT* pAppNameContext = (APPNAMECONTEXT*)pContext;

	if (IsEqualGUID(
			pAppInfo->guidApplication, pAppNameContext->guidApplication)) {
		strcpy(pAppNameContext->strAppName, pAppInfo->lpszAppNameA);
		return FALSE;
	}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT GetLocalAppName(
	LPDIRECTPLAYLOBBY pDPLobby, GUID* pguidApplication, LPSTR strAppName)
{
	HRESULT hr;
	...
	hr = pDPLobby->EnumLocalApplications(
		GetLocalAppNameCallback, &AppNameContext, 0);
	if (FAILED(hr)) {
		return hr;
	}

	// Return failure if no local app found matching this guid
	if (strlen(AppNameContext.strAppName) == 0) {
		return DPERR_GENERIC;
	}

	// Return name
	strcpy(strAppName, AppNameContext.strAppName);
...
// Desc:
//-----------------------------------------------------------------------------
static VOID HandleStartSession(DPLCONNECTION* pConn, DPLAYINFO* pDPInfo)
{
	DWORD dwAppID;
	...
	if (hEvent) {
		...
		HRESULT hr = GetLocalAppName(
			pDPInfo->pDPLobby, &pConn->lpSessionDesc->guidApplication, strStr);
		if (FAILED(hr)) {
			strcpy(strStr, "unknown");
		}
...
// Desc:
//-----------------------------------------------------------------------------
static VOID HandleApplicationMessage(DPLAYINFO* /* pDPInfo */,
	DPMSG_GENERIC* /* pMsg */, DWORD /* dwMsgSize */, DPID /* idFrom */,
	DPID /* idTo */)
{
	...
	DWORD /* dwMsgSize */, DPID /* idFrom */, DPID /* idTo */)
{
	...
	case DPSYS_CREATEPLAYERORGROUP: {
		...
		strStr = (LPSTR)GlobalAllocPtr(
			GHND, strlen(strDisplayFormat) + strlen(strName) + 1);
		if (strStr == NULL) {
			break;
		}

		// Build string
		sprintf(strStr, strDisplayFormat, strName);
		break;
	}
	...
	case DPSYS_DELETEPLAYERFROMGROUP: {
		...
		DPMSG_DELETEPLAYERFROMGROUP* pDeleteMsg =
			(DPMSG_DELETEPLAYERFROMGROUP*)pMsg;
		...
		// Allocate space for string
		strStr = (LPSTR)GlobalAllocPtr(GHND,
			strlen(strDisplayFormat) + strlen(strPlayerName) +
				strlen(strGroupName) + 1);
		// Build string
		if (strStr) {
			sprintf(strStr, strDisplayFormat, strPlayerName, strGroupName);
		}
	...
	case DPSYS_DELETEGROUPFROMGROUP: {
		...
		DPMSG_DELETEGROUPFROMGROUP* pDeleteMsg =
			(DPMSG_DELETEGROUPFROMGROUP*)pMsg;
		...
		// Allocate space for string
		strStr = (LPSTR)GlobalAllocPtr(GHND,
			strlen(strDisplayFormat) + strlen(strParentGroupName) +
				strlen(strGroupName) + 1);
		// Build string
		if (strStr) {
			sprintf(strStr, strDisplayFormat, strGroupName, strParentGroupName);
		}
	...
	case DPSYS_SESSIONLOST: {
		...
		// Allocate space for string
		strStr = (LPSTR)GlobalAllocPtr(GHND, strlen(strDisplayFormat) + 1);
		if (strStr == NULL) {
			break;
		}

		// Build string
		strcpy(strStr, strDisplayFormat);
		break;
	}
	...
	case DPSYS_SETGROUPOWNER: {
		...
		LPSTR strDisplayFormat =
			"Owner changed for group \"%s\" to \"%s\" from \"%s\"\r\n";
		HRESULT hr;
		...
		// Allocate space for string
		strStr = (LPSTR)GlobalAllocPtr(GHND,
			strlen(strDisplayFormat) + strlen(strGroupName) +
				strlen(strNewOwnerName) + strlen(strOldOwnerName) + 1);
		// Build string
		if (strStr) {
			sprintf(strStr, strDisplayFormat, strGroupName, strNewOwnerName,
				strOldOwnerName);
		}
	...
	case DPSYS_CHAT: {
		...
		DWORD dwSize = strlen(p->lpChat->lpszMessageA) + 12;
		...
		if (NULL == p->idToGroup) {
			...
			strcpy(strStr, "{whisper} ");
			strcat(strStr, p->lpChat->lpszMessageA);
		} else {
			// build string
			strcpy(strStr, p->lpChat->lpszMessageA);
		}
	} break;
	}
...
// Desc:
//-----------------------------------------------------------------------------
static DWORD WINAPI ReceiveThread(VOID* pThreadParameter)
{
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoGroupConnectionSettings(HWND hWnd, DPLAYINFO* pDPInfo)
{
	...
	if (DialogBoxParamA(g_hInstance,
			MAKEINTRESOURCEA(IDD_CONNECTIONSETTINGSDIALOG), hWnd,
...
// Desc:
//-----------------------------------------------------------------------------
static VOID InitializeLobbyGroupWindow(HWND hWnd, LOBBYGROUPCONTEXT* pContext)
{
	ENUMCONNSTRUCT enStruct;

	if (pContext->dpidRoom == 0) {
		SetWindowTextA(hWnd, "Create Room");
	} else {
		SetWindowTextA(hWnd, "Create Table");
	}
	...
	pContext->pDPInfo->pDP->EnumConnections(&BELLHOP_GUID,
		DirectPlayEnumConnectionsCallback, &enStruct, DPCONNECTION_DIRECTPLAY);

	SendDlgItemMessageA(hWnd, IDC_GROUPCONNECTIONSPCOMBO, CB_SETCURSEL, 0, 0);
}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT CreateLobbyGroup(HWND hWnd, LOBBYGROUPCONTEXT* pContext)
{
	CHAR strShortName[MAXSTRLEN];
	CHAR strLongName[MAXSTRLEN];
	CHAR strPassword[MAXSTRLEN];
	DPNAME dpName;
	DWORD dwMaxPlayers = 0;
	DPID dpID;
	...
	GetDlgItemTextA(hWnd, IDC_SHORTNAMEEDIT, strShortName, MAXSTRLEN);
	GetDlgItemTextA(hWnd, IDC_LONGNAMEEDIT, strLongName, MAXSTRLEN);
	...
	if (BST_CHECKED ==
		SendDlgItemMessageA(hWnd, IDC_HIDDEN, BM_GETCHECK, 0, 0)) {
	...
	if (pContext->dpidRoom == 0) {
		// Create a root group
		hr = pContext->pDPInfo->pDP->CreateGroup(
			&dpID, &dpName, NULL, 0, dwFlags);
	} else {
		// Create the table group
		hr = pContext->pDPInfo->pDP->CreateGroupInGroup(
			pContext->dpidRoom, &dpID, &dpName, NULL, 0, dwFlags);
	}
...
// Desc:
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK LobbyGroupWndProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LOBBYGROUPCONTEXT* pContext =
		(LOBBYGROUPCONTEXT*)GetWindowLongA(hWnd, DWL_USER);
	...
	case WM_DESTROY: {
		...
		for (;;) {
			pData = SendDlgItemMessageA(
				hWnd, IDC_APPCOMBO, CB_GETITEMDATA, index, 0);
		...
		for (;;) {
			pData = SendDlgItemMessageA(
				hWnd, IDC_GROUPCONNECTIONSPCOMBO, CB_GETITEMDATA, index, 0);
			if ((pData == CB_ERR) || (pData == 0)) {
				break;
			}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoCreateRoom(HWND hWnd, DPLAYINFO* pDPInfo)
{
	LOBBYGROUPCONTEXT context;
	context.pDPInfo = pDPInfo;
	context.dpidRoom = (DPID)0;

	if (DialogBoxParamA(g_hInstance, MAKEINTRESOURCEA(IDD_LOBBYGROUPDIALOG),
			hWnd, LobbyGroupWndProc, (LPARAM)&context)) {
		// something changed
		// We could update a status bar here.
	}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoDeleteRoom(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoDeletePlayerFromGroup(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoCreateTable(HWND hWnd, DPLAYINFO* pDPInfo)
{
	LOBBYGROUPCONTEXT context;
	context.pDPInfo = pDPInfo;
	context.dpidRoom = pDPInfo->pGroupTree->GetDPIDOfCurrentSelection();

	DialogBoxParamA(g_hInstance, MAKEINTRESOURCEA(IDD_LOBBYGROUPDIALOG), hWnd,
		LobbyGroupWndProc, (LPARAM)&context);
	return DP_OK;
}
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoSetGroupOwner(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoDeleteTable(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
	DPID dpidRoom = pDPInfo->pGroupTree->GetDPIDOfCurrentSelectionParent();
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT DoLaunch(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
	DPID dpidRoom = pDPInfo->pGroupTree->GetDPIDOfCurrentSelection();
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT InitializeLobby(HWND /* hWnd */, DPLAYINFO* pDPInfo)
{
	DPNAME name;
	DPID dpID;
	HRESULT hr;
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT NewChatString(
	LPDIRECTPLAY4A pDP, DPID dpidPlayer, LPSTR strMsg, LPSTR* pstrStr)
{
	DPNAME* pName = NULL;
	LPSTR strStr = NULL;
	LPSTR strPlayerName;
	...
	// Get name of player
	HRESULT hr = GetPlayerName(pDP, dpidPlayer, &pName);
	if (SUCCEEDED(hr)) {
		...
		strStr = (LPSTR)GlobalAllocPtr(GHND,
			strlen(strDisplayFormat) + strlen(strPlayerName) + strlen(strMsg) +
				1);
		if (strStr == NULL) {
			hr = DPERR_OUTOFMEMORY;
		} else {
			...
			// Build string
			sprintf(strStr, strDisplayFormat, strPlayerName, strMsg);
...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT SendChatMessage(HWND hWnd, DPLAYINFO* pDPInfo)
{
	LPSTR strStr = NULL;
	HRESULT hr;
	DPCHAT dpc;
	...
	LONG lStrLen = static_cast<LONG>(
		SendDlgItemMessageA(hWnd, IDC_SENDEDIT, WM_GETTEXTLENGTH, 0, 0));

	// Make room for it
	LPSTR strChatStr =
		(LPSTR)GlobalAllocPtr(GHND, static_cast<SIZE_T>(lStrLen + 1));
	if (strChatStr == NULL) {
		hr = DPERR_OUTOFMEMORY;
	} else {
		...
		lStrLen = static_cast<LONG>(
			GetDlgItemTextA(hWnd, IDC_SENDEDIT, strChatStr, lStrLen + 1));

		// Create string to display this text
		hr = NewChatString(
			pDPInfo->pDP, pDPInfo->dpidPlayer, strChatStr, &strStr);
		if (SUCCEEDED(hr)) {
			...
			hr = pDPInfo->pDP->SendChatMessage(
				pDPInfo->dpidPlayer, dpidTarget, DPSEND_GUARANTEED, &dpc);
			if (SUCCEEDED(hr)) {

				// Display this string
				PostMessageA(hWnd, WM_USER_ADDSTRING, 0, (LPARAM)strStr);
...
// Desc:
//-----------------------------------------------------------------------------
static VOID OnSize(HWND hWnd, DPLAYINFO* pDPInfo)
{
	HDWP hDWP;
	RECT ClientRect;
	int Height;
	...
	if ((hDWP = BeginDeferWindowPos(4)) != NULL) {
		//  Data structure used when calling GetEffectiveClientRect (which takes
		//  into account space taken up by the toolbars/status bars).  First
		//  half of the pair is zero when at the end of the list, second half is
		//  the control id.
		...
		DeferWindowPos(hDWP, hKeyTreeWnd, NULL, xSpacing,
			ClientRect.top + ySpacing, pDPInfo->xPaneSplit, Height,
		...
		DeferWindowPos(hDWP, hValueListWnd, NULL, x, ClientRect.top + ySpacing,
			dx, Height, SWP_NOZORDER | SWP_NOACTIVATE);
		...
		DeferWindowPos(hDWP, hSendButtonWnd, NULL,
			...
			ClientRect.bottom - (sendbuttonheight + ySpacing), sendbuttonwidth,
		...
		DeferWindowPos(hDWP, hMsgEditWnd, NULL, ClientRect.left + xSpacing,
			ClientRect.bottom - (sendbuttonheight + ySpacing),
			ClientRect.right - ClientRect.left -
				(sendbuttonwidth + xSpacing * 3),
			sendbuttonheight, SWP_NOZORDER | SWP_NOACTIVATE);
...
// Desc:
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK LobbyWndProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		pDPInfo->xPaneSplit =
			((rect.right - rect.left) - (2 * pDPInfo->xSpacing)) / 3 -
		...
		if (pDPInfo->pGroupTree) {
			...
			g_hReceiveThread = CreateThread(
				NULL, 0, ReceiveThread, pDPInfo, 0, &g_idReceiveThread);
			...
			pDPInfo->pGroupTree->Init(GetDlgItem(hWnd, IDT_MESSAGEVIEW),
				pDPInfo->pDP, pDPInfo->dpidPlayer);
			pDPInfo->pGroupTree->Refresh();
			pDPInfo->pGroupTree->CreatePlayer(
				pDPInfo->dpidPlayer, NULL, pDPInfo->dwPlayerFlags);
			// Store global window
			g_hMainWnd = hWnd;
...
{
	CHAR strStr[MAXSTRLEN];
	sprintf(strStr, strError, GetDirectPlayErrStr(hr));

	MessageBoxA(NULL, strStr, "Error", MB_OK);
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
	...
	int iResult = DialogBoxParamA(hInstance, MAKEINTRESOURCE(IDD_LOBBYDIALOG),
		NULL, LobbyWndProc, (LPARAM)&DPInfo);
