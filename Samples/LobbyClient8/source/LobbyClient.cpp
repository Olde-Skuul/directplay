...
//
// Desc: LobbyClient is a simple lobby client.  It displays all registered
// DirectPlay
	...
	PVOID /* pvUserContext */, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPL_MSGID_DISCONNECT: {
		...
		PostMessage(g_hDlg, WM_APP_APPDISCONNECTED, 0,
			static_cast<LPARAM>(pDisconnectMsg->hDisconnectId));
		break;
	}
	...
	case DPL_MSGID_SESSION_STATUS: {
		PDPL_MESSAGE_SESSION_STATUS pStatusMsg =
			(PDPL_MESSAGE_SESSION_STATUS)pMsgBuffer;
		...
		case DPLSESSION_CONNECTED:
			_tcscat(strBuffer, TEXT("Session connected"));
			break;
		case DPLSESSION_COULDNOTCONNECT:
			_tcscat(strBuffer, TEXT("Session could not connect"));
			break;
		case DPLSESSION_DISCONNECTED:
			_tcscat(strBuffer, TEXT("Session disconnected"));
			break;
		case DPLSESSION_TERMINATED:
			_tcscat(strBuffer, TEXT("Session terminated"));
			break;
		case DPLSESSION_HOSTMIGRATED:
			_tcscat(strBuffer, TEXT("Host migrated"));
			break;
		case DPLSESSION_HOSTMIGRATEDHERE:
			_tcscat(strBuffer, TEXT("Host migrated to this client"));
			break;
...
static HRESULT InitDirectPlay()
{
	...
	if (FAILED(hr = CoCreateInstance(CLSID_DirectPlay8LobbyClient, NULL,
				   CLSCTX_INPROC_SERVER, IID_IDirectPlay8LobbyClient,
				   (LPVOID*)&g_pLobbyClient))) {
		return DXTRACE_ERR(TEXT("CoCreateInstance"), hr);
	}

	if (FAILED(hr = g_pLobbyClient->Initialize(
				   NULL, DirectPlayLobbyMessageHandler, 0))) {
		return DXTRACE_ERR(TEXT("Initialize"), hr);
	}
...
//-----------------------------------------------------------------------------
static VOID FreeConnectSettings(DPL_CONNECTION_SETTINGS* pSettings)
{
	if (!pSettings) {
		return;
	}
	...
	if (pSettings->dpnAppDesc.pvReservedData) {
		delete[] static_cast<char*>(pSettings->dpnAppDesc.pvReservedData);
		pSettings->dpnAppDesc.pvReservedData = NULL;
	}
	if (pSettings->dpnAppDesc.pvApplicationReservedData) {
		delete[] static_cast<char*>(
			pSettings->dpnAppDesc.pvApplicationReservedData);
		pSettings->dpnAppDesc.pvApplicationReservedData = NULL;
	}
...
//       later to free it.
//-----------------------------------------------------------------------------
static HRESULT AllocAndInitConnectSettings(
	HWND hDlg, GUID* pAppGuid, DPL_CONNECTION_SETTINGS** ppdplConnectSettings)
{
	HRESULT hr;

	BOOL bHosting =
		static_cast<BOOL>(IsDlgButtonChecked(hDlg, IDC_HOST_SESSION));
	BOOL bNoSettings =
		static_cast<BOOL>(IsDlgButtonChecked(hDlg, IDC_NO_SETTINGS));
	...
	int nSPIndex =
		(int)SendDlgItemMessage(hDlg, IDC_SP_COMBO, CB_GETCURSEL, 0, 0);
	GUID* pSPGuid = (GUID*)SendDlgItemMessage(
		hDlg, IDC_SP_COMBO, CB_GETITEMDATA, static_cast<WPARAM>(nSPIndex), 0);

	if (*pSPGuid == CLSID_DP8SP_IPX && !bHosting) {
		MessageBox(hDlg,
			TEXT(
				"A typical lobby client would have sent a complete IPX address, this ")
				TEXT("lobby client does not support this. "),
			g_strAppName, MB_OK);
		return E_NOTIMPL;
	}
	...
	int nAdapterIndex =
		(int)SendDlgItemMessage(hDlg, IDC_ADAPTER_COMBO, CB_GETCURSEL, 0, 0);
	GUID* pAdapterGuid = (GUID*)SendDlgItemMessage(hDlg, IDC_ADAPTER_COMBO,
		CB_GETITEMDATA, static_cast<WPARAM>(nAdapterIndex), 0);
	...
	if (*pSPGuid == CLSID_DP8SP_TCPIP) {
		...
		if (bHosting) {
			...
			DWORD dwPort = static_cast<DWORD>(_ttoi(strPort));
			if (FAILED(hr = pDeviceAddress->AddComponent(DPNA_KEY_PORT, &dwPort,
			...
			DWORD dwPort = static_cast<DWORD>(_ttoi(strPort));
			if (FAILED(hr = pHostAddress->AddComponent(DPNA_KEY_PORT, &dwPort,
						   sizeof(dwPort), DPNA_DATATYPE_DWORD))) {
				return DXTRACE_ERR(TEXT("AddComponent"), hr);
			}
		}
		...
		if (!bHosting) {
			...
			if (_tcslen(strPhone) > 0) {
				...
				if (FAILED(
						hr = pHostAddress->AddComponent(DPNA_KEY_PHONENUMBER,
	...
	if (_tcslen(strSessionName) == 0) {
		...
		pSettings->dpnAppDesc.pwszSessionName =
			new WCHAR[wcslen(wstrSessionName) + 1];
...
//-----------------------------------------------------------------------------
static HRESULT LaunchApp(HWND hDlg)
{
	HRESULT hr;
	DPNHANDLE hApplication = NULL;

	BOOL bLaunchNew =
		static_cast<BOOL>(IsDlgButtonChecked(hDlg, IDC_LAUNCH_NEW));
	BOOL bLaunchNotFound =
		static_cast<BOOL>(IsDlgButtonChecked(hDlg, IDC_LAUNCH_NOT_FOUND));

	int nAppIndex =
		(int)SendDlgItemMessage(hDlg, IDC_APP_LIST, LB_GETCURSEL, 0, 0);
	GUID* pAppGuid = (GUID*)SendDlgItemMessage(
		hDlg, IDC_APP_LIST, LB_GETITEMDATA, static_cast<WPARAM>(nAppIndex), 0);
	...
	if (FAILED(hr = AllocAndInitConnectSettings(
				   hDlg, pAppGuid, &dnConnectInfo.pdplConnectionSettings))) {
		return S_FALSE;
	}

	hr = g_pLobbyClient->ConnectApplication(
		&dnConnectInfo, NULL, &hApplication, INFINITE, 0);
	if (FAILED(hr)) {
		...
		int nIndex = (int)SendDlgItemMessage(
			hDlg, IDC_ACTIVE_CONNECTIONS, LB_ADDSTRING, 0, (LPARAM)strBuffer);
		SendDlgItemMessage(hDlg, IDC_ACTIVE_CONNECTIONS, LB_SETITEMDATA,
			static_cast<WPARAM>(nIndex), (LPARAM)hApplication);

		if (LB_ERR ==
			SendDlgItemMessage(
				hDlg, IDC_ACTIVE_CONNECTIONS, LB_GETCURSEL, 0, 0)) {
			SendDlgItemMessage(
				hDlg, IDC_ACTIVE_CONNECTIONS, LB_SETCURSEL, 0, 0);
		}
	}
...
//-----------------------------------------------------------------------------
static HRESULT DisconnectFromApp(HWND hDlg)
{
	HRESULT hr;

	int nConnectIndex = (int)SendDlgItemMessage(
		hDlg, IDC_ACTIVE_CONNECTIONS, LB_GETCURSEL, 0, 0);
	if (nConnectIndex == LB_ERR) {
		return S_OK;
	}

	DPNHANDLE hApplication =
		(DPNHANDLE)SendDlgItemMessage(hDlg, IDC_ACTIVE_CONNECTIONS,
			LB_GETITEMDATA, static_cast<WPARAM>(nConnectIndex), 0);
	...
	SendDlgItemMessage(hDlg, IDC_ACTIVE_CONNECTIONS, LB_DELETESTRING,
		static_cast<WPARAM>(nConnectIndex), 0);
...
//-----------------------------------------------------------------------------
static HRESULT EnumRegisteredApplications(HWND hDlg)
{
	DWORD dwSize = 0;
	DWORD dwPrograms = 0;
	DWORD iProgram;
	...
	HRESULT hr =
		g_pLobbyClient->EnumLocalPrograms(NULL, NULL, &dwSize, &dwPrograms, 0);
	if (hr != DPNERR_BUFFERTOOSMALL && FAILED(hr))
		return DXTRACE_ERR(TEXT("EnumLocalPrograms"), hr);

	if (dwSize == 0) {
		MessageBox(NULL,
			TEXT("There are no applications registered as lobby launchable. ")
	...
	if (FAILED(hr = g_pLobbyClient->EnumLocalPrograms(
				   NULL, pData, &dwSize, &dwPrograms, 0))) {
		return DXTRACE_ERR(TEXT("EnumLocalPrograms"), hr);
	}
	...
	for (iProgram = 0; iProgram < dwPrograms; iProgram++) {
		...
		DXUtil_ConvertWideStringToGeneric(
			strAppName, pAppInfo->pwszApplicationName);

		// Add the name to the listbox
		int nIndex = (int)SendDlgItemMessage(
			hDlg, IDC_APP_LIST, LB_ADDSTRING, 0, (LPARAM)strAppName);

		// Store the guid in the listbox
		GUID* pGuid = new GUID;
		memcpy(pGuid, &pAppInfo->guidApplication, sizeof(GUID));
		SendDlgItemMessage(hDlg, IDC_APP_LIST, LB_SETITEMDATA,
...
//-----------------------------------------------------------------------------
static VOID SetupAddressFields(HWND hDlg)
{
	int nSPIndex =
		(int)SendDlgItemMessage(hDlg, IDC_SP_COMBO, CB_GETCURSEL, 0, 0);
	if (nSPIndex == LB_ERR) {
	...
	GUID* pGuid = (GUID*)SendDlgItemMessage(
		hDlg, IDC_SP_COMBO, CB_GETITEMDATA, static_cast<WPARAM>(nSPIndex), 0);
	if (pGuid == NULL) {
		return;
	}
...
static HRESULT OnInitDialog(HWND hDlg)
{
	...
	CheckRadioButton(
		hDlg, IDC_LAUNCH_NEW, IDC_DONT_LAUNCH, IDC_LAUNCH_NOT_FOUND);
	...
	if (FAILED(hr = EnumServiceProviders(
				   g_pDP, hDlg, IDC_SP_COMBO, g_strPreferredProvider))) {
	...
	if (pSPGuid != NULL) {
		...
		EnumAdapters(g_pDP, hDlg, pSPGuid, IDC_ADAPTER_COMBO);
	}
...
//-----------------------------------------------------------------------------
static HRESULT SendMsgToApp(HWND hDlg)
{
	...
	// For demonstration purposes, just send a buffer to the app.  This can be
	// used by more complex lobby clients to pass custom information to apps
	// that which can then recieve and process it.
	BYTE buffer[20];
	memset(buffer, 0x03, 20);
	if (FAILED(hr = g_pLobbyClient->Send(hApplication, buffer, 20, 0))) {
		DXTRACE_ERR(TEXT("Send"), hr);
		wsprintf(strBuffer, TEXT("Failure trying to send message to 0x%0.8x."),
			hApplication);
		MessageBox(NULL, strBuffer, g_strAppName, MB_OK | MB_ICONERROR);
	} else {
		wsprintf(strBuffer, TEXT("Successfully sent a message to 0x%0.8x."),
			hApplication);
		MessageBox(NULL, strBuffer, g_strAppName, MB_OK | MB_ICONERROR);
	}
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK LobbyClientDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		if (FAILED(hr = OnInitDialog(hDlg))) {
			...
			MessageBox(NULL,
				TEXT("Failed initializing dialog box. ")
					TEXT("The sample will now quit."),
				g_strAppName, MB_OK | MB_ICONERROR);
			EndDialog(hDlg, 0);
		}
	...
	case WM_APP_APPDISCONNECTED: {
		...
		SendDlgItemMessage(g_hDlg, IDC_ACTIVE_CONNECTIONS, LB_DELETESTRING,
	...
	case WM_COMMAND: {
		...
		case IDC_SP_COMBO: {
			...
			int nSPIndex =
				(int)SendDlgItemMessage(hDlg, IDC_SP_COMBO, CB_GETCURSEL, 0, 0);
			GUID* pSPGuid = (GUID*)SendDlgItemMessage(hDlg, IDC_SP_COMBO,
				CB_GETITEMDATA, static_cast<WPARAM>(nSPIndex), 0);
		...
		case IDC_LAUNCH:
			if (FAILED(hr = LaunchApp(hDlg))) {
				DXTRACE_ERR(TEXT("LaunchApp"), hr);
				MessageBox(NULL,
					TEXT("Failure trying to launch app. ")
						TEXT("The sample will now quit."),
					g_strAppName, MB_OK | MB_ICONERROR);
				EndDialog(hDlg, 0);
			}
	...
	case WM_DESTROY: {
		...
		for (i = 0; i < nCount; i++) {
			pGuid = (LPGUID)SendDlgItemMessage(
				hDlg, IDC_APP_LIST, LB_GETITEMDATA, static_cast<WPARAM>(i), 0);
			SAFE_DELETE(pGuid);
		}
		...
		nCount =
			(int)SendDlgItemMessage(hDlg, IDC_ADAPTER_COMBO, CB_GETCOUNT, 0, 0);
		for (i = 0; i < nCount; i++) {
			pGuid = (LPGUID)SendDlgItemMessage(hDlg, IDC_ADAPTER_COMBO,
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	DXUtil_WriteStringRegKey(
		hDPlaySampleRegKey, TEXT("Player Name"), g_strPlayerName);
	DXUtil_WriteStringRegKey(
		hDPlaySampleRegKey, TEXT("Session Name"), g_strSessionName);
	DXUtil_WriteStringRegKey(
		hDPlaySampleRegKey, TEXT("Preferred Provider"), g_strPreferredProvider);
