...
public:
	...
	virtual HRESULT SendPacket(DWORD /*to*/, void* data, DWORD size,
		BOOL /*guaranteed*/, DWORD /*dwTimeout*/)
	{
		if (m_pTarget) {
	...
	virtual HRESULT GetConnectionInfo(
		DWORD /* dwID */, TCHAR* strConnectionInfo)
	{
...
public:
	...
	virtual HRESULT SendPacket(
		void* data, DWORD size, BOOL /*guaranteed*/, DWORD /*dwTimeout*/)
	{
		if (m_pTarget) {
			return m_pTarget->OnPacket(m_dwID, data, size);
		}
