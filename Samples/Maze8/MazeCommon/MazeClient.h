...
// the server so the server can accept or reject the client based on its
// version.
...
public:
	...
	BOOL IsMazeReady()
	{
		if (WaitForSingleObject(m_hReady, 0) == WAIT_OBJECT_0) {
			return TRUE;
		} else {
			return FALSE;
		}
	...
	{
		if (bReady) {
			SetEvent(m_hGotFirstConfig);
		} else {
			ResetEvent(m_hGotFirstConfig);
		}
	}
	BOOL GotFirstConfig()
	{
		if (WaitForSingleObject(m_hGotFirstConfig, 0) == WAIT_OBJECT_0) {
			return TRUE;
		} else {
			return FALSE;
		}
...
protected:
	...
	void SendPacket(
		ClientPacket* packet, DWORD size, BOOL guaranteed, DWORD dwTimeout);

	BOOL IsValidPackSize(DWORD dwSize);

	// Arrays of cells and PlayerObjects (this consitutes the world DB), with
	// associated lock
