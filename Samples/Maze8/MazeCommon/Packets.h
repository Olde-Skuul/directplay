...
#define PACK_ARRAY_SIZE 10 // Set the number of elements in our pack array
						   // to 10.
...
enum {
	...
	PACKETTYPE_CLIENT_VERSION, // client responds to PACKETTYPE_SERVER_CONFIG w/
							   // this
	PACKETTYPE_SERVER_ACKVERSION, // server then responds to
								  // PACKETTYPE_CLIENT_VERSION w/ this and game
								  // begins
	PACKETTYPE_CLIENT_POS,        // sent to server as client moves
	PACKETTYPE_SERVER_ACKPOS      // sent to client as server acks the
								  // PACKETTYPE_CLIENT_POS packets
};
...
struct ClientPacket {
	ClientPacket();
	ClientPacket(WORD type): wType(type){};
...
struct ClientPosPacket: public ClientPacket {
	...
	ClientPosPacket(float x, float y, ANGLE cameraYaw):
		ClientPacket(PACKETTYPE_CLIENT_POS),
		fX(x),
		fY(y),
		aCameraYaw(cameraYaw){};
...
struct ClientNetConfig {
	...
	WORD wUpdateRate;
	WORD wTimeout;
	WORD wClientPackSizeArray[PACK_ARRAY_SIZE]; // Array of 10 custom sizes
												// passed in by user.
	WORD wServerPackSizeArray[PACK_ARRAY_SIZE]; // Array of 10 custom server
												// sizes.
...
struct ServerAckVersionPacket: public ServerPacket {
	...
	ServerAckVersionPacket(BOOL accepted, DWORD clientID):
		ServerPacket(PACKETTYPE_SERVER_ACKVERSION),
		bAccepted(accepted),
		dwClientID(clientID){};
...
struct ServerAckPacket: public ServerPacket {
	...
	WORD wPlayerStatePacketCount; // Count of following PlayerStatePacket
								  // structures
};
