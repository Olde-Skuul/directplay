...
void CMazeClient::Update(FLOAT fElapsed)
{
	...
	// See if it's time to send a packet to the server with our updated
	// coordinates
	...
	if ((fCurTime - m_fLastOutboundTime) * 1000.0f > m_NetConfig.wUpdateRate) {
		...
		if (m_NetConfig.wClientPackSizeArray[m_NetConfig.ubClientPackIndex] >
			0) {
			WORD wBufferSize = static_cast<WORD>(sizeof(packet) +
				m_NetConfig
					.wClientPackSizeArray[m_NetConfig.ubClientPackIndex]);
			VOID* pTempBuffer = 0;
			...
			if (NULL == pTempBuffer) {
				...
				DXTRACE_ERR_NOMSGBOX(
					TEXT("System out of Memory!"), E_OUTOFMEMORY);
				return;
			}
			...
			SendPacket((ClientPacket*)pTempBuffer, wBufferSize, FALSE,
				m_NetConfig.wTimeout);
...
void CMazeClient::DoManualPilot(FLOAT fElapsed)
{
	...
	if (pos.x >= m_Maze.GetWidth()) {
		pos.x = m_Maze.GetWidth() - 0.1f;
	}
	...
	if (pos.z >= m_Maze.GetHeight()) {
		pos.z = m_Maze.GetHeight() - 0.1f;
	}
...
void CMazeClient::DoAutopilot(FLOAT fElapsed)
{
	...
	while (fElapsed) {
		...
		if (m_aAutopilotTargetAngle != m_aCameraYaw) {
			SHORT diff = SHORT(
				(m_aAutopilotTargetAngle - m_aCameraYaw) & TRIG_ANGLE_MASK);
...
void CMazeClient::EngageAutopilot(BOOL bEngage)
{
	...
	// If we weren't on autopilot before and are are autopilot now then need to
	// init autopilot
	if (m_bAutopilot && !bPrevious) {
		...
		if (cellx >= int(m_Maze.GetWidth())) {
			m_vCameraPos.x = m_Maze.GetWidth() - 0.5f;
		}
		if (cellz < 0) {
			m_vCameraPos.z = 0.5f;
		}
		if (cellz >= int(m_Maze.GetHeight())) {
			m_vCameraPos.z = m_Maze.GetHeight() - 0.5f;
		}
...
void CMazeClient::PickAutopilotTarget()
{
	...
	// Figure out which directions are allowed. We're allowed to go in any
	// direction where there isn't a wall in the way and that takes us to a cell
	...
	if (!(cell & MAZE_WALL_NORTH) &&
		!m_AutopilotVisited[currentz - 1][currentx]) {
	...
	*/
	...
	if (dwAllowed == 0) {
		...
		if (m_AutopilotStack.GetCount() > 0) {
			...
			if (autocell.x < currentx) {
				m_aAutopilotTargetAngle = WEST_ANGLE;
			} else if (autocell.x > currentx) {
		...
		// If it's allowed to go forward, then have 1 in 2 chance of doing
		// that anyway, otherwise pick randomly from available alternatives
		if (bPossible && (rand() & 0x1000)) {
			m_aAutopilotTargetAngle = m_aCameraYaw;
		} else {
			m_aAutopilotTargetAngle =
				alloweddirs[(rand() % (dwAllowed << 3)) >> 3];
		}
...
HRESULT CMazeClient::OnPacket(DWORD /* dwFrom */, void* dwData, DWORD dwSize)
{
	...
	case PACKETTYPE_SERVER_CONFIG: {
		if (dwSize != sizeof(ServerConfigPacket)) {
			...
			m_pMazeApp->ConsolePrintf(LINE_LOG,
				TEXT("Disconnected because MazeClient is out of date."));
			m_pMazeApp->ConsolePrintf(
				LINE_LOG, TEXT("Please get updated version"));
			m_pMazeApp->ConsolePrintf(
				LINE_LOG, TEXT("from http://msdn.microsoft.com/directx/"));
			break;
		}
		...
		m_pMazeApp->ConsolePrintf(
			LINE_LOG, TEXT("Got MazeServer config settings"));
		m_pMazeApp->ConsolePrintf(LINE_LOG,
			TEXT("Maze Size=(%d,%d) ReliableRate=%d%%"),
		...
		m_pMazeApp->ConsolePrintf(
			LINE_LOG, TEXT("ThreadWait=%dms"), m_NetConfig.dwThreadWait);
		m_pMazeApp->ConsolePrintf(LINE_LOG,
			TEXT("ClientPackSize=%d ServerPackSize=%d (bytes)"),
		...
		{
			// The client expects the server to send a ServerConfigPacket packet
			// first, then the client sends a ClientVersionPacket, and then the
			// server sends a ServerAckVersionPacket packet and the game begins
	...
	case PACKETTYPE_SERVER_ACKVERSION: {
		...
		ServerAckVersionPacket* pAckVersionPacket =
			(ServerAckVersionPacket*)pPacket;
		...
		m_pMazeApp->ConsolePrintf(
			LINE_LOG, TEXT("Server assigned ID: 0x%0.8x"), m_dwLocalClientID);
		m_pMazeApp->ConsolePrintf(
			LINE_LOG, TEXT("Server accepted client version"));

		hr = m_Maze.Init(
			m_NetConfig.dwMazeWidth, m_NetConfig.dwMazeHeight, DEFAULT_SEED);
		if (FAILED(hr)) {
			DXTRACE_ERR(TEXT("Init"), hr);
		}

		// Seed the random number generator.
		dwSRand = (DWORD)(DXUtil_Timer(TIMER_GETABSOLUTETIME) *
	...
	default:
		m_pMazeApp->ConsolePrintf(LINE_LOG,
			TEXT("Received unknown %d byte packet from server"), dwSize);
		break;
	}

	// If the server has given us a custom wait time, Let's sleep for that
	// amount of time.
...
BOOL CMazeClient::IsValidPackSize(DWORD dwSize)
{
	...
	if (dwSize != m_NetConfig.wServerPackSizeArray[ubPackLocation]) {
		for (--ubPackLocation; ubPackLocation != m_NetConfig.ubServerPackIndex;
			 ubPackLocation--) {
			if (dwSize == m_NetConfig.wServerPackSizeArray[ubPackLocation]) {
				fFoundSize = TRUE;
				break;
...
// Desc:
//-----------------------------------------------------------------------------
void CMazeClient::SendPacket(
	ClientPacket* pPacket, DWORD dwSize, BOOL bGuaranteed, DWORD dwTimeout)
{
...
void CMazeClient::HandlePlayerObjectsInAckPacket(ServerAckPacket* pPacket)
{
	...
	for (DWORD count = pPacket->wPlayerStatePacketCount; count;
		 count--, pClientInfo++) {
		// Get the PlayerObject we think this is
		PlayerObject* pPlayerObject =
			&m_PlayerObjects[pClientInfo->dwID & PLAYER_OBJECT_SLOT_MASK];

		// Does the ID match the one we have?
		if (pPlayerObject->dwID != pClientInfo->dwID) {
			// No, so the PlayerObject we have needs to be deleted (server
			// reused the same slot number, so old PlayerObject must be toast)
			...
			if (newcellx != pPlayerObject->wCellX ||
				newcelly != pPlayerObject->wCellY) {
...
void CMazeClient::AddPlayerObjectToCells(PlayerObject* pPlayerObject)
{
	...
	if (FALSE ==
		IsPlayerObjectInCell(
			pPlayerObject->wCellX, pPlayerObject->wCellY, pPlayerObject)) {
		PlayerObject** ppCell =
			&m_pctCells[pPlayerObject->wCellY][pPlayerObject->wCellX];
...
void CMazeClient::RemovePlayerObjectFromCells(PlayerObject* pPlayerObject)
{
	...
	while (pCur) {
		if (pCur == pPlayerObject) {
			...
			// Found pPlayerObject, so remove pPlayerObject from the m_pctCells
			// linked list
			if (pPrev) {
...
PlayerObject* CMazeClient::GetFirstPlayerObjectInCell(DWORD x, DWORD z)
{
	...
	// Remove any PlayerObjects which are out of date (since they're probably
	// not really in this cell any more, but the server has just stopped telling
	// us about them)
	...
	while (pCur) {
		...
		if ((fCurTime - pCur->fLastValidTime) > 5.0f) {
			...
			// pPlayerObject is too old, so remove pPlayerObject from the
			// m_pctCells linked list
			if (pPrev) {
				pPrev->pNext = pPlayerObject->pNext;
			} else {
				*ppCell = pPlayerObject->pNext;
...
// Desc:
//-----------------------------------------------------------------------------
BOOL CMazeClient::IsPlayerObjectInCell(
	DWORD wCellX, DWORD wCellY, PlayerObject* pPlayerObject)
{
