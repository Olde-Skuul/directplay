...
int CMazeApp::Run(HINSTANCE hInst)
{
	...
	DWORD dwSRand = (DWORD)(DXUtil_Timer(TIMER_GETABSOLUTETIME) * UINT_MAX *
	...
	if (SUCCEEDED(m_pMazeGraphics->Create(hInst))) {
		...
		m_hOutputMsgThread = (HANDLE)_beginthreadex(
			NULL, 0, StaticOutputMsgThread, NULL, 0, &dwOutputMsgThreadID);
	...
	if (m_DP8Client.IsSessionLost()) {
		...
		if (FALSE == m_bLocalLoopbackInitDone) {
			if (m_bInitDone) {
				...
				if (m_DP8Client.GetSessionLostReason() ==
					DISCONNNECT_REASON_CLIENT_OUT_OF_DATE) {
					ConsolePrintf(LINE_LOG,
						TEXT(
							"Disconnected because MazeClient is out of date."));
					ConsolePrintf(LINE_LOG, TEXT("Please get updated version"));
					ConsolePrintf(LINE_LOG,
						TEXT("from http://msdn.microsoft.com/directx/"));
					m_bOutOfDateClient = TRUE;
				}

				// Disconnected, so retry in 10 seconds
				s_fStopEnumTime =
					fCurTime - m_Config.dwNetworkRetryDelay * 60.0f + 10.0f;
			} else {
		...
		if ((!s_fEnumStarted &&
					fCurTime - s_fStopEnumTime >
						m_Config.dwNetworkRetryDelay * 60.0f ||
				m_bConnectNow) &&
			...
			if (SUCCEEDED(hr = StartSessionEnum())) {
				...
				ConsolePrintf(
					LINE_LOG, TEXT("Starting DirectPlay host enumeration"));
				s_fStartEnumTime = fCurTime;
				s_fEnumStarted = TRUE;
			} else {
				// If we are in the stress connect, try the next connection.
				// When we get back to 1, then we have gone through the list, so
				// let's wait.
				if (m_Config.dwStressConnect > 1) {
					// We are going through the list of machines to attempt
					// connection. So lets try the next right away.
					ConsolePrintf(
						LINE_LOG, TEXT("Host not found. Trying local subnet."));
					m_bConnectNow = TRUE; // Try the enum right away with the
										  // next item in the list.
				} else {
					ConsolePrintf(LINE_LOG,
						TEXT("DirectPlay host enumeration failed to start."));
					ConsolePrintf(LINE_LOG,
						TEXT("Will try again in %d minutes."),
						m_Config.dwNetworkRetryDelay);
				}
		...
		if (s_fEnumStarted && fCurTime - s_fStartEnumTime > 5.0f * 60.0f) {
			...
			if (m_Config.dwStressConnect > 1) {
				...
				ConsolePrintf(
					LINE_LOG, TEXT("Host not found. Trying local subnet."));
				m_bConnectNow = TRUE; // Try the enum right away with the next
									  // item in the list.
			} else {
				ConsolePrintf(LINE_LOG,
					TEXT(
						"No host found. Stopping DirectPlay host enumeration"));
				ConsolePrintf(LINE_LOG, TEXT("Will try again in %d minutes."),
					m_Config.dwNetworkRetryDelay);
			}
			// Stop enumeration
		...
		if (s_fEnumStarted && fCurTime - s_fLastConnect > 0.5f) {
			if (TRUE == TryToConnect()) {
				// Connect successful
				ConsolePrintf(LINE_LOG,
					TEXT("Connected to server.  Host enumeration stopped."));
		...
		if (m_Config.dwLogLevel > 1) {
			...
			if (fCurTime - fLastPosUpdate > 10.0f) {
				...
				ConsolePrintf(LINE_LOG,
					TEXT(
						"Position: (%5.1f,%5.1f), Players: %d, Nearby Players: %d"),
		...
		if (m_Config.dwAutoPrintStats > 0 &&
			...
			ConsolePrintf(LINE_LOG,
				...
				m_MazeClient.GetLocalClientID());
			ConsolePrintf(LINE_LOG,
				TEXT(
					"(Key: G=Guaranteed NG=Non-Guaranteed B=Bytes P=Packets)"));
		...
		if (m_Config.bAutoDisconnnect) {
			...
			if (fDisconnectCountdown < 0.0f) {
				...
				ConsolePrintf(LINE_LOG,
					TEXT(
						"Intentional disconnect.  Connecting again for %0.0f seconds..."),
					fDisconnectCountdown);
				m_DP8Client.Shutdown();
				m_MazeClient.Shutdown();
			}
		}
...
HRESULT CMazeApp::StartSessionEnum()
{
	...
	if (m_Config.bConnectToMicrosoftSite) {
		ConsolePrintf(
			LINE_LOG, TEXT("Connecting to DirectPlayMaze.rte.microsoft.com"));
		hr = m_DP8Client.StartSessionEnum(MICROSOFT_SERVER);
	} else if (m_Config.bConnectToLocalServer) {
		ConsolePrintf(LINE_LOG,
			TEXT("Connecting to local server (searches the local subnet)"));
		hr = m_DP8Client.StartSessionEnum(TEXT(""));
	} else if (m_Config.bConnectToRemoteServer) {
		ConsolePrintf(LINE_LOG, TEXT("Connecting to remote server at '%s'"),
			m_Config.szIPAddress);
		hr = m_DP8Client.StartSessionEnum(m_Config.szIPAddress);
	}
	// If users wants Stress Connect, go through the sequence they have
	// requested.
...
#define LOOPBACK_MAZE_HEIGHT 16
	...
	if (FAILED(hr = m_MazeClient.m_Maze.Init(
				   LOOPBACK_MAZE_WIDTH, LOOPBACK_MAZE_HEIGHT, DEFAULT_SEED))) {
...
#define MAX_LOG_LINES 100
	...
	for (;;) {
		...
		while (m_lQueueSize > 0 && dwNumProcessed < 5) {
			...
			case LINE_LOG: {
				// Add m_szOutputMsgBuffer[m_dwNextOutputMsg] to szLogBuffer
				// array, and redisplay the array on the top half of the screen
				_stprintf(szLogBuffer, TEXT("%s %s"), m_strTimeStamp,
					m_szOutputMsgBuffer[m_dwNextOutputMsg]);
				...
				if (m_hLogFile) {
					...
					WriteFile(m_hLogFile, szLogBuffer,
						static_cast<DWORD>(lstrlen(szLogBuffer)), &dwWritten,
						NULL);
					TCHAR strEOL = TEXT('\r');
					WriteFile(
						m_hLogFile, &strEOL, sizeof(TCHAR), &dwWritten, NULL);
					strEOL = TEXT('\n');
					WriteFile(
						m_hLogFile, &strEOL, sizeof(TCHAR), &dwWritten, NULL);
			...
			m_pMazeGraphics->HandleOutputMsg(
				m_EnumLineType[m_dwNextOutputMsg], szLogBuffer);
...
void CMazeApp::ConsolePrintf(EnumLineType enumLineType, const TCHAR* fmt, ...)
{
	...
	if (m_lQueueSize != MAX_OUTPUT_QUEUE) {
		...
		_vstprintf(m_szOutputMsgBuffer[m_dwNextFreeOutputMsg], fmt, args);
...
void CMazeApp::CreateTempLogFile()
{
	...
	SYSTEMTIME sysTime;
	GetLocalTime(&sysTime);
	_stprintf(strTime, TEXT("client-%04d-%02d-%02d-"), sysTime.wYear,
	...
	m_hLogFile = CreateFile(strTempFileName, GENERIC_WRITE, FILE_SHARE_READ,
		NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
	if (m_hLogFile == INVALID_HANDLE_VALUE) {
		ConsolePrintf(LINE_LOG, TEXT("Could not create create temp file '%s'"),
			strTempFileName);
		goto LFail;
	}
...
void CMazeApp::ReadConfig()
{
	...
	DXUtil_ReadBoolRegKey(hKey, TEXT("ConnectToMicrosoftSite"),
		&m_Config.bConnectToMicrosoftSite, TRUE);
	DXUtil_ReadBoolRegKey(hKey, TEXT("ConnectToLocalServer"),
		&m_Config.bConnectToLocalServer, FALSE);
	DXUtil_ReadBoolRegKey(hKey, TEXT("ConnectToRemoteServer"),
		&m_Config.bConnectToRemoteServer, FALSE);
	DXUtil_ReadIntRegKey(
		hKey, TEXT("NetworkRetryDelay"), &m_Config.dwNetworkRetryDelay, 30);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("FileLogging"), &m_Config.bFileLogging, TRUE);
	DXUtil_ReadStringRegKey(hKey, TEXT("IPAddress"), m_Config.szIPAddress,
		sizeof(m_Config.szIPAddress), TEXT("\0"));

	DXUtil_ReadBoolRegKey(
		hKey, TEXT("ShowFramerate"), &m_Config.bShowFramerate, TRUE);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("ShowIndicators"), &m_Config.bShowIndicators, TRUE);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("DrawMiniMap"), &m_Config.bDrawMiniMap, TRUE);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("FullScreen"), &m_Config.bFullScreen, TRUE);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("Reflections"), &m_Config.bReflections, FALSE);

	DXUtil_ReadBoolRegKey(
		hKey, TEXT("AutoDisconnnect"), &m_Config.bAutoDisconnnect, FALSE);
	DXUtil_ReadBoolRegKey(
		hKey, TEXT("AutoConnnect"), &m_Config.bAutoConnnect, FALSE);
	DXUtil_ReadIntRegKey(hKey, TEXT("LogLevel"), &m_Config.dwLogLevel, 2);
	DXUtil_ReadIntRegKey(
		hKey, TEXT("AutoPrintStats"), &m_Config.dwAutoPrintStats, 10);
...
void CMazeApp::WriteConfig()
{
	...
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("ConnectToMicrosoftSite"), m_Config.bConnectToMicrosoftSite);
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("ConnectToLocalServer"), m_Config.bConnectToLocalServer);
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("ConnectToRemoteServer"), m_Config.bConnectToRemoteServer);
	DXUtil_WriteIntRegKey(
		hKey, TEXT("NetworkRetryDelay"), m_Config.dwNetworkRetryDelay);
	...
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("ShowFramerate"), m_Config.bShowFramerate);
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("ShowIndicators"), m_Config.bShowIndicators);
	...
	DXUtil_WriteBoolRegKey(
		hKey, TEXT("AutoDisconnnect"), m_Config.bAutoDisconnnect);
	...
	DXUtil_WriteIntRegKey(
		hKey, TEXT("AutoPrintStats"), m_Config.dwAutoPrintStats);
