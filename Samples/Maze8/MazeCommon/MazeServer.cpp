...
HRESULT CMazeServer::Init(BOOL bLocalLoopback, const CMaze* pMaze)
{
	...
	// Validate size. Must be a power-of-2 times LOCK_GRID_SIZE. Compute the
	// shifts.
	if (m_dwWidth > SERVER_MAX_WIDTH || m_dwHeight > SERVER_MAX_HEIGHT) {
		return DXTRACE_ERR(
			TEXT("Maze height and width need to be less than 128"),
			E_INVALIDARG);
	}
	if ((m_dwWidth % LOCK_GRID_SIZE) != 0 ||
		(m_dwHeight % LOCK_GRID_SIZE) != 0) {
		return DXTRACE_ERR(
			TEXT("Maze height and width need to be divisable by 16"),
			E_INVALIDARG);
	}
	...
	if (((DWORD(LOCK_GRID_SIZE) << m_dwMazeXShift) != m_dwWidth) ||
		...
		return DXTRACE_ERR(
			TEXT("Maze height and width need to be power of 2"), E_INVALIDARG);
	}
	...
	m_PlayerDatas[MAX_PLAYER_OBJECTS - 1].pPrevious =
		&m_PlayerDatas[MAX_PLAYER_OBJECTS - 2];
...
void CMazeServer::OnAddConnection(DWORD id)
{
	...
	if (m_dwLogLevel > 0) {
		...
		ConsolePrintf(
			SLINE_LOG, TEXT("Players connected = %d"), m_dwPlayerCount);
	}
	...
	if (pPlayerData == NULL) {
		ConsolePrintf(SLINE_LOG,
			TEXT("ERROR! Unable to create new PlayerData for client!"));
...
HRESULT CMazeServer::OnPacket(DWORD dwFrom, void* pData, DWORD size)
{
	...
	case PACKETTYPE_CLIENT_VERSION:
		if (size == sizeof(ClientVersionPacket)) {
			HandleClientVersionPacket(
				dwFrom, (ClientVersionPacket*)pClientPack);
		} else {
			m_pNet->RejectClient(dwFrom, DISCONNNECT_REASON_CLIENT_OUT_OF_DATE);
		}
		break;
	...
	case PACKETTYPE_SERVER_CONFIG:
		...
		// Server config packet sent to all users (including server). Just
		// ignore and continue.
	...
	FLOAT fDiffTime =
		(DXUtil_Timer(TIMER_GETAPPTIME) - fStartTime) - m_fAvgThreadTime;
...
BOOL CMazeServer::IsValidPackSize(DWORD dwSize)
{
	...
	if (dwSize != m_ClientNetConfig.wClientPackSizeArray[ubPackLocation]) {
		for (--ubPackLocation;
			 ubPackLocation != m_ClientNetConfig.ubClientPackIndex;
			 ubPackLocation--) {
			if (dwSize ==
				m_ClientNetConfig.wClientPackSizeArray[ubPackLocation]) {
				// Found valid size in the array.
				fFoundSize = TRUE;
				break;
			}
			if (ubPackLocation >= PACK_ARRAY_SIZE) {
				ubPackLocation = PACK_ARRAY_SIZE; // Wrap the array.
			}
		}
...
PlayerData* CMazeServer::CreatePlayerData()
{
	...
	if (pPlayerData) {
		...
		pPlayerData->dwID = (DWORD)((pPlayerData - m_PlayerDatas) |
			(m_dwPlayerDataUniqueValue << PLAYER_OBJECT_SLOT_BITS));
...
// Desc:
//-----------------------------------------------------------------------------
void CMazeServer::HandleClientPosPacket(
	DWORD dwFrom, ClientPosPacket* pClientPosPack)
{
	...
	if (pFromPlayer == NULL) {
		if (m_dwLogLevel > 1) {
			ConsolePrintf(SLINE_LOG,
				TEXT(
					"DPNID %0.8x: Could not find data structure for this player"),
				pFromPlayer->NetID);
		}
		return;
	}
	...
	if (FALSE == pFromPlayer->bAllow) {
		if (m_dwLogLevel > 0) {
			ConsolePrintf(SLINE_LOG,
				TEXT(
					"DPNID %0.8x: Got position packet from bad client.  Rejecting client"),
				pFromPlayer->NetID);
		}
		m_pNet->RejectClient(dwFrom, DISCONNNECT_REASON_CLIENT_OUT_OF_DATE);
	...
	DWORD newcellx = static_cast<DWORD>(int(pClientPosPack->fX));
	DWORD newcelly = static_cast<DWORD>(int(pClientPosPack->fY));
	...
	if (newcellx != oldcellx || newcelly != oldcelly) {
		...
		pFromPlayer->wCellX = WORD(newcellx);
		pFromPlayer->wCellY = WORD(newcelly);
	...
	dwAllocSize = sizeof(ServerAckPacket) +
		dwMaxPlayerStatePackets * sizeof(PlayerStatePacket);
	...
	*pSvrAckPack = ServerAckPacket(m_dwPlayerCount);
		...
		for (DWORD x = minx; x <= maxx; x++) {
			...
			while (pCurPlayerData) {
				if (pCurPlayerData != pFromPlayer) {
					if (pSvrAckPack->wPlayerStatePacketCount >=
						dwMaxPlayerStatePackets) {
						// Make sure pChunk is where we think it is
						assert((BYTE*)pChunk ==
							(BYTE*)((BYTE*)pSvrAckPack +
								sizeof(ServerAckPacket) +
								pSvrAckPack->wPlayerStatePacketCount *
									sizeof(PlayerStatePacket)));

						// There are more than just 4 new nearby players, so
						// resize the buffer pSvrAckPack to allow 16 more
						// PlayerStatePacket's.
						...
						pNewSvrAckPack =
							(ServerAckPacket*)realloc(pSvrAckPack, dwAllocSize);
						...
						pChunk =
							(PlayerStatePacket*)((BYTE*)((BYTE*)pSvrAckPack +
								sizeof(ServerAckPacket) +
								pSvrAckPack->wPlayerStatePacketCount *
									sizeof(PlayerStatePacket)));
	...
	if (m_dwLogLevel > 2) {
		ConsolePrintf(SLINE_LOG, TEXT("DPNID %0.8x: Position is (%0.2f,%0.2f)"),
			pFromPlayer->NetID, pFromPlayer->fPosX, pFromPlayer->fPosY);
	} else if (m_dwLogLevel == 2) {
		FLOAT fTime = DXUtil_Timer(TIMER_GETAPPTIME);
		if (fTime - pFromPlayer->fLastDisplayTime > 60.0f) {
			ConsolePrintf(SLINE_LOG,
				TEXT("DPNID %0.8x: Position is (%0.2f,%0.2f)"),
				pFromPlayer->NetID, pFromPlayer->fPosX, pFromPlayer->fPosY);
	...
	DWORD acksize = sizeof(ServerAckPacket) +
		(pSvrAckPack->wPlayerStatePacketCount * sizeof(PlayerStatePacket));

	// Pack the buffer with dummy data.
	if (m_ClientNetConfig
			.wServerPackSizeArray[m_ClientNetConfig.ubServerPackIndex] > 0) {
		DWORD dwBufferSize = acksize +
			m_ClientNetConfig
				.wServerPackSizeArray[m_ClientNetConfig.ubServerPackIndex];
		VOID* pTempBuffer = 0;
...
// Desc:
//-----------------------------------------------------------------------------
void CMazeServer::HandleClientVersionPacket(
	DWORD dwFrom, ClientVersionPacket* pClientVersionPack)
{
	...
	if (m_bLocalLoopback) {
		...
		pPlayerData->bAllow =
			IsClientVersionSupported(pClientVersionPack->dwVersion);
	}

	if (m_dwLogLevel > 0) {
		ConsolePrintf(SLINE_LOG, TEXT("DPNID %0.8x: Client version=%d (%s)"),
			pPlayerData->NetID, pPlayerData->dwVersion,
			pPlayerData->bAllow ? TEXT("Accepted") : TEXT("Rejected"));
	}

	if (FALSE == pPlayerData->bAllow) {
		if (m_dwLogLevel > 0) {
			ConsolePrintf(SLINE_LOG, TEXT("DPNID %0.8x: Rejecting client"),
				pPlayerData->NetID);
		}
	...
	// Send acknowledgement to client that the client was either accepted or
	// rejected
...
// Desc:
//-----------------------------------------------------------------------------
void CMazeServer::HandleUnknownPacket(
	DWORD dwFrom, ClientPacket* /* pClientPack */, DWORD size)
{
	if (m_dwLogLevel > 1) {
		ConsolePrintf(SLINE_LOG,
			TEXT("ERROR! Unknown %d byte packet from player %0.8x"), size,
			dwFrom);
	}
...
{
	DWORD hash = ((id) + (id >> 8) + (id >> 16) + (id >> 24)) &
		(NUM_ID_HASH_BUCKETS - 1);
	return hash;
}
...
void CMazeServer::RemovePlayerDataID(PlayerData* pPlayerData)
{
	...
	const DWORD buckets_per_lock =
		NUM_ID_HASH_BUCKETS / NUM_ID_HASH_BUCKET_LOCKS;
...
{
	if (m_pNet) {
		// Find the player that was displayed the longest time ago, and display
		// it.
		...
		if (pOldestPlayerData) {
			ConsolePrintf(
				SLINE_LOG, TEXT("Displaying connection info for next player"));
...
void CMazeServer::PrintStats()
{
	...
	ConsolePrintf(SLINE_LOG,
		TEXT("Players online (not including server player): %d"),
		m_dwPlayerCount);
	ConsolePrintf(
		SLINE_LOG, TEXT("Peak player count: %d"), m_dwPeakPlayerCount);
}
...
void CMazeServer::DisplayConnectionInfo(DWORD dwID)
{
	...
	ConsolePrintf(
		SLINE_LOG, TEXT("Displaying connection info for %0.8x"), dwID);
	ConsolePrintf(SLINE_LOG,
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CMazeServer::SendPacket(
	DWORD to, void* pData, DWORD size, BOOL reliable, DWORD dwTimeout)
{
...
void CMazeServer::SendConfigPacketToAll(ServerConfigPacket* pPacket)
{
	...
	if (m_pNet) {
		...
		SendPacket(DPNID_ALL_PLAYERS_GROUP, pPacket, sizeof(ServerConfigPacket),
			TRUE, 0);
	}
}
...
void CMazeServer::SetClientPackSize(DWORD size)
{
	...
	m_ClientNetConfig
		.ubClientPackIndex++; // Increase index and verify location in array.
	if (m_ClientNetConfig.ubClientPackIndex >= PACK_ARRAY_SIZE) {
		m_ClientNetConfig.ubClientPackIndex = 0;
	}

	m_ClientNetConfig
		.wClientPackSizeArray[m_ClientNetConfig.ubClientPackIndex] = WORD(size);
	ServerConfigPacket packet(m_ClientNetConfig);
	m_ClientNetConfigLock.Leave();
...
void CMazeServer::SetServerPackSize(DWORD size)
{
	...
	m_ClientNetConfig
		.ubServerPackIndex++; // Increase index and verify location in array.
	if (m_ClientNetConfig.ubServerPackIndex >= PACK_ARRAY_SIZE) {
		m_ClientNetConfig.ubServerPackIndex = 0;
	}

	m_ClientNetConfig
		.wServerPackSizeArray[m_ClientNetConfig.ubServerPackIndex] = WORD(size);
	ServerConfigPacket packet(m_ClientNetConfig);
	m_ClientNetConfigLock.Leave();
