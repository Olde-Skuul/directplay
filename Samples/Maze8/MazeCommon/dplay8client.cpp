...
HRESULT CDPlay8Client::Init(MazeConfig* pMazeConfig)
{
	...
	if (FAILED(hr = CoCreateInstance(CLSID_DirectPlay8Client, NULL, CLSCTX_ALL,
				   IID_IDirectPlay8Client, (LPVOID*)&m_pDPlay))) {
	...
	if ((m_MazeConfig->dwSPThreads >= MIN_SP_THREADS) &&
		(m_MazeConfig->dwSPThreads <= MAX_SP_THREADS)) {
...
HRESULT CDPlay8Client::StartSessionEnum(const TCHAR* ipaddress)
{
	...
	if (FAILED(hr = CoCreateInstance(CLSID_DirectPlay8Address, NULL, CLSCTX_ALL,
	...
	hr = pDP8AddressHost->AddComponent(
		DPNA_KEY_PORT, &dwPort, sizeof(dwPort), DPNA_DATATYPE_DWORD);
	if (FAILED(hr)) {
	...
	if (ipaddress != NULL && ipaddress[0] != 0) {
		...
		hr = pDP8AddressHost->AddComponent(DPNA_KEY_HOSTNAME, wszHostName,
			(wcslen(wszHostName) + 1) * sizeof(WCHAR), DPNA_DATATYPE_STRING);
		if (FAILED(hr)) {
			DXTRACE_ERR(TEXT("AddComponent"), hr);
			goto LCleanup;
		}
	...
	hr = m_pDPlay->EnumHosts(&dpnAppDesc, pDP8AddressHost, pDP8AddressLocal,
		NULL, 0, INFINITE, 0, INFINITE, NULL, &m_dpnhEnum, 0);
	...
	IDirectPlay8Address* pDP8AddressDevice)
{
	...
	if (m_dwNumSessions < MAX_SESSIONS) {
		...
		// the current entry.
		DWORD dwIndex;
		for (dwIndex = 0; dwIndex < m_dwNumSessions; dwIndex++) {
		...
		SAFE_RELEASE(m_pHostAddresses[dwIndex]);
		pDP8AddressHost->QueryInterface(
			IID_IDirectPlay8Address, (LPVOID*)&m_pHostAddresses[dwIndex]);

		// Copy pDP8AddressDevice to m_pDeviceAddresses[dwIndex]
		SAFE_RELEASE(m_pDeviceAddresses[dwIndex]);
		pDP8AddressDevice->QueryInterface(
			IID_IDirectPlay8Address, (LPVOID*)&m_pDeviceAddresses[dwIndex]);

		if (pdesc->pwszSessionName != NULL) {
			DXUtil_ConvertWideStringToGeneric(
				m_szSessionNames[dwIndex], pdesc->pwszSessionName);
		} else {
...
HRESULT CDPlay8Client::JoinSession(DWORD num)
{
	...
	if (FAILED(
			hr = m_pDPlay->Connect(&dpnAppDesc, // Application description
				pHostAddress,                   // Session host address
				pDeviceAddress, // Address of device used to connect to the host
				NULL,
				NULL,    // Security descriptions & credientials (MBZ in DPlay8)
				NULL, 0, // User data & its size
				NULL,    // Asynchronous connection context (returned with
						 // DPNMSG_CONNECT_COMPLETE in async handshaking)
				NULL,    // Asynchronous connection handle (used to cancel
						 // connection process)
				DPNOP_SYNC))) // Connect synchronously
	{
		if (hr == DPNERR_NORESPONSE || hr == DPNERR_ABORTED) {
			goto LCleanup; // These are possible if the server exits while
						   // joining
		}

		if (hr == DPNERR_INVALIDINSTANCE) {
			goto LCleanup; // This is possible if the original server exits and
						   // another server comes online while we are
						   // connecting
		}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDPlay8Client::SendPacket(
	void* pData, DWORD dwSize, BOOL bGuaranteed, DWORD dwTimeout)
{
	if (NULL == m_pDPlay) {
		return S_OK;
	}
	...
	// Must pass in a value for the Asyn handle. Will be thrown when proc
	// completes.
	phAsync = &hAsync;
	...
	m_pDPlay->Send(&dpnBufferDesc, 1, dwTimeout, NULL, phAsync, dwFlags);
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDPlay8Client::StaticReceiveHandler(
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CDPlay8Client::ReceiveHandler(
	void* /* pvContext */, DWORD dwMessageType, void* pvMessage)
{
	...
	case DPN_MSGID_RECEIVE: {
		...
		if (m_pClient != NULL) {
			m_pClient->OnPacket(pRecvData->dpnidSender, pRecvData->pReceiveData,
	...
	case DPN_MSGID_TERMINATE_SESSION: {
		m_dwSessionLostReason = DISCONNNECT_REASON_UNKNOWN;
		PDPNMSG_TERMINATE_SESSION pTermMsg =
			(PDPNMSG_TERMINATE_SESSION)pvMessage;
	...
	case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
		PDPNMSG_ENUM_HOSTS_RESPONSE pEnumResponse =
			(PDPNMSG_ENUM_HOSTS_RESPONSE)pvMessage;

		EnumSessionCallback(pEnumResponse->pApplicationDescription,
			pEnumResponse->pAddressSender, pEnumResponse->pAddressDevice);
		break;
	}
	}
...
DWORD CDPlay8Client::GetThroughputBPS()
{
	...
	if (fCurTime - s_fLastThroughputBPSTime > 1.0f) {
		m_fThroughputBPS =
			(float)m_dwThroughputBytes / (fCurTime - s_fLastThroughputBPSTime);
