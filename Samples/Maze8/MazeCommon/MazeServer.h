...
public:
	...
	// Change of client sending packets via reliable transport. Setting this
	// causes the server to propagate this setting to all currently connected
	// clients
	...
	// Change client update rate. Setting this causes the server to propagate
	// this setting to all currently connected clients
	...
	// Change client timeout. Setting this causes the server to propagate this
	// setting to all currently connected clients
	...
	// Change Client position pack size. Setting this causes the server to
	// propagate this setting to all currently connected clients
	void SetClientPackSize(DWORD size);
	DWORD GetClientPackSize() const
	{
		return DWORD(
			m_ClientNetConfig
				.wClientPackSizeArray[m_ClientNetConfig.ubClientPackIndex]);
	}
...
protected:
	...
	// A fixed sized grid of locks which we lay over the maze to control access
	// to it We demand that the maze dimensions are a power-of-2 times the
	// dimensions of this grid, and pre-store that power to allow fast
	// translation
	...
	{
		m_PlayerDataLocks[((pPlayerData - m_PlayerDatas) &
							  (NUM_PLAYER_OBJECT_LOCKS - 1))]
			.Enter();
	}
	...
	// Unsafe versions of add/remove. Must have playerdata and cell locked when
	// you call this
	...
	void HandleClientVersionPacket(
	...
	// Send packet wrapper
	HRESULT SendPacket(
		DWORD dwTo, void* pData, DWORD dwSize, BOOL bReliable, DWORD dwTimeout);
...
//       In the client it should probably just be a dummy function, in the
//       server it should probably just spew out to the console
//-----------------------------------------------------------------------------
