...
HRESULT CMaze::Init(DWORD dwWidth, DWORD dwHeight, DWORD dwSeed)
{
	...
	if (m_dwSize == 0) {
		hr = E_INVALIDARG;
		DXTRACE_ERR(TEXT("Maze height and width need must be greater than 0"),
			E_INVALIDARG);
		goto LFail;
	}
	...
	if (m_dwWidth > SERVER_MAX_WIDTH || m_dwHeight > SERVER_MAX_HEIGHT) {
		hr = E_INVALIDARG;
		DXTRACE_ERR(
			TEXT("Maze height and width must be less than 128"), E_INVALIDARG);
		goto LFail;
	}
	...
	if ((m_dwWidth % LOCK_GRID_SIZE) != 0 ||
		...
		hr = E_INVALIDARG;
		DXTRACE_ERR(TEXT("Maze height and width need to be divisable by 16"),
			E_INVALIDARG);
		goto LFail;
	}
	...
	// Okay, now we're going to generate the maze. We use Kruskal's algorithm,
	// which works by walking through the list of walls in a random order,
	// removing a wall if it would connect two previously (path-)disconnected
	// cells. This guarantees a fully connected maze (i.e. you can reach any
	// cell from any other).
	...
	dwNumWalls =
		((m_dwWidth - 1) * m_dwHeight) + ((m_dwHeight - 1) * m_dwWidth);
	...
	for (i = 0; i < dwNumWalls; i++, pWallNode++) {
		...
		if (pCellA->pPartition != pCellB->pPartition) {
			// Nope, so let's take out that wall. First, connect the partition
			// lists
			while (pCellA->pNext) {
				pCellA = pCellA->pNext;
			}
	...
	float fov, MazeCellRef* plist, DWORD maxlist)
{
	...
	if (pos.x < 0.0f || pos.y < 0.0f || pos.x >= float(m_dwWidth) ||
		pos.y >= float(m_dwHeight)) {
		return 0;
	}
	...
	state.dwMinX =
		(state.dwPosX > m_dwMaxView) ? state.dwPosX - m_dwMaxView : 0;
	state.dwMaxX = ((state.dwPosX + m_dwMaxView) > m_dwWidth) ?
        m_dwWidth :
        state.dwPosX + m_dwMaxView;
	state.dwMinY =
		(state.dwPosY > m_dwMaxView) ? state.dwPosY - m_dwMaxView : 0;
	state.dwMaxY = ((state.dwPosY + m_dwMaxView) > m_dwHeight) ?
        m_dwHeight :
        state.dwPosY + m_dwMaxView;
...
// Desc:
//-----------------------------------------------------------------------------
void CMaze::RecurseCheckCellVis(
	VisState& state, DWORD x, DWORD y, D3DXVECTOR2 left, D3DXVECTOR2 right)
{
	...
	if (x < state.dwMinX || x > state.dwMaxX || y < state.dwMinY ||
		y > state.dwMaxY) {
		return;
	}

	// If cell is already marked, then we don't visit it either
	if (state.pArray[x - state.dwMinX +
			((y - state.dwMinY) * state.dwArrayPitch)]) {
		return;
	}

	// Mark cell as visited
	state.pArray[x - state.dwMinX + ((y - state.dwMinY) * state.dwArrayPitch)] =
		1;
	...
	// If there is an edge which clips all points, then the cell isn't in
	// frustrum
...
{
	BYTE flag = static_cast<BYTE>((D3DXVec2Dot(&offset, &dir) >= 0) ? 0 : 1);
