...
// Desc:
//-----------------------------------------------------------------------------
static HRESULT ParseCommandLine(int argc, TCHAR* argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (strArg[0] == '/' || strArg[0] == '-') {
			...
			if (_tcsnicmp(strArg + 1, TEXT("SIZE:"), 4) == 0) {
				TCHAR* strHeight =
					const_cast<TCHAR*>(_tcsrchr(strArg + 6, ','));
				if (strHeight) {
					// Change the value of the ',' to NULL, then move to next
					// val.
					strHeight[0] = '\0';
					strHeight++;

					// Assign the argument to our global variables.
					g_dwWidth = static_cast<DWORD>(_ttoi(strArg + 6));
					g_dwHeight = static_cast<DWORD>(_ttoi(strHeight));

					// Do some simple validation on the width and height.
					if (!g_dwWidth || (g_dwWidth % LOCK_GRID_SIZE) ||
						(g_dwWidth > SERVER_MAX_WIDTH)) {
						printf("Invalid Width Value!\n");
						printf(
							"  Width Value must be a power of 2 from  %d to %d.\n",
							LOCK_GRID_SIZE, SERVER_MAX_WIDTH);
						printf(
							"  Format: MazeServer.exe [/size:width,height]\n\n");

						hr = E_INVALIDARG;
					}
					// Simple validation on the height
					else if (!g_dwHeight || (g_dwHeight % LOCK_GRID_SIZE) ||
						(g_dwHeight > SERVER_MAX_HEIGHT)) {
						printf("Invalid Height Value!\n");
						printf(
							"  Height Value must be a power of 2 from  %d to %d.\n",
			...
			// User wants to enter a custom number of SP Threads in the Server
				...
				g_dwNumSPThreads = static_cast<DWORD>(_ttoi(strArg + 11));

				// Do some simple validation on our computed Value.
				if ((!g_dwNumSPThreads) ||
					(g_dwNumSPThreads < MIN_SP_THREADS) ||
					(g_dwNumSPThreads > MAX_SP_THREADS)) {
			...
			// User wants to enter a custom SP Buffer Size on the Server
				...
				g_dwSPBufferSize = static_cast<DWORD>(_ttoi(strArg + 10));
			...
			// Flag to set the size of the client buffer pack.
				...
				DWORD packsize = static_cast<DWORD>(_ttoi(strArg + 16));
				...
				if (packsize > MAX_PACK_SIZE) {
					...
					printf(
						"  Client Buffer Pack Size must be from 0 bytes to %d bytes.\n",
						MAX_PACK_SIZE);
					printf(
						"  Format: MazeServer.exe [/ClientPackSize:num]\n\n");
			...
			// Enter a custom memory pack to add
				...
				DWORD packsize = static_cast<DWORD>(
					_ttoi(strArg + 16)); // Points to the number of bytes the
				...
				if (packsize > MAX_PACK_SIZE) {
					...
					printf(
						"  Server Buffer Pack Size must be from 0 bytes to %d bytes.\n",
						MAX_PACK_SIZE);
					printf(
						"  Format: MazeServer.exe [/ServerPackSize:num]\n\n");

					hr = E_INVALIDARG;
				}
			}
			// Flag to set the size of the client buffer pack.
			else if (_tcsnicmp(strArg + 1, TEXT("ClientThreadWait:"), 14) ==
				0) {
				DWORD threadwait = static_cast<DWORD>(_ttoi(strArg + 18));
				...
				if (threadwait > MAX_THREAD_WAIT) {
					...
					printf(
						"  Client Thread Wait Time must be from 0 bytes to %d ms.\n",
						MAX_THREAD_WAIT);
					printf(
						"  Format: MazeServer.exe [/ClientThreadWait:num]\n\n");

					hr = E_INVALIDARG;
				}
			}
			// Enter a custom memory pack to add
			else if (_tcsnicmp(strArg + 1, TEXT("ServerThreadWait:"), 14) ==
				0) {
				DWORD threadwait = static_cast<DWORD>(
					_ttoi(strArg + 18)); // Points to the number of bytes the
				...
				if (threadwait > MAX_THREAD_WAIT) {
					...
					printf(
						"  Server Thread Wait Time must be from 0 ms to %d ms.\n",
						MAX_THREAD_WAIT);
					printf(
						"  Format: MazeServer.exe [/ServerThreadWait:num]\n\n");
				...
				printf("MazeServer\n");
				printf(
					"  Format: MazeServer.exe [/SIZE:width,height] [/SPThreads:num] [/ServerPackSize:num] [/ClientPackSize:num]\n");
				printf("\n");
				printf("  Options:\n");
				printf(
					"       /SIZE:x,y : Sets the Width and Height of the maze. Power of 2 from %d to %d!\n",
					LOCK_GRID_SIZE, SERVER_MAX_HEIGHT);
				printf(
					"       /SPThreads:N : Sets SP threads to N. Range from %d to %d. \n",
					MIN_SP_THREADS, MAX_SP_THREADS);
				printf(
					"       /SPBuffer:N  : Sets SP Buffer to N. Range from 0 to %d. \n",
					MAX_SP_BUFFER);
				printf(
					"       /ClientPackSize : Custom client packet buffer. Range from 0 to %d. \n",
					MAX_PACK_SIZE);
				printf(
					"       /ServerPackSize : Custom server packet buffer. Range from 0 to %d. \n",
					MAX_PACK_SIZE);
				printf(
					"       /ClientThreadWait : Custom client thread wait time. Range from 0 to %d ms. \n",
					MAX_THREAD_WAIT);
				printf(
					"       /ServerThreadWait : Custom server thread wait time. Range from 0 to %d ms. \n",
					MAX_THREAD_WAIT);
				printf("\n");
				printf("  Examples: \n");
				...
				// They just wanted info, fail the condition so as to not start
				// the server.
				hr = E_INVALIDARG;
			}
		}
...
void ConsolePrintf(ServerBufferType EnumLineType, const TCHAR* fmt, ...)
{
	...
	if (g_dwQueueSize != MAX_OUTPUT_QUEUE) {
		...
		_vstprintf(g_szOutputBuffer[g_dwNextFreeOutput], fmt, args);
...
// Desc:
//-----------------------------------------------------------------------------
static UINT WINAPI UpdateThread(LPVOID /* pParam */)
{
	for (;;) {
		if (g_bQuitThread) {
			break;
		...
		if (g_fNextCIRate != 0.0f &&
			fCurTime - s_fLastStatsDisplay > g_fNextCIRate * 60.0f) {
			g_MazeServer.PrintStats();
			s_fLastStatsDisplay = fCurTime;
		}

		if (g_fNextCIRate != 0.0f &&
			fCurTime - s_fLastCIDisplay > g_fNextCIRate * 60.0f) {
...
// Desc:
//-----------------------------------------------------------------------------
static VOID SetupConsole(DWORD dwWindowSizeY)
{
	static TCHAR strEmpty[255] = TEXT(
		"                                                                                                                                                                                                                                                              ");
	DWORD dwWritten;
	COORD coord = {0, 0};
	SMALL_RECT rcWindow = {0, 0, 79, (SHORT)(dwWindowSizeY - 1)};

	SetConsoleWindowInfo(g_hStdOut, TRUE, &rcWindow);

	COORD crdBufferSize;
	crdBufferSize.X = 80;
	crdBufferSize.Y = (SHORT)dwWindowSizeY;
	...
	for (int i = rcWindow.Top; i < rcWindow.Bottom; i++) {
		...
		WriteConsole(g_hStdOut, strEmpty,
			static_cast<DWORD>(rcWindow.Right + 1), &dwWritten, NULL);
	}
}
...
static void ClearScreen()
{
	...
	for (DWORD i = 0; i < g_dwWindowSizeY; i++) {
		...
		SetConsoleCursorPosition(g_hStdOut, coord);
		WriteConsole(g_hStdOut, strEmpty,
			static_cast<DWORD>(g_SavedConsoleInfo.srWindow.Right + 1),
			&dwWritten, NULL);
	}
...
static void UpdateTimeStamp()
{
	...
	_stprintf(g_strTimeStamp, TEXT("[%02d-%02d-%02d %02d:%02d:%02d]"),
		sysTime.wMonth, sysTime.wDay, sysTime.wYear % 100, sysTime.wHour,
...
// Desc:
//-----------------------------------------------------------------------------
static VOID RestoreOldConsoleSettings()
{
	static TCHAR strEmpty[255] = TEXT(
		"                                                                                                                                                                                                                                                              ");
	DWORD dwWritten;
	...
	for (int i = g_SavedConsoleInfo.srWindow.Top;
		 i < g_SavedConsoleInfo.srWindow.Bottom; i++) {
		coord.Y = (SHORT)i;
		SetConsoleCursorPosition(g_hStdOut, coord);
		WriteConsole(g_hStdOut, strEmpty,
			static_cast<DWORD>(g_SavedConsoleInfo.srWindow.Right + 1),
...
// Desc:
//-----------------------------------------------------------------------------
static UINT WINAPI OutputThread(LPVOID /* pParam */)
{
	...
	for (;;) {
		...
		while (g_dwQueueSize && dwNumProcess < 5) {
			...
			case SLINE_LOG: {
				...
				if (g_hLogFile) {
					...
					WriteFile(g_hLogFile, szLogBuffer[dwLogBufferNext],
						static_cast<DWORD>(
							lstrlen(szLogBuffer[dwLogBufferNext])),
						&dwWritten, NULL);
					TCHAR strEOL = TEXT('\r');
					WriteFile(
						g_hLogFile, &strEOL, sizeof(TCHAR), &dwWritten, NULL);
					strEOL = TEXT('\n');
					WriteFile(
						g_hLogFile, &strEOL, sizeof(TCHAR), &dwWritten, NULL);

					static float s_fLastFlushTime =
			...
			case SLINE_CMD: {
				...
				// and redisplay the array on the top half of the screen
				_tcscpy(szCmdBuffer[dwCmdBufferNext],
					g_szOutputBuffer[g_dwNextOutput]);
				...
				if (g_EnumLineType[g_dwNextOutput] == SLINE_PROMPT) {
					...
					dwCaretPos =
						static_cast<int>(_tcslen(szCmdBuffer[dwCmdBufferNext]));
				}
			...
			case SLINE_INPUT: {
				// Update the last prompt line in the szCmdBuffer array with
				// this string of input, so what was typed in is displayed as it
				// scrolls
				_tcscpy(&szCmdBuffer[dwLastPrompt][dwCaretPos],
					g_szOutputBuffer[g_dwNextOutput]);
			...
			if (g_EnumLineType[g_dwNextOutput] == SLINE_PROMPT) {
				...
				COORD coord = {(SHORT)dwCaretPos, (SHORT)(g_dwWindowSizeY - 2)};
				SetConsoleCursorPosition(g_hStdOut, coord);
			}
...
static void PrintHelp()
{
	...
	ConsolePrintf(SLINE_CMD,
		TEXT("    CONNECTIONINFO, NEXTCONNECTINFO, NEXTCONNECTINFORATE"));
	ConsolePrintf(SLINE_CMD, TEXT("    CLIENTPACKSIZE, SERVERPACKSIZE"));
	ConsolePrintf(SLINE_CMD,
		TEXT("    SPTHREADS, SPBUFFER, CLIENTTHREADWAIT, SERVERTHREADWAIT"));
}
...
// Desc:
//-----------------------------------------------------------------------------
static void ParseInput(TCHAR* buffer)
{
	...
	if (!_tcscmp(token, TEXT("STATS"))) {
		g_MazeServer.PrintStats();
	} else if (!_tcscmp(token, TEXT("STOP")) || !_tcscmp(token, TEXT("QUIT")) ||
		!_tcscmp(token, TEXT("EXIT")) || !_tcscmp(token, TEXT("Q"))) {
		g_bQuitThread = TRUE;
		SetEvent(g_hOutputEvent);
		...
		if (token) {
			DWORD chance = static_cast<DWORD>(_ttol(token));
			if (chance > 100) {
				ConsolePrintf(
					SLINE_CMD, TEXT("Must enter a value between 0 and 100"));
			} else {
				g_MazeServer.SetServerReliableRate(chance);
				ConsolePrintf(SLINE_CMD,
					TEXT("Server reliable packet rate set to %d%%"), chance);
			}
		} else {
			ConsolePrintf(SLINE_CMD,
				TEXT("Server reliable packet rate is %d%%"),
		...
		if (token) {
			...
			if (chance > 100) {
				...
				ConsolePrintf(SLINE_CMD,
					TEXT("Client reliable packet rate set to %d%%"), chance);
			}
		} else {
			ConsolePrintf(SLINE_CMD,
				TEXT("Client reliable packet rate is %d%%"),
				g_MazeServer.GetClientReliableRate());
		}
	} else if (!_tcscmp(token, TEXT("CLIENTUPDATE")) ||
		!_tcscmp(token, TEXT("CU"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD rate = static_cast<DWORD>(_ttol(token));
			if (rate > 1500 || rate < 50) {
				ConsolePrintf(SLINE_CMD,
					TEXT(
						"Must enter a value between 50 and 1500 (milliseconds)"));
			} else {
				g_MazeServer.SetClientUpdateRate(rate);
				ConsolePrintf(SLINE_CMD,
					TEXT("Client update frequency set to %dms"), rate);
			}
		} else {
			ConsolePrintf(SLINE_CMD, TEXT("Client update frequency is %dms"),
				g_MazeServer.GetClientUpdateRate());
		}
	} else if (!_tcscmp(token, TEXT("SERVERTIMEOUT")) ||
		!_tcscmp(token, TEXT("ST"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			...
			if (timeout > 3000 || timeout < 0) {
				ConsolePrintf(SLINE_CMD,
					TEXT(
						"Must enter a value between 0 and 3000 (milliseconds) (0 for none)"));
			} else {
				g_MazeServer.SetServerTimeout(static_cast<DWORD>(timeout));
				if (timeout == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Server timeout set to 'no timeout'"));
				} else {
					ConsolePrintf(
						SLINE_CMD, TEXT("Server timeout set to %dms"), timeout);
				}
			}
		} else {
			if (g_MazeServer.GetServerTimeout() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Server timeout is %dms"),
					g_MazeServer.GetServerTimeout());
			}
		}
	} else if (!_tcscmp(token, TEXT("CLIENTTIMEOUT")) ||
		!_tcscmp(token, TEXT("CT"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			long timeout = _ttol(token);
			if (timeout > 3000 || timeout < 0) {
				...
				g_MazeServer.SetClientTimeout(static_cast<DWORD>(timeout));
				if (timeout == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Client timeout set to 'no timeout'"));
				} else {
					ConsolePrintf(
						SLINE_CMD, TEXT("Client timeout set to %dms"), timeout);
				}
			}
		} else {
			if (g_MazeServer.GetClientTimeout() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Client timeout is %dms"),
					g_MazeServer.GetClientTimeout());
			}
		}
	} else if (!_tcscmp(token, TEXT("NEXTCONNECTINFORATE")) ||
		!_tcscmp(token, TEXT("NCIRATE"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
		...
		if (g_fNextCIRate == 0.0f) {
			ConsolePrintf(SLINE_CMD,
				TEXT("NEXTCONNECTIONINFO will not be called automatically"));
		} else {
			ConsolePrintf(SLINE_CMD,
				TEXT("NEXTCONNECTIONINFO will be called every %0.0f mins"),
				g_fNextCIRate);
		}
		ConsolePrintf(SLINE_CMD,
			TEXT(
				"     Sets how often NEXTCONNECTIONINFO is automatically called (0 for never)"));
	} else if (!_tcscmp(token, TEXT("NEXTCONNECTIONINFO")) ||
		!_tcscmp(token, TEXT("NCI"))) {
		g_MazeServer.DisplayNextConnectionInfo();
	} else if (!_tcscmp(token, TEXT("CONNECTIONINFO")) ||
		!_tcscmp(token, TEXT("CI"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		BOOL bTokenOK = FALSE;
		if (token) {
			if (_tcslen(token) == 8) {
		...
		if (!bTokenOK) {
			ConsolePrintf(
				SLINE_CMD, TEXT("Must enter the 8 digit client DPNID"));
		}
	} else if (!_tcscmp(token, TEXT("LOGLEVEL"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD dwLevel = static_cast<DWORD>(_ttol(token));
			if (dwLevel > 3) {
				ConsolePrintf(
					SLINE_CMD, TEXT("Must enter a value between 0 and 3"));
			} else {
		...
		ConsolePrintf(
			SLINE_CMD, TEXT("Log level set to %d"), g_MazeServer.GetLogLevel());
		ConsolePrintf(
			SLINE_CMD, TEXT("     Level 1: Connects and disconnects only"));
		ConsolePrintf(
			SLINE_CMD, TEXT("     Level 2: Client position every 60 seconds"));
		ConsolePrintf(
			SLINE_CMD, TEXT("     Level 3: Show all client position "));
	} else if (!_tcscmp(token, TEXT("HELP")) || !_tcscmp(token, TEXT("?"))) {
		PrintHelp();
	} else if (!_tcscmp(token, TEXT("CLS"))) {
		ClearScreen();
	} else if (!_tcscmp(token, TEXT("CLIENTPACKSIZE")) ||
		!_tcscmp(token, TEXT("CPS"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD packsize = static_cast<DWORD>(_ttol(token));
			if (packsize > 2048) {
				ConsolePrintf(SLINE_CMD,
					TEXT(
						"Must enter a value between 0 and 2048 (bytes) (0 for none)"));
			} else {
				g_MazeServer.SetClientPackSize(packsize);
				if (packsize == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Client Packsize set to 'no data'"));
				} else {
					ConsolePrintf(SLINE_CMD,
						TEXT("Client Packsize set to %d bytes"), packsize);
				}
			}
		} else {
			if (g_MazeServer.GetClientPackSize() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Client Packsize is %d bytes"),
					g_MazeServer.GetClientPackSize());
			}
		}
	} else if (!_tcscmp(token, TEXT("SERVERPACKSIZE")) ||
		!_tcscmp(token, TEXT("SPS"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD packsize = static_cast<DWORD>(_ttol(token));
			if (packsize > 2048) {
				...
				if (packsize == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Server Packsize set to 'no data'"));
				} else {
					ConsolePrintf(SLINE_CMD,
						TEXT("Server Packsize set to %d bytes"), packsize);
				}
			}
		} else {
			if (g_MazeServer.GetServerPackSize() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Server Packsize is %d bytes"),
					g_MazeServer.GetServerPackSize());
			}
		}
	} else if (!_tcscmp(token, TEXT("CLIENTTHREADWAIT")) ||
		!_tcscmp(token, TEXT("CTW"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD threadwait = static_cast<DWORD>(_ttol(token));
			if (threadwait > 60000) {
				ConsolePrintf(SLINE_CMD,
					TEXT(
						"Must enter a value between 0 and 60000 (ms) (0 for none)"));
			} else {
				g_MazeServer.SetClientThreadWait(threadwait);
				if (threadwait == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Client ThreadWait set to 'no delay'"));
				} else {
					ConsolePrintf(SLINE_CMD,
						TEXT("Client ThreadWait set to %d ms"), threadwait);
				}
			}
		} else {
			if (g_MazeServer.GetClientThreadWait() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Client ThreadWait is %d ms"),
					g_MazeServer.GetClientThreadWait());
			}
		}
	} else if (!_tcscmp(token, TEXT("SERVERTHREADWAIT")) ||
		!_tcscmp(token, TEXT("STW"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD threadwait = static_cast<DWORD>(_ttol(token));
			if (threadwait > 60000) {
				...
				if (threadwait == 0) {
					ConsolePrintf(
						SLINE_CMD, TEXT("Server ThreadWait set to 'no delay'"));
				} else {
					ConsolePrintf(SLINE_CMD,
						TEXT("Server ThreadWait set to %d ms"), threadwait);
				}
			}
		} else {
			if (g_MazeServer.GetServerThreadWait() == 0) {
				...
				ConsolePrintf(SLINE_CMD, TEXT("Server ThreadWait is %d ms"),
					g_MazeServer.GetServerThreadWait());
			}
		}
	} else if (!_tcscmp(token, TEXT("SPTHREADS")) ||
		!_tcscmp(token, TEXT("SPT"))) {
		DWORD currentthreads = g_pDPlay8Server->GetNumSPThreads();

		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD threads = static_cast<DWORD>(_ttol(token));
			if ((threads > MAX_SP_THREADS) || (threads <= currentthreads)) {
				ConsolePrintf(SLINE_CMD,
					TEXT("Must enter a value between %d and %d"),
			...
			ConsolePrintf(
				SLINE_CMD, TEXT("SP Threads set to %d"), currentthreads);
		}
	} else if (!_tcscmp(token, TEXT("SPBUFFER")) ||
		!_tcscmp(token, TEXT("SPB"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD uBufferSize = static_cast<DWORD>(_ttol(token));
			if ((uBufferSize > MAX_SP_BUFFER)) {
				ConsolePrintf(SLINE_CMD,
					TEXT("Must enter a value between 0 and %d"), MAX_SP_BUFFER);
			} else {
				g_pDPlay8Server->SetSPBuffer(uBufferSize);
		...
		ConsolePrintf(
			SLINE_CMD, TEXT("Unknown command. Type HELP for list of commands"));
	}
}
...
static void CreateTempLogFile()
{
	...
	if (GetFileAttributes(strTempPath) == -1) {
		BOOL bSuccess = CreateDirectory(strTempPath, NULL);
		if (!bSuccess) {
			ConsolePrintf(SLINE_LOG,
				TEXT("Could not create create temp directory '%s'"),
				strTempPath);
	...
	SYSTEMTIME sysTime;
	GetLocalTime(&sysTime);
	_stprintf(strTime, TEXT("server-%04d-%02d-%02d-"), sysTime.wYear,
	...
	g_hLogFile = CreateFile(strTempFileName, GENERIC_WRITE, FILE_SHARE_READ,
		NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
	if (g_hLogFile == INVALID_HANDLE_VALUE) {
		ConsolePrintf(SLINE_LOG, TEXT("Could not create create temp file '%s'"),
			strTempFileName);
	} else {
...
// Desc:
//-----------------------------------------------------------------------------
static VOID SuspendPowerManagement()
{
	...
	if (hInstKernel32 != NULL) {
		pSetThreadExecutionState = (LPSETTHREADEXECUTIONSTATE)GetProcAddress(
			hInstKernel32, "SetThreadExecutionState");
...
int __cdecl main(int argc, TCHAR* argv[])
{
	...
	if (FAILED(hr = g_Maze.Init(g_dwWidth, g_dwHeight, DEFAULT_SEED))) {
		...
		_stprintf(szMazeDescript,
			TEXT("Maze size (%d,%d) Created. (width,height)"), g_dwWidth,
			g_dwHeight);
		...
		if (FAILED(hr = g_MazeServer.Init(FALSE, &g_Maze))) {
			...
			if (g_pDPlay8Server == NULL) {
				...
				if (FAILED(hr = g_pDPlay8Server->Start(g_dwNumSPThreads))) {
					...
					g_hOutputThread = (HANDLE)_beginthreadex(
						NULL, 0, OutputThread, NULL, 0, &dwOutputThreadID);

					// Spin up a thread to update output
					UINT dwUpdateThreadID;
					g_hUpdateThread = (HANDLE)_beginthreadex(
						NULL, 0, UpdateThread, NULL, 0, &dwUpdateThreadID);

					ConsolePrintf(
						SLINE_CMD, TEXT("DirectPlay8 Server started"));
					ConsolePrintf(
						SLINE_CMD, TEXT("Type 'HELP' for a list of commands."));
					...
					while (!g_bQuitThread) {
						...
						DWORD dwRead;
						ReadConsole(GetStdHandle(STD_INPUT_HANDLE), buffer, 128,
							&dwRead, NULL);
						if (dwRead > 2) {
							buffer[dwRead - 2] = 0;
