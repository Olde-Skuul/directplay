...
HRESULT CConsoleGraphics::Create(HINSTANCE /* hInstance */)
{
	...
	if (m_pConfig->bAutoConnnect) {
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"Type 'AUTOCONNECT OFF' to stop automatically connecting upon startup."));
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT("Type 'SETUP' to change current connections settings."));
	}
	m_pMazeApp->ConsolePrintf(
		LINE_CMD, TEXT("Type 'HELP' for a list of commands."));
...
INT CConsoleGraphics::Run()
{
	...
	m_hPromptThread = (HANDLE)_beginthreadex(
		NULL, 0, StaticPromptThread, NULL, 0, &dwPromptThreadID);
...
// Desc:
//-----------------------------------------------------------------------------
WCHAR** CConsoleGraphics::ConvertCommandLineToArgvW(
	WCHAR* strCommandLine, int* argc)
{
...
BOOL CConsoleGraphics::ParseCommandLine()
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (arg[0] == L'/' || arg[0] == L'-') {
			if (_wcsicmp(arg + 1, L"m") == 0) {
				...
				if (spBufferSize > MAX_SP_BUFFER) {
					...
					printf("  SPBuffer Value must be between 0 and %i.\n",
						MAX_SP_BUFFER);
					printf(
						"  Format: MazeConsoleClient.exe [/SPBuffer:n] : Sets SP Buffer Size to n \n\n");
			...
			if (_wcsicmp(arg + 1, L"f") == 0 || _wcsicmp(arg + 1, L"f+") == 0) {
				...
				m_pConfig->dwLogLevel = static_cast<DWORD>(_wtoi(arg + 5));
				bParamFound = TRUE;
			} else if (_wcsicmp(arg + 1, L"?") == 0) {
				printf("MazeConsoleClient\n");
				printf(
					"  Format: MazeConsoleClient.exe [/M] [/L] [/R[:ipaddress]] [/D] [/F] [/LOG]\n");
				printf("\n");
				printf("  Options:\n");
				printf("           /M  : connects to Microsoft server\n");
				printf("           /L  : connects to local server\n");
				printf(
					"           /R[:ipaddress] : connects to remote server at [ipaddress]\n");
				...
				printf(
					"           /SPThreads:n : SP threads to n. Range %d - %d\n",
					MIN_SP_THREADS, MAX_SP_THREADS);
				printf(
					"           /SPBuffer:n : SP buffer size to n. Range 0 - %d\n",
					MAX_SP_BUFFER);
				printf(
					"           /StressConnect[:ipaddress] : tries [ipaddress], then local server.\n");
				printf("\n");
				printf("  Examples: \n");
...
// Desc:
//-----------------------------------------------------------------------------
VOID CConsoleGraphics::HandleOutputMsg(
	EnumLineType enumLineType, TCHAR* strLine)
{
	...
	case LINE_CMD: {
		...
		if (enumLineType == LINE_PROMPT) {
			...
			m_dwCaretPos =
				static_cast<int>(_tcslen(m_szCmdBuffer[m_dwCmdBufferNext]));
		}
	...
	if (enumLineType == LINE_PROMPT) {
		...
		COORD coord = {(SHORT)m_dwCaretPos, (SHORT)(m_dwWindowSizeY - 2)};
...
VOID CConsoleGraphics::SetupConsole(DWORD dwWindowSizeY)
{
	...
	for (int i = rcWindow.Top; i < rcWindow.Bottom; i++) {
		coord.Y = (SHORT)i;
		SetConsoleCursorPosition(m_hStdOut, coord);
		WriteConsole(m_hStdOut, strEmpty,
...
VOID CConsoleGraphics::RestoreOldConsoleSettings()
{
	...
	for (int i = m_SavedConsoleInfo.srWindow.Top;
		 i < m_SavedConsoleInfo.srWindow.Bottom; i++) {
		coord.Y = (SHORT)i;
		SetConsoleCursorPosition(m_hStdOut, coord);
		WriteConsole(m_hStdOut, strEmpty,
			static_cast<DWORD>(m_SavedConsoleInfo.srWindow.Right + 1),
			&dwWritten, NULL);
	}
}
...
void CConsoleGraphics::PrintHelp()
{
	...
	m_pMazeApp->ConsolePrintf(
		LINE_CMD, TEXT("    SETUP, EXIT, LOGLEVEL, CONNECTIONINFO"));
	m_pMazeApp->ConsolePrintf(
		LINE_CMD, TEXT("    FILELOG, CONNECT, DISCONNECT"));
	m_pMazeApp->ConsolePrintf(
		LINE_CMD, TEXT("    AUTOCONNECT, AUTODISCONNECT"));
...
void CConsoleGraphics::DoPrompt(TCHAR* strPromptString, TCHAR* strBuffer)
{
	...
	DWORD dwRead;
	BOOL bSuccess;
	bSuccess = ReadConsole(
		GetStdHandle(STD_INPUT_HANDLE), strBuffer, 128, &dwRead, NULL);
...
void CConsoleGraphics::RunSetupWizard()
{
	...
	while (bWrongToken) {
		...
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"Do you want to connect to DirectPlayMaze.rte.microsoft.com? (YES/NO)"));
		...
		if (token != NULL) {
			...
			if (!_tcscmp(token, TEXT("Y")) || !_tcscmp(token, TEXT("YES"))) {
				m_pConfig->bConnectToMicrosoftSite = TRUE;
				bWrongToken = FALSE;
			} else if (!_tcscmp(token, TEXT("N")) ||
				!_tcscmp(token, TEXT("NO"))) {
				bWrongToken = FALSE;
			}
		} else {
			m_pConfig->bConnectToMicrosoftSite = TRUE;
	...
	if (m_pConfig->bConnectToMicrosoftSite == FALSE) {
		...
		while (bWrongToken) {
			...
			m_pMazeApp->ConsolePrintf(LINE_CMD,
				TEXT(
					"Do you want to connect to a local server (searches local subnet)? (YES/NO)"));
			DoPrompt(TEXT("(Default:YES) > "), buffer);

			TCHAR* token = _tcstok(buffer, TEXT(" \t"));
			if (token != NULL) {
				_tcsupr(token);
				if (!_tcscmp(token, TEXT("Y")) ||
					!_tcscmp(token, TEXT("YES"))) {
					m_pConfig->bConnectToLocalServer = TRUE;
					bWrongToken = FALSE;
				} else if (!_tcscmp(token, TEXT("N")) ||
					!_tcscmp(token, TEXT("NO"))) {
		...
		if (m_pConfig->bConnectToLocalServer == FALSE) {
			...
			while (bWrongToken) {
				m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT(""));
				m_pMazeApp->ConsolePrintf(LINE_CMD,
					TEXT("What IP address do you want to connect to? "));
	...
	while (bWrongToken) {
		m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT(""));
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("Turn file logging on? (YES/NO)"));
		DoPrompt(TEXT("(Default:YES) > "), buffer);
	...
	while (bWrongToken) {
		...
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"Use these settings when disconnected and when the app starts? (YES/NO)"));
		DoPrompt(TEXT("(Default:YES) > "), buffer);
	...
	m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT(""));
	m_pMazeApp->ConsolePrintf(LINE_CMD,
		TEXT("Type 'SETUP' anytime to change these connections settings."));
	m_pMazeApp->ConsolePrintf(
		LINE_CMD, TEXT("Type 'HELP' for a list of commands."));
	m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT(""));
...
void CConsoleGraphics::ParseInput(TCHAR* buffer)
{
	...
	if (!_tcscmp(token, TEXT("STOP")) || !_tcscmp(token, TEXT("QUIT")) ||
		...
		m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT("Auto-Connect set to %s"),
			(m_pConfig->bAutoConnnect) ? TEXT("ON") : TEXT("OFF"));
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"  If ON, MazeConsoleClient connects automatically reconnects"));
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT("         when disconnected, and connects when loading app."));
	} else if (!_tcscmp(token, TEXT("SETRETRYDELAY"))) {
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD dwDelay = static_cast<DWORD>(_ttol(token));
			if (m_pConfig->dwNetworkRetryDelay < 1 ||
				m_pConfig->dwNetworkRetryDelay > 300) {
				m_pMazeApp->ConsolePrintf(
					LINE_CMD, TEXT("Must enter a value between 1 and 300"));
			} else {
				m_pConfig->dwNetworkRetryDelay = dwDelay;
			}
		}

		m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT("Retry Delay set to %d"),
			m_pConfig->dwNetworkRetryDelay);
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"  Sets how long the app waits between attempts to connect to the server."));
		...
		if (token) {
			...
			if (dwLevel > 3) {
				m_pMazeApp->ConsolePrintf(
					LINE_CMD, TEXT("Must enter a value between 0 and 3"));
			} else {
		...
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("Log level set to %d"), m_pConfig->dwLogLevel);
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("     Level 1: No client position "));
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("     Level 2: Client position every 10 seconds"));
		...
		if (m_pConfig->dwAutoPrintStats != 0) {
			m_pMazeApp->ConsolePrintf(LINE_CMD,
				TEXT("Auto-displaying stats every %d mins"),
				m_pConfig->dwAutoPrintStats);
		} else {
			m_pMazeApp->ConsolePrintf(
				LINE_CMD, TEXT("Not auto-displaying stats"));
		}
		m_pMazeApp->ConsolePrintf(LINE_CMD,
			TEXT(
				"     Controls how often the connection stats are auto-displayed"));
		...
		m_pMazeApp->ConsolePrintf(
			LINE_LOG, TEXT("Log Directory: '%s'"), m_pMazeApp->GetLogDir());
		m_pMazeApp->ConsolePrintf(LINE_LOG, TEXT("Logging to temp file: '%s'"),
			m_pMazeApp->GetLogFile());
		...
		m_pMazeApp->ConsolePrintf(LINE_CMD, TEXT("Auto-Disconnect set to %s"),
			(m_pConfig->bAutoDisconnnect) ? TEXT("ON") : TEXT("OFF"));
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("  If ON, app disconnects randomly"));
		...
		m_pMazeApp->ConsolePrintf(LINE_LOG,
			TEXT("Displaying connection info for 0x%0.8x"),
			m_pMazeClient->GetLocalClientID());
		m_pMazeApp->ConsolePrintf(LINE_LOG,
			TEXT("(Key: G=Guaranteed NG=Non-Guaranteed B=Bytes P=Packets)"));
		...
		if (token) {
			...
			if ((threads > MAX_SP_THREADS) || (threads <= currentthreads)) {
				...
				m_pMazeApp->ConsolePrintf(
					LINE_CMD, TEXT("SP Threads Set to %d"), threads);
			}
		} else {
		...
		token = _tcstok(NULL, TEXT(" \t"));
		if (token) {
			DWORD tbuffer = static_cast<DWORD>(_ttol(token));
			if ((tbuffer > MAX_SP_BUFFER)) {
				m_pMazeApp->ConsolePrintf(LINE_CMD,
					TEXT("Must enter a value between 0 and %d"), MAX_SP_BUFFER);
			} else {
				m_pDP8Client->SetSPBuffer(tbuffer);
				m_pMazeApp->ConsolePrintf(LINE_CMD,
					TEXT("SP Buffer Size Set to %d"),
					m_pDP8Client->GetSPBuffer());
			}
		} else {
			m_pMazeApp->ConsolePrintf(LINE_CMD,
				TEXT("SP Buffer Size set to %d"), m_pDP8Client->GetSPBuffer());
		}
		...
		m_pMazeApp->ConsolePrintf(
			LINE_CMD, TEXT("Unknown command. Type HELP for list of commands"));
	}
}
