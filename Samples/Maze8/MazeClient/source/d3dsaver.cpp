...
CD3DScreensaver::CD3DScreensaver()
{
	...
	// Note: clients should load a resource into m_strWindowTitle to localize
	// this string
...
HRESULT CD3DScreensaver::Create(HINSTANCE hInstance)
{
	...
	if (m_SaverMode == sm_preview) {
		...
		// before proceeding, so the display control panel knows to update
		// itself.
		m_bWaitForInputIdle = TRUE;

		// Post a message to mark the end of the initial group of window
		// messages
...
VOID CD3DScreensaver::EnumMonitors(VOID)
{
	...
	while (EnumDisplayDevices(NULL, iDevice, (DISPLAY_DEVICE*)&dispdev, 0)) {
		...
		if ((dispdev.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER) == 0) {
			...
			// a second time, passing dispdev.DeviceName (from the first call)
			// as the first parameter.
			EnumDisplayDevices(
				dispdev.DeviceName, 0, (DISPLAY_DEVICE*)&dispdev2, 0);
			...
			if (dispdev.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP) {
				EnumDisplaySettings(
					dispdev.DeviceName, ENUM_CURRENT_SETTINGS, &devmode);
				...
				pMonitorInfoNew->rcScreen.right =
					pMonitorInfoNew->rcScreen.left + devmode.dmPelsWidth;
				pMonitorInfoNew->rcScreen.bottom =
					pMonitorInfoNew->rcScreen.top + devmode.dmPelsHeight;
				pMonitorInfoNew->hMonitor = MonitorFromRect(
					&pMonitorInfoNew->rcScreen, MONITOR_DEFAULTTONULL);
			}
			m_dwNumMonitors++;
...
SaverMode CD3DScreensaver::ParseCommandLine(TCHAR* pstrCommandLine)
{
	...
	if (*pstrCommandLine == TEXT('\"')) {
		pstrCommandLine++;
		while (
			*pstrCommandLine != TEXT('\0') && *pstrCommandLine != TEXT('\"')) {
			pstrCommandLine++;
		}
		if (*pstrCommandLine == TEXT('\"')) {
			pstrCommandLine++;
	...
	case 'C':
		...
		if (isdigit(*pstrCommandLine)) {
			...
			DXUtil_ConvertGenericStringToAnsi(
				strCommandLine, pstrCommandLine, 2048);
...
HRESULT CD3DScreensaver::CreateSaverWindow()
{
	...
	if (m_SaverMode == sm_preview) {
		...
		AdjustWindowRect(
			&rect, WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP, FALSE);
		hwnd = CreateWindow(TEXT("Parent"), TEXT("FakeShell"),
			WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_POPUP, rect.left,
			rect.top, rect.right - rect.left, rect.bottom - rect.top, NULL,
	...
	*/
	...
	case sm_preview:
		...
		m_hWnd = CreateWindow(TEXT("D3DSaverWndClass"), m_strWindowTitle,
	...
	case sm_test:
		...
		dwStyle = WS_VISIBLE | WS_OVERLAPPED | WS_CAPTION | WS_MINIMIZEBOX |
			WS_SYSMENU;
		AdjustWindowRect(&rc, dwStyle, FALSE);
		m_hWnd = CreateWindow(TEXT("D3DSaverWndClass"), m_strWindowTitle,
			dwStyle, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top,
	...
	case sm_full:
		// Create windows for each monitor.  Note that m_hWnd is NULL when
		// CreateWindowEx is called for the first monitor, so that window has no
		// parent.  Windows for additional monitors are created as children of
		// the window for the first monitor.
		...
		for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
			...
			pMonitorInfo->hWnd = CreateWindowEx(WS_EX_TOPMOST,
				TEXT("D3DSaverWndClass"), m_strWindowTitle, dwStyle, rc.left,
				rc.top, rc.right - rc.left, rc.bottom - rc.top, m_hWnd, NULL,
				m_hInstance, this);
	...
	if (m_SaverMode == sm_full && m_bIs9x) {
		...
		if (RegOpenKey(HKEY_CURRENT_USER, REGSTR_PATH_SCREENSAVE, &hKey) ==
			...
			if ((RegQueryValueEx(hKey, REGSTR_VALUE_USESCRPASSWORD, NULL, NULL,
				...
				if (m_hPasswordDLL) {
					m_VerifySaverPassword = (VERIFYPWDPROC)GetProcAddress(
						m_hPasswordDLL, "VerifyScreenSavePwd");
				}
				RegCloseKey(hKey);
			}
		}
...
//-----------------------------------------------------------------------------
LRESULT CALLBACK CD3DScreensaver::SaverProcStub(
...
//-----------------------------------------------------------------------------
LRESULT CD3DScreensaver::SaverProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg) {
	case WM_USER:
	...
	case WM_ERASEBKGND:
		...
		if (!m_bCheckingSaverPassword) {
			...
			for (DWORD iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits;
				 iRenderUnit++) {
	...
	if (m_SaverMode == sm_test ||
		...
		if (m_bIs9x && m_SaverMode == sm_full) {
			...
			if (m_VerifySaverPassword != NULL) {
				...
				for (iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits;
					 iRenderUnit++) {
					pRenderUnit = &m_RenderUnits[iRenderUnit];
				...
				// mode changes)
				D3DAdapterInfo* pD3DAdapterInfo;
				for (DWORD iAdapter = 0; iAdapter < m_dwNumAdapters;
					 iAdapter++) {
						...
						if (FAILED(hr)) {
							...
							if (FAILED(hr = InitDeviceObjects())) {
								...
								if (FAILED(hr = RestoreDeviceObjects())) {
									m_bErrorMode = TRUE;
									m_hrError =
										D3DAPPERR_INITDEVICEOBJECTSFAILED;
								} else {
...
HRESULT CD3DScreensaver::Initialize3DEnvironment()
{
	...
	if (m_SaverMode == sm_full) {
		...
		for (iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
			...
			if (pD3DAdapterInfo->bDisableHW &&
				!pD3DAdapterInfo->bHasAppCompatSW && !m_bAllowRef) {
				pD3DAdapterInfo->bLeaveBlack = TRUE;
			}
	...
	for (iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
		...
		if (!m_bWindowed) {
			SetWindowPos(pMonitorInfo->hWnd, HWND_TOPMOST,
	...
	if (!m_bWindowed) {
		for (iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits; iRenderUnit++) {
			...
			UnionRect(
				&m_rcRenderTotal, &m_rcRenderTotal, &pMonitorInfo->rcScreen);
		}
	}
...
HRESULT CD3DScreensaver::CreateFullscreenRenderUnit(RenderUnit* pRenderUnit)
{
	...
	for (DWORD iDevice = 0; iDevice < pD3DAdapterInfo->dwNumDevices;
		...
		if (pD3DDeviceInfo->DeviceType == D3DDEVTYPE_HAL &&
			...
			m_bAllowRef && curType != D3DDEVTYPE_SW) {
			dwCurrentDevice = iDevice;
	...
	if (pD3DDeviceInfo->dwCurrentMode == 0xffff) {
		if (pD3DDeviceInfo->DeviceType == D3DDEVTYPE_SW) {
			...
			DWORD dwSmallestHeight = (DWORD)-1;
			pD3DDeviceInfo->dwCurrentMode =
				0; // unless we find something better
	...
	if (pD3DDeviceInfo->dwCurrentMode == 0xffff) {
		...
		for (DWORD iMode = 0; iMode < pD3DDeviceInfo->dwNumModes; iMode++) {
			...
			if (dwWidthCur > dwWidthMax || dwHeightCur > dwHeightMax ||
				dwWidthCur == dwWidthMax && dwHeightCur == dwHeightMax &&
					dwBppCur > dwBppMax) {
	...
	if (SUCCEEDED(hr) && bAtLeastOneFailure && m_strRegPath[0] != TEXT('\0')) {
		...
		pD3DAdapterInfo->d3dfmtUserPrefFormat =
			pRenderUnit->d3dpp.BackBufferFormat;

		if (ERROR_SUCCESS ==
			RegCreateKeyEx(HKEY_CURRENT_USER, m_strRegPath, 0, NULL,
				REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hkeyParent,
				NULL)) {
			wsprintf(strKey, TEXT("Screen %d"), iMonitor + 1);
			if (ERROR_SUCCESS ==
				RegCreateKeyEx(hkeyParent, strKey, 0, NULL,
				...
				RegSetValueEx(hkey, TEXT("Format"), NULL, REG_DWORD,
					(BYTE*)&pD3DAdapterInfo->d3dfmtUserPrefFormat,
					sizeof(DWORD));
				RegSetValueEx(hkey, TEXT("Adapter ID"), NULL, REG_BINARY,
					(BYTE*)&pD3DAdapterInfo->d3dAdapterIdentifier
						.DeviceIdentifier,
					sizeof(GUID));
				RegCloseKey(hkey);
			}
...
BOOL CD3DScreensaver::FindNextLowerMode(D3DDeviceInfo* pD3DDeviceInfo)
{
	...
	BOOL b32BitCur =
		(d3dfmtCur == D3DFMT_A8R8G8B8 || d3dfmtCur == D3DFMT_X8R8G8B8);
	DWORD iModeNew;
	...
	for (iModeNew = 0; iModeNew < pD3DDeviceInfo->dwNumModes; iModeNew++) {
		...
		b32BitNew =
			(d3dfmtNew == D3DFMT_A8R8G8B8 || d3dfmtNew == D3DFMT_X8R8G8B8);

		// If we're currently 32-bit and new mode is same width/height and
		// 16-bit, take it
		if (b32BitCur && !b32BitNew && pD3DModeInfoNew->Width == dwWidthCur &&
...
HRESULT CD3DScreensaver::CreateWindowedRenderUnit(RenderUnit* pRenderUnit)
{
	...
	// if it's there, otherwise SW, otherwise REF.
	pD3DAdapterInfo->dwCurrentDevice =
		0xffff; // unless we find something better
	curType = D3DDEVTYPE_FORCE_DWORD;
	for (DWORD iDevice = 0; iDevice < pD3DAdapterInfo->dwNumDevices;
		 iDevice++) {
		pD3DDeviceInfo = &pD3DAdapterInfo->devices[iDevice];
		if (pD3DDeviceInfo->DeviceType == D3DDEVTYPE_HAL &&
			!pD3DAdapterInfo->bDisableHW && pD3DDeviceInfo->bCanDoWindowed) {
	...
	if (FAILED(CheckWindowedFormat(iAdapter, &D3DWindowedModeInfo))) {
		...
		if (FAILED(CheckWindowedFormat(iAdapter, &D3DWindowedModeInfo))) {
			...
			if (FAILED(CheckWindowedFormat(iAdapter, &D3DWindowedModeInfo))) {
				...
				if (FAILED(
						CheckWindowedFormat(iAdapter, &D3DWindowedModeInfo))) {
					D3DWindowedModeInfo.BackBufferFormat = D3DFMT_R5G6B5;
					if (FAILED(CheckWindowedFormat(
							iAdapter, &D3DWindowedModeInfo))) {
						return E_FAIL;
					}
				}
			}
	...
	for (iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits; iRenderUnit++) {
		...
		if (pRenderUnit->dwBehavior & D3DCREATE_HARDWARE_VERTEXPROCESSING &&
			...
			if (pRenderUnit->DeviceType == D3DDEVTYPE_HAL) {
				...
				lstrcat(
					pRenderUnit->strDeviceStats, TEXT(" (simulated hw vp)"));
			}
			...
			if (pRenderUnit->DeviceType == D3DDEVTYPE_HAL) {
				...
				lstrcat(
					pRenderUnit->strDeviceStats, TEXT(" (simulated mixed vp)"));
			}
		} else if (pRenderUnit->dwBehavior &
		...
		if (pRenderUnit->DeviceType == D3DDEVTYPE_HAL) {
			...
			DXUtil_ConvertAnsiStringToGeneric(szDescription,
				m_Adapters[pRenderUnit->iAdapter]
...
//-----------------------------------------------------------------------------
VOID CD3DScreensaver::BuildProjectionMatrix(
	FLOAT fNear, FLOAT fFar, D3DXMATRIX* pMatrix)
{
	D3DXMATRIX mat;
	INT cx, cy;
	INT dx, dy;
...
HRESULT CD3DScreensaver::BuildDeviceList()
{
	...
	for (UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++) {
		...
		m_pD3D->GetAdapterIdentifier(
			iAdapter, D3DENUM_NO_WHQL_LEVEL, &pAdapter->d3dAdapterIdentifier);
		...
		for (UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++) {
			...
			m_pD3D->GetDeviceCaps(
				iAdapter, DeviceTypes[iDevice], &pDevice->d3dCaps);
			...
			for (DWORD f = 0; f < dwNumFormats; f++) {
				...
				// device
				if (FAILED(m_pD3D->CheckDeviceType(iAdapter,
						pDevice->DeviceType, formats[f], formats[f], FALSE))) {
					continue;
				}
				...
				if (FALSE == bFormatConfirmed[f]) {
					...
					if (SUCCEEDED(ConfirmDevice(
							&pDevice->d3dCaps, dwBehavior[f], formats[f]))) {
						bFormatConfirmed[f] = TRUE;
					}
				}
					...
					if (modes[m].Format == formats[f]) {
						if (bFormatConfirmed[f] == TRUE) {
							...
							pDevice->modes[pDevice->dwNumModes].dwBehavior =
								dwBehavior[f];
							pDevice->modes[pDevice->dwNumModes]
								.DepthStencilFormat = fmtDepthStencil[f];
							pDevice->dwNumModes++;
		...
		// Count adapters even if no devices, so we can throw up blank windows
		// on them
		...
		for (DWORD d = 0; d < m_Adapters[a]->dwNumDevices; d++) {
			if (m_Adapters[a]->devices[d].bWindowed) {
				...
				if (m_Adapters[a]->devices[d].DeviceType == D3DDEVTYPE_REF) {
					if (!bHALExists) {
						DisplayErrorMsg(
							D3DAPPERR_NOHARDWAREDEVICE, MSGWARN_SWITCHEDTOREF);
					} else if (!bHALIsSampleCompatible) {
						DisplayErrorMsg(
							D3DAPPERR_HALNOTCOMPATIBLE, MSGWARN_SWITCHEDTOREF);
					} else if (!bHALIsWindowedCompatible) {
						DisplayErrorMsg(
							D3DAPPERR_NOWINDOWEDHAL, MSGWARN_SWITCHEDTOREF);
					} else if (!bHALIsDesktopCompatible) {
						DisplayErrorMsg(
							D3DAPPERR_NODESKTOPHAL, MSGWARN_SWITCHEDTOREF);
					} else {	// HAL is desktop compatible, but not
						sample compatible DisplayErrorMsg(
							D3DAPPERR_NOHALTHISMODE, MSGWARN_SWITCHEDTOREF);
					}
				}
				return S_OK;
			}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DScreensaver::CheckWindowedFormat(
	UINT iAdapter, D3DWindowedModeInfo* pD3DWindowedModeInfo)
{
	HRESULT hr;
	D3DAdapterInfo* pD3DAdapterInfo = m_Adapters[iAdapter];
	D3DDeviceInfo* pD3DDeviceInfo =
	...
	if (FAILED(
			hr = m_pD3D->CheckDeviceType(iAdapter, pD3DDeviceInfo->DeviceType,
...
//-----------------------------------------------------------------------------
BOOL CD3DScreensaver::FindDepthStencilFormat(UINT iAdapter,
	D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat,
	D3DFORMAT* pDepthStencilFormat)
{
	if (m_dwMinDepthBits <= 16 && m_dwMinStencilBits == 0) {
		if (SUCCEEDED(
				m_pD3D->CheckDeviceFormat(iAdapter, DeviceType, TargetFormat,
			...
			if (D3DERR_DEVICENOTRESET == hr) {
				...
				if (m_bWindowed) {
					m_pD3D->GetAdapterDisplayMode(
						pRenderUnit->iAdapter, &pAdapterInfo->d3ddmDesktop);
					//                    m_d3dpp.BackBufferFormat =
					//                    pAdapterInfo->d3ddmDesktop.Format;
				}
...
VOID CD3DScreensaver::UpdateErrorBox()
{
	...
	if (m_szError[0] == TEXT('\0')) {
		GetTextForError(
			m_hrError, m_szError, sizeof(m_szError) / sizeof(TCHAR));
	}
	...
	for (DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
		...
		if (pMonitorInfo->widthError == 0) {
			if (m_SaverMode == sm_preview) {
				pMonitorInfo->widthError =
					(float)(rcBounds.right - rcBounds.left);
				pMonitorInfo->heightError =
					(float)(rcBounds.bottom - rcBounds.top);
				pMonitorInfo->xError = 0.0f;
				pMonitorInfo->yError = 0.0f;
				pMonitorInfo->xVelError = 0.0f;
				pMonitorInfo->yVelError = 0.0f;
				InvalidateRect(
					hwnd, NULL, FALSE); // Invalidate the hwnd so it gets drawn
				UpdateWindow(hwnd);
			} else {
				pMonitorInfo->widthError = 300;
				pMonitorInfo->heightError = 150;
				pMonitorInfo->xError = (rcBounds.right + rcBounds.left -
										   pMonitorInfo->widthError) /
					2.0f;
				pMonitorInfo->yError = (rcBounds.bottom + rcBounds.top -
										   pMonitorInfo->heightError) /
					2.0f;
				pMonitorInfo->xVelError =
					(rcBounds.right - rcBounds.left) / 10.0f;
				pMonitorInfo->yVelError =
					(rcBounds.bottom - rcBounds.top) / 20.0f;
			}
		} else {
			if (m_SaverMode != sm_preview) {
				RECT rcOld;
				...
				SetRect(&rcOld, (INT)pMonitorInfo->xError,
					(INT)pMonitorInfo->yError,
					(INT)(pMonitorInfo->xError + pMonitorInfo->widthError),
					(INT)(pMonitorInfo->yError + pMonitorInfo->heightError));

				// Update rect velocity
				if ((pMonitorInfo->xError +
								pMonitorInfo->xVelError * fTimeDelta +
								pMonitorInfo->widthError >
							rcBounds.right &&
						pMonitorInfo->xVelError > 0.0f) ||
					(pMonitorInfo->xError +
				...
				if ((pMonitorInfo->yError +
								pMonitorInfo->yVelError * fTimeDelta +
								pMonitorInfo->heightError >
							rcBounds.bottom &&
						pMonitorInfo->yVelError > 0.0f) ||
					(pMonitorInfo->yError +
				...
				SetRect(&rcNew, (INT)pMonitorInfo->xError,
					(INT)pMonitorInfo->yError,
					(INT)(pMonitorInfo->xError + pMonitorInfo->widthError),
					(INT)(pMonitorInfo->yError + pMonitorInfo->heightError));

				if (rcOld.left != rcNew.left || rcOld.top != rcNew.top) {
					InvalidateRect(hwnd, &rcOld,
						FALSE); // Invalidate old rect so it gets erased
					InvalidateRect(hwnd, &rcNew,
						FALSE); // Invalidate new rect so it gets drawn
					UpdateWindow(hwnd);
				}
			}
		}
...
//-----------------------------------------------------------------------------
BOOL CD3DScreensaver::GetTextForError(
	HRESULT hr, TCHAR* pszError, DWORD dwNumChars)
{
	const DWORD dwErrorMap[][2] = {//  HRESULT, stringID
		(DWORD)E_FAIL, IDS_ERR_GENERIC, D3DAPPERR_NODIRECT3D,
		IDS_ERR_NODIRECT3D, D3DAPPERR_NOWINDOWEDHAL, IDS_ERR_NOWINDOWEDHAL,
		...
		D3DAPPERR_NOHALTHISMODE, IDS_ERR_NOHALTHISMODE, D3DAPPERR_MEDIANOTFOUND,
		IDS_ERR_MEDIANOTFOUND, D3DAPPERR_RESIZEFAILED, IDS_ERR_RESIZEFAILED,
...
VOID CD3DScreensaver::UpdateFrameStats()
{
	...
	if (fTime - fLastTime > 1.0f) {
		...
		for (iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits; iRenderUnit++) {
			...
			_stprintf(pRenderUnit->strFrameStats, TEXT("%.02f fps (%dx%dx%d)"),
				m_fFPS, mode.Width, mode.Height,
...
VOID CD3DScreensaver::DoPaint(HWND hwnd, HDC hdc)
{
	...
	rc2 = rc;
	height = DrawText(
		hdc, m_szError, -1, &rc, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
	rc = rc2;
...
VOID CD3DScreensaver::ReadScreenSettings(HKEY hkeyParent)
{
	...
	for (iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
		...
		wsprintf(strKey, TEXT("Screen %d"), iMonitor + 1);
		if (ERROR_SUCCESS ==
			RegCreateKeyEx(hkeyParent, strKey, 0, NULL, REG_OPTION_NON_VOLATILE,
			...
			if (guidAdapterID ==
					pD3DAdapterInfo->d3dAdapterIdentifier.DeviceIdentifier ||
...
VOID CD3DScreensaver::WriteScreenSettings(HKEY hkeyParent)
{
	...
	for (iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++) {
		...
		if (ERROR_SUCCESS ==
			...
			RegSetValueEx(hkey, TEXT("Adapter ID"), NULL, REG_BINARY,
				(BYTE*)&pD3DAdapterInfo->d3dAdapterIdentifier.DeviceIdentifier,
				sizeof(GUID));
			RegCloseKey(hkey);
		}
	}
...
// Desc:
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CD3DScreensaver::ScreenSettingsDlgProcStub(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
// Desc:
//-----------------------------------------------------------------------------
INT_PTR CD3DScreensaver::ScreenSettingsDlgProc(
	HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG: {
		...
		CheckDlgButton(
			hWnd, IDC_SAME, (m_bAllScreensSame ? BST_CHECKED : BST_UNCHECKED));
	}
		return TRUE;
	...
	case WM_COMMAND:
		...
		case IDC_SAME:
			m_bAllScreensSame =
				(IsDlgButtonChecked(hWnd, IDC_SAME) == BST_CHECKED);
			break;
		...
		case IDC_MODESCOMBO:
			if (HIWORD(wParam) == CBN_SELCHANGE) {
				...
				if (iSel == 0) {
					...
					pD3DDeviceInfo =
						&pD3DAdapterInfo
							 ->devices[pD3DAdapterInfo->dwCurrentDevice];
					iMode = (DWORD)ComboBox_GetItemData(hwndModeList, iSel);
					pD3DModeInfo = &pD3DDeviceInfo->modes[iMode];
					m_Adapters[iAdapter]->dwUserPrefWidth = pD3DModeInfo->Width;
					m_Adapters[iAdapter]->dwUserPrefHeight =
						pD3DModeInfo->Height;
					m_Adapters[iAdapter]->d3dfmtUserPrefFormat =
						pD3DModeInfo->Format;
				}
			}
			break;
		...
		case IDC_MOREINFO: {
			...
			MessageBox(hWnd, szText, pMonitorInfo->strDeviceName,
...
VOID CD3DScreensaver::SetupAdapterPage(HWND hWnd)
{
	...
	for (DWORD iMode = 0; iMode < pD3DDeviceInfo->dwNumModes; iMode++) {
		...
		wsprintf(strMode, strModeFmt, pD3DModeInfo->Width, pD3DModeInfo->Height,
			dwBitDepth);
