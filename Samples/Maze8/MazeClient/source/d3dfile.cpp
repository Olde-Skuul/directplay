	...
	if (pMtrlBuffer && m_dwNumMaterials > 0) {
		...
		for (DWORD i = 0; i < m_dwNumMaterials; i++) {
			...
			if (d3dxMtrls[i].pTextureFilename) {
				...
				DXUtil_ConvertAnsiStringToGeneric(
					strTextureTemp, d3dxMtrls[i].pTextureFilename);
				DXUtil_FindMediaFile(strTexture, strTextureTemp);

				if (FAILED(D3DXCreateTextureFromFile(
						pd3dDevice, strTexture, &m_pTextures[i]))) {
					m_pTextures[i] = NULL;
				}
			}
		}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DMesh::Create(
	LPDIRECT3DDEVICE8 pd3dDevice, LPDIRECTXFILEDATA pFileData)
{
	LPD3DXBUFFER pMtrlBuffer = NULL;
	LPD3DXBUFFER pAdjacencyBuffer = NULL;
	HRESULT hr;
	...
	if (FAILED(hr = D3DXLoadMeshFromXof(pFileData, D3DXMESH_SYSTEMMEM,
				   pd3dDevice, &pAdjacencyBuffer, &pMtrlBuffer,
				   &m_dwNumMaterials, &m_pSysMemMesh))) {
		return hr;
	}
...
HRESULT CD3DMesh::SetFVF(LPDIRECT3DDEVICE8 pd3dDevice, DWORD dwFVF)
{
	...
	if (m_pSysMemMesh) {
		if (FAILED(m_pSysMemMesh->CloneMeshFVF(
				D3DXMESH_SYSTEMMEM, dwFVF, pd3dDevice, &pTempSysMemMesh))) {
			return E_FAIL;
		}
	}
	if (m_pLocalMesh) {
		if (FAILED(m_pLocalMesh->CloneMeshFVF(
				0L, dwFVF, pd3dDevice, &pTempLocalMesh))) {
...
HRESULT CD3DMesh::RestoreDeviceObjects(LPDIRECT3DDEVICE8 pd3dDevice)
{
	...
	if (FAILED(m_pSysMemMesh->CloneMeshFVF(
			0L, m_pSysMemMesh->GetFVF(), pd3dDevice, &m_pLocalMesh))) {
		return E_FAIL;
	}
...
// Desc:
//-----------------------------------------------------------------------------
BOOL CD3DFrame::EnumMeshes(BOOL (*EnumMeshCB)(CD3DMesh*, VOID*), VOID* pContext)
{
	if (m_pMesh) {
	...
	BOOL bDrawAlphaSubsets, D3DXMATRIX* pmatWorldMatrix)
{
	...
	if (m_pChild) {
		m_pChild->Render(
			pd3dDevice, bDrawOpaqueSubsets, bDrawAlphaSubsets, &matWorld);
	}
	...
	if (m_pNext) {
		m_pNext->Render(
			pd3dDevice, bDrawOpaqueSubsets, bDrawAlphaSubsets, &matSavedWorld);
	}
...
HRESULT CD3DFile::LoadFrame(LPDIRECT3DDEVICE8 pd3dDevice,
	LPDIRECTXFILEDATA pFileData, CD3DFrame* pParentFrame)
{
	...
	if (*pGUID == TID_D3DRMFrame) {
		...
		while (SUCCEEDED(pFileData->GetNextObject(&pChildObj))) {
			...
			hr = pChildObj->QueryInterface(
				IID_IDirectXFileData, (VOID**)&pChildData);
			if (SUCCEEDED(hr)) {
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DFile::CreateFromResource(
	LPDIRECT3DDEVICE8 pd3dDevice, TCHAR* strResource, TCHAR* strType)
{
	...
	if (FAILED(hr = pDXFile->RegisterTemplates(
				   (VOID*)D3DRM_XTEMPLATES, D3DRM_XTEMPLATE_BYTES))) {
		pDXFile->Release();
		return E_FAIL;
	}
	...
	hr = pDXFile->CreateEnumObject(
		(VOID*)&dxlr, DXFILELOAD_FROMRESOURCE, &pEnumObj);
	if (FAILED(hr)) {
		pDXFile->Release();
		return hr;
	}
...
HRESULT CD3DFile::Create(LPDIRECT3DDEVICE8 pd3dDevice, TCHAR* strFilename)
{
	...
	// Create enum object
	hr = pDXFile->CreateEnumObject(
		(VOID*)strPathANSI, DXFILELOAD_FROMFILE, &pEnumObj);
	if (FAILED(hr)) {
		pDXFile->Release();
		return hr;
	}
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DFile::Render(
	LPDIRECT3DDEVICE8 pd3dDevice, D3DXMATRIX* pmatWorldMatrix)
{
