...
VOID D3DUtil_InitMaterial(
	D3DMATERIAL8& mtrl, FLOAT r, FLOAT g, FLOAT b, FLOAT a)
{
...
VOID D3DUtil_InitLight(
	D3DLIGHT8& light, D3DLIGHTTYPE ltType, FLOAT x, FLOAT y, FLOAT z)
{
	...
	D3DXVec3Normalize((D3DXVECTOR3*)&light.Direction, &temp);
	light.Position.x = x;
	light.Position.y = y;
	light.Position.z = z;
	light.Range = 1000.0f;
	...
	LPDIRECT3DTEXTURE8* ppTexture, D3DFORMAT d3dFormat)
{
	...
	return D3DXCreateTextureFromFileEx(pd3dDevice, strPath, D3DX_DEFAULT,
		D3DX_DEFAULT, D3DX_DEFAULT, 0, d3dFormat, D3DPOOL_MANAGED,
...
HRESULT D3DUtil_CreateVertexShader(LPDIRECT3DDEVICE8 pd3dDevice,
	TCHAR* strFilename, DWORD* pdwVertexDecl, DWORD* pdwVertexShader)
{
	LPD3DXBUFFER pCode;
	TCHAR strPath[MAX_PATH];
	HRESULT hr;
	...
	if (FAILED(
			hr = D3DXAssembleShaderFromFile(strPath, 0, NULL, &pCode, NULL))) {
		return hr;
	}
...
D3DXQUATERNION D3DUtil_GetRotationFromCursor(HWND hWnd, FLOAT fTrackBallRadius)
{
	...
	FLOAT t = D3DXVec3Length(&temp) / (2.0f * fTrackBallRadius);
	if (t > +1.0f) {
		t = +1.0f;
	}
	if (t < -1.0f) {
...
//-----------------------------------------------------------------------------
HRESULT D3DUtil_SetDeviceCursor(
	LPDIRECT3DDEVICE8 pd3dDevice, HCURSOR hCursor, BOOL bAddWatermark)
{
	HRESULT hr = E_FAIL;
	ICONINFO iconinfo;
	BOOL bBWCursor;
	...
	if (!bBWCursor) {
		...
		GetDIBits(hdcColor, iconinfo.hbmColor, 0, dwHeightDest, pcrArrayColor,
			&bmi, DIB_RGB_COLORS);
	}
...
// Desc:
//-----------------------------------------------------------------------------
LRESULT CD3DArcBall::HandleMouseMessages(
	HWND /* hWnd */, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
		...
		if (MK_LBUTTON & wParam) {
			...
			FLOAT fDeltaX =
				(iCurMouseX - iMouseX) * m_fRadiusTranslation / m_iWidth;
			FLOAT fDeltaY =
				(iCurMouseY - iMouseY) * m_fRadiusTranslation / m_iHeight;

			if (wParam & MK_RBUTTON) {
				D3DXMatrixTranslation(
					&m_matTranslationDelta, -2 * fDeltaX, 2 * fDeltaY, 0.0f);
				D3DXMatrixMultiply(&m_matTranslation, &m_matTranslation,
					&m_matTranslationDelta);
			} else // wParam & MK_MBUTTON
			{
				D3DXMatrixTranslation(
					&m_matTranslationDelta, 0.0f, 0.0f, 5 * fDeltaY);
				D3DXMatrixMultiply(&m_matTranslation, &m_matTranslation,
					&m_matTranslationDelta);
			}
...
// Desc:
//-----------------------------------------------------------------------------
VOID CD3DCamera::SetViewParams(
	D3DXVECTOR3& vEyePt, D3DXVECTOR3& vLookatPt, D3DXVECTOR3& vUpVec)
{
	// Set attributes for the view matrix
	m_vEyePt = vEyePt;
	m_vLookatPt = vLookatPt;
	m_vUpVec = vUpVec;
	D3DXVECTOR3 temp = (m_vLookatPt - m_vEyePt);
	D3DXVec3Normalize(&m_vView, &temp);
	...
	FLOAT fFOV, FLOAT fAspect, FLOAT fNearPlane, FLOAT fFarPlane)
{
	...
	D3DXMatrixPerspectiveFovLH(
		&m_matProj, fFOV, fAspect, fNearPlane, fFarPlane);
}
