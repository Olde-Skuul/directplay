...
// Desc:
//-----------------------------------------------------------------------------
VOID CD3DGraphics::Init(
	CMazeApp* pMazeApp, CDPlay8Client* pDP8Client, CMazeClient* pMazeClient)
{
	m_pMazeApp = pMazeApp;
...
HRESULT CD3DGraphics::PostCreate()
{
	...
	if (m_SaverMode == sm_test || m_SaverMode == sm_full) {
...
INT CD3DGraphics::Run()
{
	...
	case sm_preview:
		/*
					// This is temp code to allow stepping thru code in the
		   preview case WNDCLASS    cls; cls.hCursor        = NULL; cls.hIcon =
		   NULL; cls.lpszMenuName   = NULL; cls.lpszClassName  = "Parent";
					cls.hbrBackground  = (HBRUSH) GetStockObject(WHITE_BRUSH);
					cls.hInstance      = m_hInstance;
					cls.style          =
		   CS_VREDRAW|CS_HREDRAW|CS_SAVEBITS|CS_DBLCLKS; cls.lpfnWndProc    =
		   DefWindowProc; cls.cbWndExtra     = 0; cls.cbClsExtra     = 0;
					RegisterClass( &cls );
		   ...
		   WS_VISIBLE|WS_OVERLAPPED|WS_CAPTION|WS_POPUP , FALSE ); hwnd=
		   CreateWindow( "Parent", "FakeShell" ,
										   WS_VISIBLE|WS_OVERLAPPED|WS_CAPTION|WS_POPUP
		   , rect.left , rect.top , rect.right-rect.left , rect.bottom-rect.top
		   , NULL , NULL , m_hInstance , NULL ); m_hWndParent = hwnd;
		*/
	case sm_test:
	case sm_full:
...
int CD3DGraphics::ScreenSaverDoConfig(BOOL bIsScreenSaverSettings)
{
	...
	if (DialogBox(m_hInstance, MAKEINTRESOURCE(IDD_SAVERCONFIG), NULL,
			StaticSaverConfigDlgProc) == IDOK) {
...
// Desc:
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CD3DGraphics::StaticSaverConfigDlgProc(
...
// Desc:
//-----------------------------------------------------------------------------
INT_PTR CALLBACK CD3DGraphics::SaverConfigDlgProc(
	HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM /* lParam */)
{
	switch (uMsg) {
	case WM_INITDIALOG:
...
void CD3DGraphics::PopulateSaverConfigDlg(HWND hDlg)
{
	...
	SendDlgItemMessage(hDlg, IDC_IPADDRESS, EM_SETLIMITTEXT,
		sizeof(m_pConfig->szIPAddress) - 1, 0);
}
...
{
	m_pConfig->bConnectToMicrosoftSite =
		IsDlgButtonChecked(hDlg, IDC_MICROSOFT);
	m_pConfig->bConnectToLocalServer =
		IsDlgButtonChecked(hDlg, IDC_LOCAL_SERVER);
	m_pConfig->bConnectToRemoteServer =
		IsDlgButtonChecked(hDlg, IDC_REMOTE_SERVER);
	m_pConfig->dwNetworkRetryDelay =
		GetDlgItemInt(hDlg, IDC_RETRY_DELAY, NULL, FALSE);
	...
	GetDlgItemText(hDlg, IDC_IPADDRESS, m_pConfig->szIPAddress,
		sizeof(m_pConfig->szIPAddress));
}
...
// Desc:
//-----------------------------------------------------------------------------
void CD3DGraphics::HandleOutputMsg(
	EnumLineType /* enumLineType */, TCHAR* /* strLine */)
{
...
// Desc:
//-----------------------------------------------------------------------------
LRESULT CD3DGraphics::SaverProc(
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DGraphics::ConfirmDevice(
	D3DCAPS8* /* pCaps */, DWORD dwBehavior, D3DFORMAT /* Format */)
{
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DGraphics::CreateTextureFromResource(
	INT nResource, LPDIRECT3DTEXTURE8* ppTexture)
{
	if (FAILED(D3DXCreateTextureFromResourceEx(m_pd3dDevice, NULL,
			MAKEINTRESOURCE(nResource), D3DX_DEFAULT, D3DX_DEFAULT,
			D3DX_DEFAULT, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
			D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR,
			D3DX_FILTER_TRIANGLE | D3DX_FILTER_MIRROR, 0xFF000000, NULL, NULL,
			ppTexture)))
...
HRESULT CD3DGraphics::InitDeviceObjects()
{
	...
	if (FAILED(CreateTextureFromResource(IDB_WALLTEXTURE, &m_pWallTexture)))
		return DXTRACE_ERR_NOMSGBOX(
			TEXT("IDB_WALLTEXTURE"), D3DAPPERR_MEDIANOTFOUND);
	if (FAILED(CreateTextureFromResource(IDB_FLOORTEXTURE, &m_pFloorTexture)))
		return DXTRACE_ERR_NOMSGBOX(
			TEXT("IDB_FLOORTEXTURE"), D3DAPPERR_MEDIANOTFOUND);
	if (FAILED(
			CreateTextureFromResource(IDB_CEILINGTEXTURE, &m_pCeilingTexture)))
		return DXTRACE_ERR_NOMSGBOX(
			TEXT("IDB_CEILINGTEXTURE"), D3DAPPERR_MEDIANOTFOUND);
	if (FAILED(CreateTextureFromResource(IDB_NETICON, &m_pNetIconTexture)))
		return DXTRACE_ERR_NOMSGBOX(
			TEXT("IDB_NETICON"), D3DAPPERR_MEDIANOTFOUND);
	if (FAILED(CreateTextureFromResource(IDB_LOCALICON, &m_pLocalIconTexture)))
		return DXTRACE_ERR_NOMSGBOX(
			TEXT("IDB_LOCALICON"), D3DAPPERR_MEDIANOTFOUND);

	// Create airplane for rendering players
	if (FAILED(m_pPlayerMesh->CreateFromResource(
			m_pd3dDevice, MAKEINTRESOURCE(IDB_TANK), TEXT("XFILE"))))
		return DXTRACE_ERR(TEXT("tank.x"), D3DAPPERR_MEDIANOTFOUND);

	// Create sphere for rendering clients
	if (FAILED(
			hr = D3DXCreateSphere(m_pd3dDevice, 0.2f, 8, 8, &m_pSphere, NULL)))
		return DXTRACE_ERR_NOMSGBOX(TEXT("D3DXCreateSphere"), hr);

	if (FAILED(hr = m_pd3dDevice->CreateVertexBuffer(
				   sizeof(VERTEX_TL_DIFFUSE) * 4, D3DUSAGE_WRITEONLY,
				   FVF_TL_DIFFUSE, D3DPOOL_MANAGED, &m_pMiniMapBackgroundVB)))
		return DXTRACE_ERR_NOMSGBOX(TEXT("CreateVertexBuffer"), hr);

	if (FAILED(hr = m_pd3dDevice->CreateVertexBuffer(
				   sizeof(VERTEX_TL_TEXTURED) * 4, D3DUSAGE_WRITEONLY,
				   FVF_TL_TEXTURED, D3DPOOL_MANAGED, &m_pIndicatorVB)))
		return DXTRACE_ERR_NOMSGBOX(TEXT("CreateVertexBuffer"), hr);
...
HRESULT CD3DGraphics::RestoreDeviceObjects()
{
	...
	if (FAILED(
			hr = m_pd3dDevice->CreateVertexBuffer(sizeof(VERTEX_DIFFUSE) * 1024,
	...
	const float viewheight = viewwidth * float(m_d3dsdBackBuffer.Height) /
	...
	// Init light
	D3DUtil_InitLight(m_Light, D3DLIGHT_POINT, m_vCameraPos.x, m_vCameraPos.y,
		m_vCameraPos.z);
...
HRESULT CD3DGraphics::Render()
{
	// Clear the viewport
	m_pd3dDevice->Clear(
		0L, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0x00000000, 1.0f, 0L);
	...
	if (SUCCEEDED(m_pd3dDevice->BeginScene())) {
		...
		D3DXMatrixTranslation(
			&matTrans, -m_vCameraPos.x, -m_vCameraPos.y, -m_vCameraPos.z);
		...
		m_dwNumVisibleCells = m_pMazeClient->m_Maze.GetVisibleCells(
			D3DXVECTOR2(m_vCameraPos.x, m_vCameraPos.z), dir, 1.8f,
			m_mcrVisList, MAX_VISLIST);
		...
		// Show frame rate
		m_pStatsFont->DrawText(
			2, 0, D3DCOLOR_ARGB(255, 255, 255, 0), m_strFrameStats);
		m_pStatsFont->DrawText(
			2, 20, D3DCOLOR_ARGB(255, 255, 255, 0), m_strDeviceStats);
...
void CD3DGraphics::DrawPlayers()
{
	...
	for (DWORD i = 0; i < m_dwNumVisibleCells; i++, pCell++) {
		PlayerObject* pPlayerObject =
			m_pMazeClient->GetFirstPlayerObjectInCell(pCell->x, pCell->y);
		while (pPlayerObject) {
			D3DXMatrixTranslation(&matTrans, pPlayerObject->vPos.x,
				pPlayerObject->vPos.y - 0.5f, pPlayerObject->vPos.z);
			D3DXMatrixRotationYawPitchRoll(&matRot,
				-AngleToFloat(pPlayerObject->aCameraYaw), -D3DX_PI / 2.0f,
				0.0f);
...
void CD3DGraphics::DrawFloor()
{
	...
	for (DWORD i = 0; i < m_dwNumVisibleCells; i++, pCell++) {
		...
		m_SmartVB.MakeRoom(
			verts_per_side, indicies_per_side, &pVert, &pIndex, &offset);
		LoadQuad(pVert, pIndex, offset, pos, D3DXVECTOR3(1, 0, 0),
	...
	for (DWORD i = 0; i < m_dwNumVisibleCells; i++, pCell++) {
		...
		if (cell & MAZE_WALL_NORTH) {
			...
			m_SmartVB.MakeRoom(
				verts_per_side, indicies_per_side, &pVert, &pIndex, &offset);
			LoadQuad(pVert, pIndex, offset, pos, D3DXVECTOR3(1, 0, 0),
				D3DXVECTOR3(0, -1, 0), D3DXVECTOR3(0, 0, 1));
		}
		...
		if (cell & MAZE_WALL_WEST) {
			...
			m_SmartVB.MakeRoom(
				verts_per_side, indicies_per_side, &pVert, &pIndex, &offset);
			LoadQuad(pVert, pIndex, offset, pos, D3DXVECTOR3(0, 0, -1),
				D3DXVECTOR3(0, -1, 0), D3DXVECTOR3(1, 0, 0));
		}
	...
	const D3DXVECTOR3& vBasis2, const D3DXVECTOR3& vNormal)
{
	...
	float u = 0;
	DWORD i;
	for (i = 0; i <= m_dwTesselation; i++, rowstart += vStep1, u += fStep) {
		D3DXVECTOR3 pos = rowstart;
		float v = 0;
		for (DWORD j = 0; j <= m_dwTesselation;
			 j++, pos += vStep2, v += fStep) {
			pVerts->vPos = pos;
...
void CD3DGraphics::DrawMiniMap()
{
	...
	v[0].vPos.x = float(mapx) - 0.5f;
	v[0].vPos.y = float(mapy) - 0.5f;
	v[1].vPos.x = float(mapx + mapsize) - 0.5f;
	v[1].vPos.y = float(mapy) - 0.5f;
	v[2].vPos.x = float(mapx + mapsize) - 0.5f;
	v[2].vPos.y = float(mapy + mapsize) - 0.5f;
	v[3].vPos.x = float(mapx) - 0.5f;
	v[3].vPos.y = float(mapy + mapsize) - 0.5f;
	v[0].vPos.z = v[1].vPos.z = v[2].vPos.z = v[3].vPos.z = 0.1f;
	v[0].fRHW = v[1].fRHW = v[2].fRHW = v[3].fRHW = 1.0f;
	v[0].dwDiffuse = v[1].dwDiffuse = v[2].dwDiffuse = v[3].dwDiffuse =
		0x80008000;
	...
	m_pd3dDevice->SetStreamSource(
		0, m_pMiniMapBackgroundVB, sizeof(VERTEX_TL_DIFFUSE));
	...
	D3DXMATRIX rot;
	D3DXMatrixRotationZ(&rot, -m_fCameraYaw);
	orthcam = orthcam * rot;
	...
	DWORD maxx =
		centrecellx + 6 > mazewidth - 1 ? mazewidth - 1 : centrecellx + 5;
	DWORD maxy =
		centrecelly + 6 > mazeheight - 1 ? mazeheight - 1 : centrecelly + 5;
	...
	rot.m[3][0] = m_vCameraPos.x;
	rot.m[3][2] = m_vCameraPos.z;
	m_pd3dDevice->SetTransform(D3DTS_WORLD, &rot);

	m_pMiniMapVB->Lock(0, 0, (BYTE**)&verts, D3DLOCK_DISCARD);
	verts[0].vPos.y = verts[1].vPos.y = verts[2].vPos.y = verts[3].vPos.y =
		1.0f;
	verts[0].dwDiffuse = verts[1].dwDiffuse = verts[2].dwDiffuse =
		verts[3].dwDiffuse = 0xb01010;
	verts[0].vPos.x = 0.0f;
	verts[0].vPos.z = 0.4f;
	verts[1].vPos.x = 0.2f;
	verts[1].vPos.z = 0.0f;
	...
	for (x = minx; x <= maxx; x++) {
		for (y = miny; y <= maxy; y++) {
			PlayerObject* pPlayerObject =
				m_pMazeClient->GetFirstPlayerObjectInCell(x, y);
			while (pPlayerObject) {
				D3DXMatrixTranslation(&matTrans, pPlayerObject->vPos.x,
					pPlayerObject->vPos.y, pPlayerObject->vPos.z);
				D3DXMatrixRotationYawPitchRoll(&matRot,
					-AngleToFloat(pPlayerObject->aCameraYaw), 0.0f, 0.0f);
...
void CD3DGraphics::DrawIndicators()
{
	...
	v[3].fU = 0;
	v[3].fV = 1;
	v[0].vPos.x = (xscale * 16) - 0.5f;
	v[0].vPos.y = ((480 - 68) * yscale) - 0.5f;
	v[1].vPos.x = (xscale * 48) - 0.5f;
	v[1].vPos.y = ((480 - 68) * yscale) - 0.5f;
	v[2].vPos.x = (xscale * 48) - 0.5f;
	v[2].vPos.y = ((480 - 36) * yscale) - 0.5f;
	v[3].vPos.x = (xscale * 16) - 0.5f;
	v[3].vPos.y = ((480 - 36) * yscale) - 0.5f;
	...
	m_pd3dDevice->SetStreamSource(
		0, m_pIndicatorVB, sizeof(VERTEX_TL_TEXTURED));
	m_pd3dDevice->DrawPrimitive(D3DPT_TRIANGLEFAN, 0, 2);
	...
	if (FALSE == m_pDP8Client->IsSessionLost()) {
		...
		m_pFont->DrawText((float)ptStart.x, (float)ptStart.y,
			D3DCOLOR_ARGB(255, 255, 255, 0), strText);
		wsprintf(strText, TEXT("Nearby Players: %4d"), dwNumNearbyPlayers);
		m_pFont->DrawText((float)ptStart.x, (float)ptStart.y + 20,
			D3DCOLOR_ARGB(255, 255, 255, 0), strText);
		wsprintf(strText, TEXT("          Ping: %4d ms"),
			m_pMazeClient->GetRoundTripLatencyMS());
		m_pFont->DrawText((float)ptStart.x, (float)ptStart.y + 40,
			D3DCOLOR_ARGB(255, 255, 255, 0), strText);
		wsprintf(strText, TEXT("     Data Rate: %4d Bps"),
			m_pMazeClient->GetThroughputBPS());
		m_pFont->DrawText((float)ptStart.x, (float)ptStart.y + 60,
			D3DCOLOR_ARGB(255, 255, 255, 0), strText);
	} else {
		if (m_pMazeApp->IsOutOfDateClient()) {
			POINT ptStart = {
				(long)(xscale * 60.0f), (long)((480 - 75) * yscale)};
			TCHAR strText[MAX_PATH];
