...
// Desc:
//-----------------------------------------------------------------------------
HRESULT DSUtil_FillSoundBuffer(
...
// Desc:
//-----------------------------------------------------------------------------
LPDIRECTSOUNDBUFFER DSUtil_LoadSoundBuffer(
	LPDIRECTSOUND pDS, const char* strName)
{
	...
	if (SUCCEEDED(DSUtil_GetWaveResource(NULL, strName, &dsbd.lpwfxFormat,
		...
		if (SUCCEEDED(pDS->CreateSoundBuffer(&dsbd, &pDSB, NULL))) {
			if (FAILED(DSUtil_FillSoundBuffer(
					pDSB, pbWaveData, dsbd.dwBufferBytes))) {
				pDSB->Release();
				pDSB = NULL;
			}
		} else {
...
HRESULT DSUtil_ReloadSoundBuffer(LPDIRECTSOUNDBUFFER pDSB, LPCTSTR strName)
{
	...
	if (FAILED(DSUtil_GetWaveResource(
			NULL, strName, NULL, &pbWaveData, &cbWaveSize))) {
		return E_FAIL;
	}
	...
	WAVEFORMATEX** ppWaveHeader, BYTE** ppbWaveData, DWORD* pcbWaveSize)
{
	...
	if (NULL == (hResInfo = FindResourceA(hModule, strName, "WAVE"))) {
		if (NULL == (hResInfo = FindResourceA(hModule, strName, "WAV"))) {
			return E_FAIL;
		}
	}
	...
	if (FAILED(DSUtil_ParseWaveResource(
...
SoundObject* DSUtil_CreateSound(LPCTSTR strName, DWORD dwNumConcurrentBuffers)
{
	...
	if (SUCCEEDED(DSUtil_GetWaveResource(
			NULL, strName, &pWaveHeader, &pbData, &cbData))) {
		...
		pSound->dwCurrent = 0;
		pSound->pdsbBuffers =
			new LPDIRECTSOUNDBUFFER[dwNumConcurrentBuffers + 1];

		pSound->pdsbBuffers[0] = DSUtil_LoadSoundBuffer(g_pDS, strName);

		for (DWORD i = 1; i < pSound->dwNumBuffers; i++) {
			if (FAILED(g_pDS->DuplicateSoundBuffer(
					pSound->pdsbBuffers[0], &pSound->pdsbBuffers[i]))) {
...
LPDIRECTSOUNDBUFFER DSUtil_GetFreeSoundBuffer(SoundObject* pSound)
{
	...
	if (dwStatus & DSBSTATUS_BUFFERLOST) {
		...
		if (FAILED(DSUtil_FillSoundBuffer(
				pDSB, pSound->pbWaveData, pSound->cbWaveSize))) {
			return NULL;
		}
	}
