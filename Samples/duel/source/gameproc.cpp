...
VOID* g_pvReceiveBuffer = NULL;

// size of buffer
DWORD g_dwReceiveBufferSize = 0;

// do we need to do host initializaton
BOOL g_bHaveHostInit;
...
HRESULT InitOurShip()
{
	...
	ship.dPosX = (double)randInt(0, MAX_SHIP_X);
	ship.dPosY = (double)randInt(0, MAX_SHIP_Y);
...
BOOL WINAPI SetPlayerLocalSoundDataCB(DPID dpId, DWORD /* dwPlayerType */,
	LPCDPNAME /* lpName */, DWORD /* dwFlags */, VOID* /* lpContext */)
{
	SHIP ship;
	...
	HRESULT hr = DPUtil_GetPlayerLocalData(dpId, &ship, &dwSize);
	return (DP_OK == hr);
};
...
{
	return DPUtil_EnumPlayers(
		&(g_pdpsd->guidInstance), SetPlayerLocalSoundDataCB, NULL, 0);
}
...
VOID ReleaseLocalData()
{
	if (g_nProgramState == PS_ACTIVE) {
		if (FAILED(DPUtil_EnumPlayers(
				&g_pdpsd->guidInstance, ReleasePlayerLocalDataCB, NULL, 0))) {
...
VOID UpdateState(SHIP* pShip, DWORD dwControls)
{
	...
	if (!pShip->bBulletEnable && pShip->bEnable) {
		if (dwControls & KEY_FIRE) {
			...
			// launch a new bullet
			pShip->dBulletPosX =
				(double)(WORD)(sin(fAngle) * 6.0 + 16.0 + pShip->dPosX);
			pShip->dBulletPosY =
				(double)(WORD)(-cos(fAngle) * 6.0 + 16.0 + pShip->dPosY);
...
VOID UpdateDisplayStatus(SHIP* pShip)
{
	...
	if (pShip->iCountDown < 0) {
		...
		pShip->dPosX = (double)randInt(0, MAX_SHIP_X);
		pShip->dPosY = (double)randInt(0, MAX_SHIP_Y);
...
VOID UpdatePosition(DPID dpId, SHIP* lpShip)
{
	...
	oldxCell = (BYTE)((int)(lpShip->dPosX + 16.0) >> 4);
	oldyCell = (BYTE)((int)(lpShip->dPosY + 16.0) >> 4);

	// compute new position
	lpShip->dPosX += lpShip->dVelX * dwTickDiff;
	lpShip->dPosY += lpShip->dVelY * dwTickDiff;

	newxCell = (BYTE)((int)(lpShip->dPosX + 16.0) >> 4);
	newyCell = (BYTE)((int)(lpShip->dPosY + 16.0) >> 4);
	if (oldxCell != newxCell) {
		// we allow ghosts to pass through the blocks
		if ((dpId == g_LocalPlayerDPID) && IsHit(newxCell, newyCell)) {
			if (lpShip->dVelX > 0.0) {

				lpShip->dPosX = (double)((oldxCell << 4) + 15 - 16);
			} else {

				lpShip->dPosX = (double)((oldxCell << 4) - 16);
			}
	...
	if (oldyCell != newyCell) {
		...
		if ((dpId == g_LocalPlayerDPID) && IsHit(newxCell, newyCell)) {
			if (lpShip->dVelY > 0.0) {
				lpShip->dPosY = (double)((oldyCell << 4) + 15 - 16);
			} else {
				lpShip->dPosY = (double)((oldyCell << 4) - 16);
			}
...
VOID UpdateFragment(int i)
{
	...
	g_Frags[i].dPosX += (double)(int)(g_Frags[i].dVelX * dwTickDiff);
	g_Frags[i].dPosY += (double)(int)(g_Frags[i].dVelY * dwTickDiff);
...
BOOL UpdateFrame()
{
	...
	case PS_ACTIVE:
		...
		if (!ship.bEnable) {
			...
			if (g_bIsActive &&
				((dwTickCount - dwSyncLastTick) > SYNC_INTERVAL)) {
				SendSync(&ship);
			...
			if (g_bUpdate) {
				...
				if ((dwTickCount - dwUpdateLastTick) > UPDATE_INTERVAL) {
					...
					g_ControlMsg.byState = (BYTE)g_dwKeys;
					SendGameMessage(
						(GENERICMSG*)&g_ControlMsg, DPID_ALLPLAYERS);
			...
			if (DoingLobbyMessages() &&
				// every 10 sec
				(dwTickCount - dwLobbyMsgLastTick) > 10000) {
...
//-----------------------------------------------------------------------------
BOOL WINAPI RenderPlayerCB(DPID dpId, DWORD /* dwPlayerType */,
	LPCDPNAME /* lpName */, DWORD /* dwFlags */, VOID* /* lpContext */)
{
	SHIP ship;
	SHIP ourShip;
	...
	if ((dpId != g_LocalPlayerDPID) && ourShip.bBulletEnable && ship.bEnable) {
		if ((ourShip.dBulletPosX > ship.dPosX) &&
			...
			// Save our score
			hr = DPUtil_SetPlayerLocalData(
				g_LocalPlayerDPID, &ourShip, sizeof(ourShip));
			if (FAILED(hr)) {
				ShowError(IDS_DPLAY_ERROR_SPLD);
				return FALSE;
			}
...
BOOL DrawScreen()
{
	...
	// Render players
	HRESULT hr = DPUtil_EnumPlayers(NULL, RenderPlayerCB, NULL, 0);
	if (FAILED(hr)) {
		ShowError(IDS_DPLAY_ERROR_EP);
		return FALSE;
	}
...
BOOL DrawScore()
{
	...
	strScore[0] = (CHAR)((BYTE)ship.dwScore / 100000 + '0');
	rem = (int)(ship.dwScore % 100000);
	strScore[1] = (CHAR)(rem / 10000 + '0');
	rem = (int)(ship.dwScore % 10000);
	strScore[2] = (CHAR)(rem / 1000 + '0');
	rem = (int)(ship.dwScore % 1000);
	strScore[3] = (CHAR)(rem / 100 + '0');
	rem = (int)(ship.dwScore % 100);
	strScore[4] = (CHAR)(rem / 10 + '0');
	rem = (int)(ship.dwScore % 10);
	strScore[5] = (CHAR)(rem + '0');
	strScore[6] = '\0';
	...
	for (int i = 0; i < 64; i++) {
		if (g_Frags[i].valid) {
			BltObject((int)g_Frags[i].dPosX, (int)g_Frags[i].dPosY,
				g_Frags[i].pdds, &g_Frags[i].src, DDBLTFAST_SRCCOLORKEY);
		}
	}
}
	...
	for (;;) {
		...
		if (hr == DPERR_BUFFERTOOSMALL) {
			...
			if (idFrom == DPID_SYSMSG) {
				DoSystemMessage(
					(DPMSG_GENERIC*)pvMsgBuffer, dwMsgBufferSize, idFrom, idTo);
			} else {
				DoApplicationMessage(
					(GENERICMSG*)pvMsgBuffer, dwMsgBufferSize, idFrom, idTo);
			}
		} else
			break;
	}
...
//-----------------------------------------------------------------------------
VOID DoSystemMessage(DPMSG_GENERIC* pMsg, DWORD /* dwMsgSize */,
	DPID /* idFrom */, DPID /* idTo */)
{
	...
	case DPSYS_CREATEPLAYERORGROUP: {
		...
		// Loop through the used ship types array and find the one that's been
		// used the least
		for (i = 0; i < NUM_SHIP_TYPES; i++) {
	...
	case DPSYS_DESTROYPLAYERORGROUP: {
		SHIP* pShip;
		DPMSG_DESTROYPLAYERORGROUP* pDestroyMsg =
		...
		g_ShipTypesUsed[pShip->byType]--; // decrement the leaving players' ship
										  // type usage counter
	...
	case DPSYS_SENDCOMPLETE:
		// Async send status
		{
			...
			if (FAILED(pComplete->hr)) {
				...
				TRACE(
					"Async msg %u to %u transmission failed. err = %s %#08X\n",
					pComplete->dwMsgID, pComplete->idTo, strErr, pComplete->hr);
			}
			break;
		}
	}
	...
	GENERICMSG* pMsg, DWORD dwMsgSize, DPID idFrom, DPID /* idTo */)
{
	...
	case MSG_HOST: {
		...
		if (!g_bHostPlayer) {
			...
			if (SUCCEEDED(hr) && dwMsgSize >= sizeof(HOSTMSG)) {
				...
				g_ShipTypesUsed[ship.byType]++; // increment it because we used
												// one

				hr =
					DPUtil_SetPlayerLocalData(g_LocalPlayerDPID, &ship, dwSize);
			}
	...
	case MSG_BLOCKHIT: {
		...
		g_Blocks.bits[pBlockHitMsg->byRow][pBlockHitMsg->byCol] &=
			~pBlockHitMsg->byMask;
		break;
	}
	...
	case MSG_SYNC: {
		...
		if (ship.bEnable) {
			// Head towards rendezvous location (accelerate/decelerate as
			// necessary)
...
VOID SendGameMessage(GENERICMSG* pMsg, DPID idTo)
{
	...
	DPUtil_Send(g_LocalPlayerDPID, idTo, dwFlags, (VOID*)pMsg, (DWORD)nBytes);
}
...
HRESULT InitializeGameSounds()
{
	...
	g_pShipEngineSound = DSUtil_CreateSound("SENGINE", 1);
	g_pBulletFiringSound = DSUtil_CreateSound("BFIRE", NUM_SHIP_TYPES);
	g_pShipExplodeSound = DSUtil_CreateSound("SBOOM", NUM_SHIP_TYPES);
	g_pShipStartSound = DSUtil_CreateSound("SSTART", NUM_SHIP_TYPES);
	g_pShipStopSound = DSUtil_CreateSound("SSTOP", NUM_SHIP_TYPES);
	g_pShipBounceSound = DSUtil_CreateSound("SBOUNCE", NUM_SHIP_TYPES);
	g_pBlockExplodeSound = DSUtil_CreateSound("LBOOM", NUM_SHIP_TYPES);
