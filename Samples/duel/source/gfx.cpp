...
// primary surface
LPDIRECTDRAWSURFACE g_pddsFrontBuffer = NULL;

// back buffer for animation
LPDIRECTDRAWSURFACE g_pddsBackBuffer = NULL;

// ship bitmaps
LPDIRECTDRAWSURFACE g_pddsShip[NUM_SHIP_TYPES];

// Numbers bitmap
LPDIRECTDRAWSURFACE g_pddsNumbers;
...
HRESULT InitGraphics()
{
	...
	// Create a window
	g_hwndMain = CreateWindowExA(WS_EX_APPWINDOW, "DuelClass", "Duel",
		WS_POPUP | WS_SYSMENU, 0, 0, GetSystemMetrics(SM_CXSCREEN),
		GetSystemMetrics(SM_CYSCREEN), NULL, NULL, g_hInst, NULL);
	if (NULL == g_hwndMain) {
		return E_FAIL;
	}
	...
	if (g_bFullscreen) {
		hr = g_pDD->SetCooperativeLevel(
			g_hwndMain, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN);
	} else {
		hr = g_pDD->SetCooperativeLevel(g_hwndMain, DDSCL_NORMAL);
	}
	...
	if (g_bFullscreen) {
		...
		SetWindowLongA(g_hwndMain, GWL_STYLE, (LONG)dwStyle);

		// Aet window size
		SetRect(&rc, 0, 0, MAX_DEFWIN_X, MAX_DEFWIN_Y);

		AdjustWindowRectEx(&rc, GetWindowStyle(g_hwndMain),
			GetMenu(g_hwndMain) != NULL, GetWindowExStyle(g_hwndMain));
	...
	if (g_bFullscreen) {
		...
		if (FAILED(hr = g_pddsFrontBuffer->GetAttachedSurface(
					   &ddscaps, &g_pddsBackBuffer))) {
...
HRESULT BltSplashScreen(RECT* prc)
{
	...
	HBITMAP hbm = (HBITMAP)LoadImageA(GetModuleHandle(NULL), "SPLASH",
	...
	for (;;) {
		hr = g_pddsFrontBuffer->Blt(
			&g_rcWindow, g_pddsBackBuffer, prc, DDBLT_WAIT, NULL);
		if (SUCCEEDED(hr)) {
			return S_OK;
		}
...
//-----------------------------------------------------------------------------
HRESULT BltObject(
	int x, int y, LPDIRECTDRAWSURFACE pdds, RECT* prc, DWORD flags)
{
	if (NULL == pdds) {
		return E_FAIL;
	}
	...
	for (;;) {
		HRESULT hr =
			g_pddsBackBuffer->BltFast((DWORD)x, (DWORD)y, pdds, prc, flags);
		if (FAILED(hr)) {
			if (hr == DDERR_WASSTILLDRAWING) {
				continue;
			}
...
VOID EraseScreen()
{
	...
	for (;;) {
		HRESULT hr =
			g_pddsBackBuffer->Blt(NULL, NULL, NULL, DDBLT_COLORFILL, &ddbltfx);
		if (SUCCEEDED(hr)) {
			return;
		}
...
VOID FlipScreen()
{
	...
	if (g_bFullscreen) {
		...
		g_pddsFrontBuffer->Blt(
			&g_rcWindow, g_pddsBackBuffer, NULL, DDBLT_WAIT, NULL);
	}
}
...
HRESULT RestoreSurfaces()
{
	...
	g_pArtPalette = DDUtil_LoadPalette(g_pDD, "Duel8");
	...
	HBITMAP hbm = (HBITMAP)LoadImageA(GetModuleHandle(NULL), "Duel8",
		IMAGE_BITMAP, 0, 0, LR_CREATEDIBSECTION);
	if (NULL == hbm) {
		return E_FAIL;
	}
