...
// Globals
//-----------------------------------------------------------------------------
static IDirectInput* g_pDI;                // DirectInput interface
...
HRESULT DIUtil_InitInput(HWND hWnd)
{
	...
	if (FAILED(DirectInput8Create((HINSTANCE)GetWindowLong(hWnd, GWL_HINSTANCE),
			DIRECTINPUT_VERSION, IID_IDirectInput8A, (void**)&g_pDI, NULL))) {
	...
	if (FAILED(g_pdidKeyboard->SetCooperativeLevel(
...
VOID DIUtil_ReadKeys(DWORD* pdwKeys)
{
	...
	HRESULT hr = g_pdidKeyboard->GetDeviceState(sizeof(rgbKeybd), rgbKeybd);
	if (FAILED(hr)) {
