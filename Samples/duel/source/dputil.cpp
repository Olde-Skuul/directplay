...
static void CheckCaps()
{
	...
	if (g_bAsyncSupported) {
		TRACE("Capabilities supported: Async %s %s %s\n",
			(dpcaps.dwFlags & DPCAPS_SENDPRIORITYSUPPORTED ? "SendPriority" :
															 ""),
			(dpcaps.dwFlags & DPCAPS_SENDTIMEOUTSUPPORTED ? "SendTimeout" : ""),
		...
		TRACE("CheckCaps - Async not supported\n");
	}
}
...
//-----------------------------------------------------------------------------
HRESULT DPUtil_CreatePlayer(DPID* ppidID, char* strPlayerName, HANDLE hEvent,
...
//       guid.
//-----------------------------------------------------------------------------
HRESULT DPUtil_CreateSession(char* strSessionName)
{
	if (NULL == g_pDP) {
...
HRESULT DPUtil_EnumPlayers(GUID* pSessionGuid,
	LPDPENUMPLAYERSCALLBACK2 pEnumCallback, VOID* pContext, DWORD dwFlags)
{
	if (NULL == g_pDP) {
		return E_FAIL;
	}
...
HRESULT DPUtil_EnumSessions(DWORD dwTimeout,
	LPDPENUMSESSIONSCALLBACK2 pEnumCallback, VOID* pContext, DWORD dwFlags)
{
	if (NULL == g_pDP) {
		return E_FAIL;
	}
	...
	return g_pDP->EnumSessions(
		&dpDesc, dwTimeout, pEnumCallback, pContext, dwFlags);
}
...
HRESULT DPUtil_GetPlayerLocalData(DPID pid, VOID* pData, DWORD* pdwDataSize)
{
	...
	if (FAILED(hr)) {
		TRACE("Get Player local data failed for id %d\n", pid);
	}
	return hr;
}
...
HRESULT DPUtil_Release()
{
	...
	// Release dplay
	HRESULT hr = (HRESULT)g_pDP->Release();
	g_pDP = NULL;
	...
	DPID idFrom, DPID idTo, DWORD dwFlags, VOID* pData, DWORD dwDataSize)
{
	...
	if (dwFlags & DPSEND_ASYNC) {
		...
		return g_pDP->SendEx(
			idFrom, idTo, dwFlags, pData, dwDataSize, 0, 0, NULL, NULL);
	} else {
...
HRESULT DPUtil_SetPlayerLocalData(DPID pid, VOID* pData, DWORD dwSize)
{
	...
	if (FAILED(hr)) {
		TRACE("Set Player local data failed for id %d\n", pid);
	}
	return hr;
}
