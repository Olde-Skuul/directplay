	...
	LPDIRECTDRAWSURFACE pdds, HBITMAP hbm, int x, int y, int dx, int dy)
{
	...
	GetObjectA(hbm, sizeof(bm), &bm); // get size of bitmap
	...
	if (SUCCEEDED(hr = pdds->GetDC(&hdc))) {
		StretchBlt(hdc, 0, 0, (int)ddsd.dwWidth, (int)ddsd.dwHeight, hdcImage,
			x, y, dx, dy, SRCCOPY);
		pdds->ReleaseDC(hdc);
	}
...
//       default 332 palette.
//-----------------------------------------------------------------------------
LPDIRECTDRAWPALETTE DDUtil_LoadPalette(LPDIRECTDRAW pDD, char* strBitmap)
{
	...
	if (strBitmap &&
		((h = FindResourceA(NULL, strBitmap, RT_BITMAP)) != NULL)) {
		...
		(fh = CreateFileA(strBitmap, GENERIC_READ, FILE_SHARE_READ, NULL,
	...
	// The Watcom header has an error and declares this
	// with the wrong type.
	pDD->CreatePalette(
		DDPCAPS_8BIT, ape, (LPDIRECTDRAWPALETTE)&pddPalette, NULL);
#else
