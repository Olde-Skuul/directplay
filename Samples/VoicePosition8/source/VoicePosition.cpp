...
#define PLAYER_ADDREF(pPlayerInfo) \
	if (pPlayerInfo) \
		pPlayerInfo->lRefCount++;
#define PLAYER_RELEASE(pPlayerInfo) \
	if (pPlayerInfo) { \
		pPlayerInfo->lRefCount--; \
		if (pPlayerInfo->lRefCount <= 0) { \
			DeleteCriticalSection(&pPlayerInfo->csPlayer); \
...
GUID g_guidApp = {0x9363cc5d, 0xf8be, 0x47cf,
	{0xa2, 0x88, 0xe4, 0xdd, 0x73, 0xf0, 0xbc, 0xa6}};
	...
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	...
	case DPN_MSGID_CREATE_PLAYER: {
		...
		while (hr == DPNERR_CONNECTING) {
			hr = g_pDP->GetPeerInfo(
				pCreatePlayerMsg->dpnidPlayer, pdpPlayerInfo, &dwSize, 0);
		}
	...
	case DPN_MSGID_DESTROY_PLAYER: {
		...
		APP_PLAYER_INFO* pPlayerInfo =
			(APP_PLAYER_INFO*)pDestroyPlayerMsg->pvPlayerContext;
	...
	case DPN_MSGID_RECEIVE: {
		...
		if (pMsg->dwType == GAME_MSGID_PLAYERMOVED) {
			GAMEMSG_PLAYERMOVED* pPlayerMovedMsg = (GAMEMSG_PLAYERMOVED*)pMsg;

			APP_PLAYER_INFO* pPlayerInfo =
				(APP_PLAYER_INFO*)pReceiveMsg->pvPlayerContext;
	...
	// Make sure the DirectPlay MessageHandler calls the CNetConnectWizard
	// handler, so it can be informed of messages such as
...
//-----------------------------------------------------------------------------
static HRESULT WINAPI DirectPlayLobbyMessageHandler(
	PVOID pvUserContext, DWORD dwMessageId, PVOID pMsgBuffer)
{
	switch (dwMessageId) {
	...
	case DPL_MSGID_DISCONNECT: {
		...
		// (PDPL_MESSAGE_DISCONNECT)pMsgBuffer;
	...
	case DPL_MSGID_RECEIVE: {
		// PDPL_MESSAGE_RECEIVE pReceiveMsg = (PDPL_MESSAGE_RECEIVE)pMsgBuffer;
	...
	case DPL_MSGID_CONNECTION_SETTINGS: {
		...
		// (PDPL_MESSAGE_CONNECTION_SETTINGS)pMsgBuffer;
	...
	// Make sure the DirectPlay MessageHandler calls the CNetConnectWizard
	// handler, so the wizard can be informed of lobby messages such as
	// DPL_MSGID_CONNECT
	if (g_pNetConnectWizard) {
...
static HRESULT InitDirectPlay()
{
	...
	if (FAILED(
			hr = g_pLobbiedApp->Initialize(NULL, DirectPlayLobbyMessageHandler,
				&hLobbyLaunchedConnection, 0))) {
		return DXTRACE_ERR(TEXT("Initialize"), hr);
	}

	// IDirectPlay8LobbiedApplication::Initialize returns a handle to a
	// connnection if we have been lobby launced.  Initialize is guanteeded to
	// return after the DPL_MSGID_CONNECT msg has been processed.  So unless a
	// we are expected multiple lobby connections, we do not need to remember
	// the lobby connection handle since it will be recorded upon the
	// DPL_MSGID_CONNECT msg.
...
//-----------------------------------------------------------------------------
static HRESULT InitDirectSound(HWND hDlg)
{
	HRESULT hr;

	if (FAILED(hr = DirectSoundCreate(
				   &DSDEVID_DefaultVoicePlayback, &g_pDS, NULL))) {
	...
	if (FAILED(hr = pDSBPrimary->QueryInterface(
				   IID_IDirectSound3DListener, (VOID**)&g_pDSListener))) {
	...
	dslsn.vOrientFront.x = 0.0f;
	dslsn.vOrientFront.y = 0.0f;
	dslsn.vOrientFront.z = 1.0f;
	dslsn.vOrientTop.x = 0.0f;
	dslsn.vOrientTop.y = 1.0f;
	dslsn.vOrientTop.z = 0.0f;
	dslsn.vPosition.x = 0.0f;
	dslsn.vPosition.y = 0.0f;
	dslsn.vPosition.z = 0.0f;
	dslsn.vVelocity.x = 0.0f;
	dslsn.vVelocity.y = 0.0f;
	dslsn.vVelocity.z = 0.0f;
...
// Desc: Updates the local position, and sends the new position to all the
// players
//-----------------------------------------------------------------------------
static HRESULT SendLocalPosition(HWND /* hDlg */)
{
	...
	DPNHANDLE hAsync;
	g_pDP->SendTo(DPNID_ALL_PLAYERS_GROUP, &bufferDesc, 1, 0, NULL, &hAsync,
...
static HRESULT OnInitDialog(HWND hDlg)
{
	...
	DWORD dwVertScrollBar = static_cast<DWORD>(GetSystemMetrics(SM_CXVSCROLL));
	GetClientRect(hListView, &rctListView);
	DWORD dwListViewWidth = rctListView.right - dwVertScrollBar;
	...
	column.cx = static_cast<int>(dwListViewWidth * 5 / 10);
	column.pszText = strHeader;
	column.cchTextMax = static_cast<int>(_tcslen(strHeader));
	...
	SendDlgItemMessage(
		hDlg, IDC_HORIZONTAL_SLIDER, TBM_SETRANGE, 0, MAKELONG(0, MAP_WIDTH));
	SendDlgItemMessage(
		hDlg, IDC_VERTICAL_SLIDER, TBM_SETRANGE, 0, MAKELONG(0, MAP_HEIGHT));

	SendDlgItemMessage(
		hDlg, IDC_HORIZONTAL_SLIDER, TBM_SETPOS, TRUE, START_POSITION_X);
	SendDlgItemMessage(
		hDlg, IDC_VERTICAL_SLIDER, TBM_SETPOS, TRUE, START_POSITION_Y);
...
//-----------------------------------------------------------------------------
static HRESULT VoiceConfigEnumCompressionCodecs(HWND hDlg)
{
	...
	hr = pVoiceClient->GetCompressionTypes(NULL, &dwSize, &dwNumElements, 0);
	if (hr != DVERR_BUFFERTOOSMALL && FAILED(hr)) {
		return DXTRACE_ERR(TEXT("GetCompressionTypes"), hr);
	}

	LPBYTE pBuffer = new BYTE[dwSize];
	if (FAILED(hr = pVoiceClient->GetCompressionTypes(
				   pBuffer, &dwSize, &dwNumElements, 0))) {
		return DXTRACE_ERR(TEXT("GetCompressionTypes"), hr);
	}
	...
	for (DWORD dwIndex = 0; dwIndex < dwNumElements; dwIndex++) {
		...
		DXUtil_ConvertWideStringToGeneric(
			strName, pdvCompressionInfo[dwIndex].lpszName);
		LONG lIndex =
			(LONG)SendMessage(hPulldown, CB_ADDSTRING, 0, (LPARAM)strName);

		LPGUID pGuid = new GUID;
		(*pGuid) = pdvCompressionInfo[dwIndex].guidType;
		SendMessage(hPulldown, CB_SETITEMDATA, static_cast<WPARAM>(lIndex),
			(LPARAM)pGuid);
	...
	SendMessage(hPulldown, CB_SETCURSEL, static_cast<WPARAM>(lFirst), 0);
...
//-----------------------------------------------------------------------------
static VOID VoiceConfigDlgOnOK(HWND hDlg)
{
	DWORD dwSliderPos;
	...
	if (IsDlgButtonChecked(hDlg, IDC_PLAYBACK_DEFAULT)) {
		...
		dwSliderPos = (DWORD)SendDlgItemMessage(
			hDlg, IDC_PLAYBACK_SLIDER, TBM_GETPOS, 0, 0);
		g_dvClientConfig.lPlaybackVolume = DSBVOLUME_MIN +
			(LONG)(dwSliderPos / 100.0f * (DSBVOLUME_MAX - DSBVOLUME_MIN));
	}
	...
	if (IsDlgButtonChecked(hDlg, IDC_RECORD_AUTO)) {
		...
		dwSliderPos = (DWORD)SendDlgItemMessage(
			hDlg, IDC_RECORD_SLIDER, TBM_GETPOS, 0, 0);
		g_dvClientConfig.lRecordVolume = DSBVOLUME_MIN +
			(LONG)(dwSliderPos / 100.0f * (DSBVOLUME_MAX - DSBVOLUME_MIN));
	}
	...
	if (IsDlgButtonChecked(hDlg, IDC_THRESHOLD_AUTO)) {
		...
		dwSliderPos = (DWORD)SendDlgItemMessage(
			hDlg, IDC_THRESHOLD_SLIDER, TBM_GETPOS, 0, 0);
	...
	if (IsDlgButtonChecked(hDlg, IDC_QUALITY_DEFAULT)) {
		...
		dwSliderPos = (DWORD)SendDlgItemMessage(
			hDlg, IDC_QUALITY_SLIDER, TBM_GETPOS, 0, 0);
	...
	if (IsDlgButtonChecked(hDlg, IDC_AGGRESSIVENESS_DEFAULT)) {
		g_dvClientConfig.dwBufferAggressiveness =
			DVBUFFERAGGRESSIVENESS_DEFAULT;
	} else {
		dwSliderPos = (DWORD)SendDlgItemMessage(
			hDlg, IDC_AGGRESSIVENESS_SLIDER, TBM_GETPOS, 0, 0);
	...
	if (g_bHostPlayer) {
		...
		LRESULT lCurSelection =
			SendDlgItemMessage(hDlg, IDC_COMPRESSION_COMBO, CB_GETCURSEL, 0, 0);
		if (lCurSelection != CB_ERR) {
			LPGUID pGuidCT =
				(LPGUID)SendDlgItemMessage(hDlg, IDC_COMPRESSION_COMBO,
					CB_GETITEMDATA, static_cast<WPARAM>(lCurSelection), 0);
			if (pGuidCT != NULL) {
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK VoiceConfigDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendDlgItemMessage(
			hDlg, IDC_PLAYBACK_SLIDER, TBM_SETRANGE, FALSE, MAKELONG(0, 100));
		SendDlgItemMessage(
			hDlg, IDC_RECORD_SLIDER, TBM_SETRANGE, FALSE, MAKELONG(0, 100));
		SendDlgItemMessage(hDlg, IDC_QUALITY_SLIDER, TBM_SETRANGE, FALSE,
			MAKELONG(DVBUFFERQUALITY_MIN, DVBUFFERQUALITY_MAX));
		SendDlgItemMessage(hDlg, IDC_THRESHOLD_SLIDER, TBM_SETRANGE, FALSE,
			MAKELONG(DVTHRESHOLD_MIN, DVTHRESHOLD_MAX));
		SendDlgItemMessage(hDlg, IDC_AGGRESSIVENESS_SLIDER, TBM_SETRANGE, FALSE,
			MAKELONG(DVBUFFERAGGRESSIVENESS_MIN, DVBUFFERAGGRESSIVENESS_MAX));
		...
		if (g_dvClientConfig.lPlaybackVolume == DVPLAYBACKVOLUME_DEFAULT) {
			CheckRadioButton(hDlg, IDC_PLAYBACK_DEFAULT, IDC_PLAYBACK_SET,
				IDC_PLAYBACK_DEFAULT);
		} else {
			dwSliderPos =
				(DWORD)((g_dvClientConfig.lPlaybackVolume - DSBVOLUME_MIN) *
					100.0f / (DSBVOLUME_MAX - DSBVOLUME_MIN));
			CheckRadioButton(
				hDlg, IDC_PLAYBACK_DEFAULT, IDC_PLAYBACK_SET, IDC_PLAYBACK_SET);
			SendDlgItemMessage(hDlg, IDC_PLAYBACK_SLIDER, TBM_SETPOS, TRUE,
				static_cast<LPARAM>(dwSliderPos));
		}

		// Set the record controls
		if (g_dvClientConfig.dwFlags & DVCLIENTCONFIG_AUTORECORDVOLUME) {
			CheckRadioButton(
				hDlg, IDC_RECORD_DEFAULT, IDC_RECORD_AUTO, IDC_RECORD_AUTO);
		} else if (g_dvClientConfig.lRecordVolume == DVPLAYBACKVOLUME_DEFAULT) {
			CheckRadioButton(
				hDlg, IDC_RECORD_DEFAULT, IDC_RECORD_AUTO, IDC_RECORD_DEFAULT);
		} else {
			dwSliderPos =
				(DWORD)((g_dvClientConfig.lRecordVolume - DSBVOLUME_MIN) *
					100.0f / (DSBVOLUME_MAX - DSBVOLUME_MIN));
			CheckRadioButton(
				hDlg, IDC_RECORD_DEFAULT, IDC_RECORD_AUTO, IDC_RECORD_SET);
			SendDlgItemMessage(hDlg, IDC_RECORD_SLIDER, TBM_SETPOS, TRUE,
				static_cast<LPARAM>(dwSliderPos));
		}

		// Set the threshold controls
		if (g_dvClientConfig.dwFlags & DVCLIENTCONFIG_AUTOVOICEACTIVATED) {
			CheckRadioButton(hDlg, IDC_THRESHOLD_DEFAULT, IDC_THRESHOLD_AUTO,
				IDC_THRESHOLD_AUTO);
		} else if (g_dvClientConfig.dwThreshold == DVTHRESHOLD_DEFAULT) {
			CheckRadioButton(hDlg, IDC_THRESHOLD_DEFAULT, IDC_THRESHOLD_AUTO,
				IDC_THRESHOLD_DEFAULT);
		} else {
			CheckRadioButton(hDlg, IDC_THRESHOLD_DEFAULT, IDC_THRESHOLD_AUTO,
				IDC_THRESHOLD_SET);
			SendDlgItemMessage(hDlg, IDC_THRESHOLD_SLIDER, TBM_SETPOS, TRUE,
				static_cast<LPARAM>(g_dvClientConfig.dwThreshold));
		}

		// Set the quality controls
		if (g_dvClientConfig.dwBufferQuality == DVBUFFERQUALITY_DEFAULT) {
			CheckRadioButton(hDlg, IDC_QUALITY_DEFAULT, IDC_QUALITY_SET,
				IDC_QUALITY_DEFAULT);
		} else {
			CheckRadioButton(
				hDlg, IDC_QUALITY_DEFAULT, IDC_QUALITY_SET, IDC_QUALITY_SET);
			SendDlgItemMessage(hDlg, IDC_QUALITY_SLIDER, TBM_SETPOS, TRUE,
				static_cast<LPARAM>(g_dvClientConfig.dwBufferQuality));
		}

		// Set the aggressiveness controls
		if (g_dvClientConfig.dwBufferAggressiveness ==
			DVBUFFERAGGRESSIVENESS_DEFAULT) {
			CheckRadioButton(hDlg, IDC_AGGRESSIVENESS_DEFAULT,
				IDC_AGGRESSIVENESS_SET, IDC_AGGRESSIVENESS_DEFAULT);
		} else {
			CheckRadioButton(hDlg, IDC_AGGRESSIVENESS_DEFAULT,
				IDC_AGGRESSIVENESS_SET, IDC_AGGRESSIVENESS_SET);
			SendDlgItemMessage(hDlg, IDC_AGGRESSIVENESS_SLIDER, TBM_SETPOS,
				TRUE,
				static_cast<LPARAM>(g_dvClientConfig.dwBufferAggressiveness));
		}

		if (!g_bHostPlayer || g_bVoiceSessionInProgress) {
			// We are are not the host player then disable all the server only
			// options
...
#define NM_RELEASEDCAPTURE (NM_FIRST - 16)
		...
		if (((LPNMHDR)lParam)->code == NM_RELEASEDCAPTURE) {
			// If this is a release capture from a slider, then automatically
			// check its 'Set' radio button.
			...
			case IDC_THRESHOLD_SLIDER:
				CheckRadioButton(hDlg, IDC_THRESHOLD_DEFAULT,
					IDC_THRESHOLD_AUTO, IDC_THRESHOLD_SET);
				break;
	...
	case WM_DESTROY: {
		...
		int nCount = (int)SendDlgItemMessage(
			hDlg, IDC_COMPRESSION_COMBO, CB_GETCOUNT, 0, 0);
		for (int i = 0; i < nCount; i++) {
			pGuid = (LPGUID)SendDlgItemMessage(hDlg, IDC_COMPRESSION_COMBO,
				CB_GETITEMDATA, static_cast<WPARAM>(i), 0);
			SAFE_DELETE(pGuid);
		}
		break;
	}
...
//-----------------------------------------------------------------------------
static HRESULT DisplayPlayersInChat(HWND hDlg)
{
	if (hDlg == NULL) {
		return S_OK;
	}
	...
	do {
		...
		hr = g_pDP->EnumPlayersAndGroups(
			g_pPlayers, &dwNumPlayers, DPNENUM_PLAYERS);
		if (SUCCEEDED(hr)) {
			break;
		}
		...
		if (hr == DPNERR_BUFFERTOOSMALL) {
			...
			g_pPlayers = (DPNID*)realloc(
				g_pPlayers, sizeof(DPNID) * g_dwPlayersArraySize);
		}
	...
	for (DWORD i = 0; i < dwNumPlayers; i++) {
		...
		do {
			...
			// Call GetPlayerContext() until it returns something other than
			// DPNERR_NOTREADY DPNERR_NOTREADY will be returned if the callback
			// thread has not yet returned from DPN_MSGID_CREATE_PLAYER, which
			// sets the player's context
			hr = g_pDP->GetPlayerContext(
				g_pPlayers[i], (LPVOID*)&pPlayerInfo, 0);
		...
		lvItem.lParam = static_cast<LPARAM>(g_pPlayers[i]);
		lvItem.cchTextMax =
			static_cast<int>(_tcslen(pPlayerInfo->strPlayerName));
		...
		lvItem.cchTextMax = static_cast<int>(_tcslen(strStatus));
...
//-----------------------------------------------------------------------------
static VOID DrawDotOnGrid(
	HWND hWndGrid, HDC hDC, BOOL bLocalDot, FLOAT fPosX, FLOAT fPosY)
{
	RECT rc;
	COLORREF crDotColor;
	...
	DWORD dwX = (DWORD)(fPosX * (rc.left + rc.right - 3)) + 1;
	DWORD dwY = (DWORD)(fPosY * (rc.top + rc.bottom - 3)) + 1;
	...
	SetPixel(
		hDC, static_cast<int>(dwX - 1), static_cast<int>(dwY - 1), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX - 1), static_cast<int>(dwY + 0), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX - 1), static_cast<int>(dwY + 1), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX + 0), static_cast<int>(dwY - 1), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX + 0), static_cast<int>(dwY + 0), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX + 0), static_cast<int>(dwY + 1), crDotColor);
	SetPixel(
		hDC, static_cast<int>(dwX + 1), static_cast<int>(dwY - 1), crDotColor);
...
//-----------------------------------------------------------------------------
static HRESULT UpdateGrid(HWND hDlg)
{
	HRESULT hr;
	DWORD dwNumPlayers;
	APP_PLAYER_INFO* pPlayerInfo = NULL;
	...
	RedrawWindow(hWndGrid, NULL, NULL,
	...
	for (DWORD i = 0; i < dwNumPlayers; i++) {
		...
		if (pPlayerInfo->dpnidPlayer == g_LocalPlayerDPNID) {
			...
			if (g_pDSListener) {
				if (FAILED(hr = g_pDSListener->SetPosition(
							   f3DPosX, 0.0f, f3DPosZ, DS3D_DEFERRED)))
			...
			if (pPlayerInfo->pDSBVoice) {
				if (FAILED(hr = pPlayerInfo->pDSBVoice->SetPosition(
							   f3DPosX, 0.0f, f3DPosZ, DS3D_DEFERRED))) {
					return DXTRACE_ERR(TEXT("SetPosition"), hr);
				}

				DXUtil_Trace(
					TEXT("Setting player 0x%0.8x to (%0.1ff,0.0f,%0.1ff)\n"),
...
//-----------------------------------------------------------------------------
static INT_PTR CALLBACK VoiceDlgProc(
	HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
	case WM_INITDIALOG: {
		...
		if (FAILED(g_hrDialog = g_pNetVoice->Init(hDlg, g_bHostPlayer, TRUE,
					   g_pDP, DVSESSIONTYPE_PEER, &g_guidDVSessionCT,
					   &g_dvClientConfig, g_pDS))) {
			if (g_hrDialog == DVERR_USERBACK) {
				MessageBox(hDlg,
					TEXT("The user backed out of the wizard.  ") TEXT(
						"This simple sample does not handle this case, so ")
						TEXT("the sample will quit."),
			...
			if (g_hrDialog == DVERR_USERCANCEL) {
				MessageBox(hDlg,
					TEXT("The user canceled the wizard. ") TEXT(
						"This simple sample does not handle this case, so ")
						TEXT("the sample will quit."),
					TEXT("DirectPlay Sample"), MB_OK);
				g_hrDialog = S_OK;
			...
			if (g_hrDialog == DVERR_ALREADYPENDING) {
				MessageBox(hDlg,
					TEXT(
						"Another instance of the Voice Setup Wizard is already running. ")
						TEXT(
							"This simple sample does not handle this case, so ")
							TEXT("the sample will quit."),
					TEXT("DirectPlay Sample"), MB_OK);
		...
		if (g_pNetVoice->IsHalfDuplex()) {
			MessageBox(hDlg,
				TEXT("You are running in half duplex mode. ")
	...
	case WM_COMMAND:
		...
		case IDC_SETUP: {
			...
			DWORD dwResult = (DWORD)DialogBox(g_hInst,
				MAKEINTRESOURCE(IDD_VOICE_SETUP), hDlg, VoiceConfigDlgProc);
	...
	case WM_TIMER: {
		...
		DWORD dwNumPlayers =
			static_cast<DWORD>(ListView_GetItemCount(hListView));
		...
		for (iIndex = 0; iIndex < dwNumPlayers; iIndex++) {
			...
			lvItem.iItem = static_cast<int>(iIndex);
			ListView_GetItem(hListView, &lvItem);

			DPNID dpnidPlayer = (DPNID)lvItem.lParam;

			PLAYER_LOCK(); // enter player context CS

			// Get the player context accosicated with this DPNID
			hr = g_pDP->GetPlayerContext(
				dpnidPlayer, (LPVOID* const)&pPlayerInfo, 0);

			PLAYER_ADDREF(
				pPlayerInfo); // addref player, since we are using it now
			PLAYER_UNLOCK();  // leave player context CS
	...
	case WM_PAINT: {
		...
		for (iIndex = 0; iIndex < dwNumPlayers; iIndex++) {
			...
			// Get the player context accosicated with this DPNID
			HRESULT hr = g_pDP->GetPlayerContext(
				dpnidPlayer, (LPVOID* const)&pPlayerInfo, 0);
	...
	case WM_NOTIFY: {
		...
		if (pnm->hdr.code == NM_CUSTOMDRAW &&
			(pnm->hdr.idFrom == IDC_VERTICAL_SLIDER ||
				pnm->hdr.idFrom == IDC_HORIZONTAL_SLIDER)) {
			LPNMLVCUSTOMDRAW lplvcd = (LPNMLVCUSTOMDRAW)lParam;

			if (lplvcd->nmcd.dwDrawStage == CDDS_PREPAINT) {
				POINT pt;

				pt.x = (LONG)SendDlgItemMessage(
					hDlg, IDC_HORIZONTAL_SLIDER, TBM_GETPOS, 0, 0);
				pt.y = (LONG)SendDlgItemMessage(
					hDlg, IDC_VERTICAL_SLIDER, TBM_GETPOS, 0, 0);

				if (NULL == g_pPlayerLocal)
					return CDRF_DODEFAULT;

				// Don't do anything if nothing has changed.
				if (pt.x != g_pPlayerLocal->pt.x ||
					pt.y != g_pPlayerLocal->pt.y) {
					...
					if (dwCurTime - s_dwLastSendTime > 50) {
						...
						if (FAILED(g_hrDialog = SendLocalPosition(hDlg))) {
							...
							MessageBox(hDlg,
								TEXT("Error updating the local position.  ")
									TEXT("Sample will now exit."),
								TEXT("DirectPlayVoice Sample"),
								MB_OK | MB_ICONERROR);
							EndDialog(hDlg, 0);
						}
	...
	case WM_LBUTTONDOWN: {
		...
		if (pt.x > rc.left && pt.x < rc.right && pt.y > rc.top &&
			...
			SendDlgItemMessage(
				hDlg, IDC_HORIZONTAL_SLIDER, TBM_SETPOS, TRUE, pt.x);
			g_pPlayerLocal->pt.y = pt.y;
			SendDlgItemMessage(
				hDlg, IDC_VERTICAL_SLIDER, TBM_SETPOS, TRUE, pt.y);
			...
			if (FAILED(g_hrDialog = SendLocalPosition(hDlg))) {
				...
				MessageBox(hDlg,
					TEXT("Error updating the local position.  ")
						TEXT("Sample will now exit."),
					TEXT("DirectPlayVoice Sample"), MB_OK | MB_ICONERROR);
				EndDialog(g_hDlg, 0);
			}
...
//-----------------------------------------------------------------------------
static HRESULT CALLBACK DirectPlayVoiceServerMessageHandler(
	LPVOID /* lpvUserContext */, DWORD /* dwMessageType */,
	LPVOID /* lpMessage */)
{
...
//-----------------------------------------------------------------------------
static void SetPlayerTalking(APP_PLAYER_INFO* pPlayerInfo, BOOL bTalking)
{
	if (pPlayerInfo) {
...
//-----------------------------------------------------------------------------
static HRESULT CALLBACK DirectPlayVoiceClientMessageHandler(
	LPVOID lpvUserContext, DWORD dwMessageType, LPVOID lpMessage)
{
	// Try not to stay in this message handler for too long, otherwise
	// there will be a backlog of data.  The best solution is to
	// queue data as it comes in, and then handle it on other threads.
	...
	case DVMSGID_CREATEVOICEPLAYER: {
		DVMSG_CREATEVOICEPLAYER* pCreateVoicePlayerMsg =
			(DVMSG_CREATEVOICEPLAYER*)lpMessage;
		APP_PLAYER_INFO* pPlayerInfo = NULL;
		...
		// Get the player context accosicated with this DPNID
		hr = g_pDP->GetPlayerContext(
			pCreateVoicePlayerMsg->dvidPlayer, (LPVOID* const)&pPlayerInfo, 0);
		...
		pPlayerInfo->bHalfDuplex =
			((pCreateVoicePlayerMsg->dwFlags & DVPLAYERCAPS_HALFDUPLEX) != 0);
		...
		if ((DVID)pCreateVoicePlayerMsg->dvidPlayer != g_LocalPlayerDPNID) {
			...
			LPDIRECTPLAYVOICECLIENT pVoiceClient =
				g_pNetVoice->GetVoiceClient();

			EnterCriticalSection(&pPlayerInfo->csPlayer);

			if (FAILED(hr = pVoiceClient->Create3DSoundBuffer(
						   pCreateVoicePlayerMsg->dvidPlayer, NULL, 0, 0,
						   &pPlayerInfo->pDSBVoice))) {
				return DXTRACE_ERR(TEXT("Create3DSoundBuffer"), hr);
			}

			if (FAILED(hr = pPlayerInfo->pDSBVoice->SetMinDistance(
						   5.0f, DS3D_DEFERRED))) {
				return DXTRACE_ERR(TEXT("SetMinDistance"), hr);
			}

			if (FAILED(hr = pPlayerInfo->pDSBVoice->SetMaxDistance(
						   20.0f, DS3D_DEFERRED))) {
				return DXTRACE_ERR(TEXT("SetMaxDistance"), hr);
			}

			FLOAT f3DPosX =
				(FLOAT)(pPlayerInfo->pt.x - START_POSITION_X) / 10.0f;
			FLOAT f3DPosZ =
				(FLOAT)(pPlayerInfo->pt.y - START_POSITION_Y) / 10.0f;

			if (FAILED(hr = pPlayerInfo->pDSBVoice->SetPosition(
						   (FLOAT)f3DPosX, 0.0f, f3DPosZ, DS3D_DEFERRED))) {
				return DXTRACE_ERR(TEXT("SetPosition"), hr);
			}

			if (FAILED(hr = pPlayerInfo->pDSBVoice->SetVelocity(
						   0.0f, 0.0f, 0.0f, DS3D_DEFERRED))) {
	...
	case DVMSGID_DELETEVOICEPLAYER: {
		...
		if (pMsg->dvidPlayer != g_LocalPlayerDPNID) {
			...
			LPDIRECTPLAYVOICECLIENT pVoiceClient =
				g_pNetVoice->GetVoiceClient();
			if (FAILED(
					hr = pVoiceClient->Delete3DSoundBuffer(
						pPlayerInfo->dpnidPlayer, &pPlayerInfo->pDSBVoice))) {
		...
		// Release our extra reference on the player info that we have for the
		// voice context value.
		//
		PLAYER_LOCK();
	...
	case DVMSGID_LOSTFOCUS: {
		...
		wsprintf(strWindowName, TEXT("%s%s%s"), g_strAppName,
			...
			(dwMessageType == DVMSGID_LOSTFOCUS) ? TEXT(" (Focus Lost)") :
                                                   TEXT(""));
	...
	LPSTR /* pCmdLine */, INT /* nCmdShow */)
{
	...
	RegCreateKeyEx(HKEY_CURRENT_USER, DPLAY_SAMPLE_KEY, 0, NULL,
		REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hDPlaySampleRegKey,
		NULL);
	...
	// Create helper class
	g_pNetConnectWizard =
		new CNetConnectWizard(hInst, NULL, g_strAppName, &g_guidApp);
	g_pNetVoice = new CNetVoice(DirectPlayVoiceClientMessageHandler,
		DirectPlayVoiceServerMessageHandler);

	if (FAILED(hr = InitDirectPlay())) {
		DXTRACE_ERR(TEXT("InitDirectPlay"), hr);
		MessageBox(NULL,
			TEXT("Failed initializing IDirectPlay8Peer. ")
	...
	if (g_bWasLobbyLaunched &&
		g_pNetConnectWizard->HaveConnectionSettingsFromLobby()) {
		...
		if (FAILED(hr = g_pNetConnectWizard->ConnectUsingLobbySettings())) {
			...
			MessageBox(NULL,
				TEXT("Failed to connect using lobby settings. ")
					TEXT("The sample will now quit."),
				TEXT("DirectPlay Sample"), MB_OK | MB_ICONERROR);
		...
		if (FAILED(hr)) {
			...
			MessageBox(NULL,
				TEXT("Multiplayer connect failed. ")
					TEXT("The sample will now quit."),
				TEXT("DirectPlay Sample"), MB_OK | MB_ICONERROR);
			bConnectSuccess = FALSE;
			...
			_tcscpy(g_strPreferredProvider,
				g_pNetConnectWizard->GetPreferredProvider());
			g_bHostPlayer = g_pNetConnectWizard->IsHostPlayer();

			// Write information to the registry
			DXUtil_WriteStringRegKey(
				hDPlaySampleRegKey, TEXT("Player Name"), g_strLocalPlayerName);
			DXUtil_WriteStringRegKey(
				hDPlaySampleRegKey, TEXT("Session Name"), g_strSessionName);
			DXUtil_WriteStringRegKey(hDPlaySampleRegKey,
				TEXT("Preferred Provider"), g_strPreferredProvider);
		}
	}
	...
	if (bConnectSuccess) {
		...
		// Ask the user for DirectPlayVoice setup params
		DWORD dwResult = (DWORD)DialogBox(
			hInst, MAKEINTRESOURCE(IDD_VOICE_SETUP), NULL, VoiceConfigDlgProc);
		if (dwResult != IDCANCEL) {
			g_pNetVoice->ChangeVoiceClientSettings(&g_dvClientConfig);
		}
	}
	...
	if (bConnectSuccess) {
		...
		if (FAILED(g_hrDialog)) {
			if (g_hrDialog == DPNERR_CONNECTIONLOST) {
				...
				MessageBox(NULL,
					TEXT("An error occured during the game. ")
						TEXT("The sample will now quit."),
					TEXT("DirectPlay Sample"), MB_OK | MB_ICONERROR);
			}
		}
